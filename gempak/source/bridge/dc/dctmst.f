	SUBROUTINE DC_TMST  ( isffln, dattim, stid, addstn, cirflg,
     +			      datflg, iret )
C************************************************************************
C* DC_TMST								*
C*									*
...
C*									*
C* DC_TMST  ( ISFFLN, DATTIM, STID, ADDSTN, CIRFLG, DATFLG, IRET )	*
C*									*
C* Input parameters:							*
...
C*					  0 = normal return		*
C*					 -23 = time cannot be set	*
C*					 -24 = station cannot be set	*
C**									*
C* Log:									*
...
C* D. Kidwell/NCEP       1/98	From RA_TMST, added ISTNM check         *
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C*
	CHARACTER	stat*4, coun*4, stnid*8
...
C
	IF  ( ( iret .ne. 0 )  .and. ( addstn ) )  THEN
	    stnid = stid
	    CALL ST_NUMB ( stnid, istnm, ier )
	    IF ( ier .eq. 0 ) stnid = ' '
	    CALL SF_ASTN  ( isffln, 1, stnid, istnm, slat, slon, selv,
