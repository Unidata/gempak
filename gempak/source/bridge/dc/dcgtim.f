	SUBROUTINE DC_GTIM  ( isdtar, btime, irhour, irmin, offtim,
     +			      irdtar, rtime, iret )
C************************************************************************
C* DC_GTIM								*
C*									*
...
C*									*
C* DC_GTIM  ( ISDTAR, BTIME, IRHOUR, IRMIN, OFFTIM, IRDTAR, RTIME,	*
C*	      IRET )							*
C*									*
C* Input parameters:							*
...
C*	OFFTIM		LOGICAL		Flag for decoding off-time rpts	*
C*									*
C* Output parameters:							*
...
C*					  0 = normal return		*
C*					-15 = rpt/bultn time discrepancy*
C*					-16 = invalid time		*
C**									*
C* Log:									*
C* M. desJardins/GSFC	 4/90						*
C* J. Whistler/SSAI	 4/91	Subtract 24 from ibhour when > 24	*
C* J. Whistler/SSAI	 7/91	Changed iyear to ioyear where mistyped	*
...
C* F. J. Yen/NCEP	 6/99	Renamed from RA_RTIM; Handled off-time	*
C*				reports & used return codes for dc.err	*
C* D. Kidwell/NCEP	 9/02	Added store to irdtar when iret is -15  *
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C------------------------------------------------------------------------
	...
	subday = .false.
C
C*	Set minutes for GEMPAK time depending on whether off-time
C*	reports are to be decoded.  Currently a maximum of 2
C*	off-time reports are allowed.
C
	IF ( offtim ) THEN
	    IF ( irmin .ge. 45 .or. irmin .lt. 5 ) THEN
		iomin = 0
	      ELSE
	 	IF ( irmin .ge. 5 .and. irmin .lt. 25 ) THEN
		    iomin = 20
		  ELSE
		    iomin = 40
		END IF
