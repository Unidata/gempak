	SUBROUTINE CS_DCOD  ( curtim, gemfil, stntbl, prmfil, iadstn,
     +			      maxtim, nhours, iret )
C************************************************************************
C* CS_DCOD								*
C*									*
C* This subroutine decodes WSUS4[012] and WCPA3[1-5] PHFO convective    *
C* sigmets and outlook reports (issued by KMKC and PHFO ) and writes    *
C* the data to an ASCII file.	                                        *
C*									*
C* CS_DCOD  ( CURTIM, GEMFIL, STNTBL, PRMFIL, IADSTN, MAXTIM, NHOURS,   *
C*	      IRET )    						*
C*									*
C* Input parameters:							*
...
C
     ...
     +		                   oristn, bultim, bbb, nchar, ierr )
		...
		IF ( ierr .ne. 0 ) THEN
		    ...
		    CALL DC_WLOG ( 2, 'DCCSIG', 1, errstr, ier )
		    good = .false.
		END IF
C
		IF ( good ) THEN
...
C
	        IF ( good ) THEN
C
C*		    Decode the convective sigmet/outlook report.
C
 		    CALL CS_DECD ( work ( :lenb ), lenb, iotarr,
