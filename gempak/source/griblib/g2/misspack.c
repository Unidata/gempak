...
//
      //if (rmin != rmax) {
        ...
        if (idrstmpl[1] == 0) {        //  No binary scaling
           imin=(g2int)G_NINT(rmin*dscale);
           //imax=(g2int)rint(rmax*dscale);
           rmin=(g2float)imin;
           for ( j=0; j<ndpts; j++) {
              if (ifldmiss[j] == 0) {
                jfld[nonmiss]=(g2int)G_NINT(fld[j]*dscale)-imin;
                nonmiss++;
              }
           }
        }
           ...
           for ( j=0; j<ndpts; j++ ) {
              if (ifldmiss[j] == 0) {
                jfld[nonmiss]=(g2int)G_NINT(((fld[j]*dscale)-rmin)*bscale);
                nonmiss++;
              }
           }
        }
        ...
        if (igmax != 0) {
           ...
           ceiltmp=ceil(temp);
           nbitsgref=(g2int)ceiltmp;
           // reset the ref values of any "missing only" groups.
           powtmp=int_power(2.,nbitsgref);
           mtemp=(g2int)powtmp;
           for ( j=0; j<ngroups; j++ ) {
