...
//           int g2_locfld(unsigned char *cgrib, g2int ifldnum)
//   INPUT ARGUMENTS:
...
//$$$
{
      ...
      g2int have3,have4,have5,have6,have7,ierr;
      g2int numfld,j,istart,iofst,ipos;
...
//
	  ...
	  *pos5 = cgrib + ipos;
        ...
        if (isecnum == 6) {
            ...
            gbit(cgrib,&bmapnum,iofst,8);      // Get BitMap Indicator
	    if ( bmapnum == 254) {	// use previously specified bitmap
	       if ( pos6 == NULL ) {
		  printf("g2_locfld: Prev bit-map specified, but none exist.\n");
                  ierr=17;
	          g2_free(lgfld);
                  return(ierr);
	       }
        ...
        //   one requested.
        //
        if (isecnum==7 && numfld==ifldnum ) {
          iofst=iofst-40;       // reset offset to beginning of section
	  /* use lensec to write section 7 */
          if (have3 && have4 && have5 && have6) {
	     have7 = 1;
	     *len7 = lensec;
	     *pos7 = cgrib + ipos;
	     /* when reading back from GEMPAK grid file, will inflate with
	  ...
	  g2_free(lgfld);
        ...
        //
        //  return when all sections have been processed
        //
        if (have3 && have4 && have5 && have6 && have7) {
	    g2_free(lgfld);
            return(ierr);
	}
