	SUBROUTINE AW_ACHR ( bdata, irept, lenbyt, iff, iret )
C************************************************************************
C* AW_ACHR                                                              *
C*                                                                      *
C* This subroutine is the alphanumeric characters block.  It transmits  *
C* textual information to be placed on displayed products.  This        *
C* subroutine represents mode 5, submode 1.                             *
C* 								        *
C* AW_ACHR ( BDATA, IREPT, LENBYT, IFF, IRET )                        	*
C*								        *
C* Input parameters: 						        *
...
C* A. Hardy/GSC         11/98   Added do loop;fixed logical statements  *
...
C*
	...
	CHARACTER*120   stchar 
        INTEGER         icnt, mcord, ncord, mdlta, ndlta, blmdid, 
     +                  rvblmd, chsiz, itotln, ispch, fntnum, txtwid,
     +                  txtflg 
        INTEGER         bckspc, fwdspc, dwnspc, upspc, hlflin, newlin
        INTEGER         ibte(80000)
	REAL            xlat, ylon, txtsiz
	LOGICAL         spcflg, textfl
C------------------------------------------------------------------------
	IF ( dbug .eq. 'y' ) THEN
            write(flun,*)'In ACHR'
	END IF
...
C
 ...
 59         format(a15,5x,a15)
 60         format(i10,5x,i10)
	END IF
C
C*      Get the Delta M and Delta N.
C
        mdlta = ibits ( ibte( icnt ), 0, 8 ) 
        ndlta = ibits ( ibte( icnt+1 ), 0, 8 ) 
	IF ( mdlta .gt. 127 ) mdlta = mdlta - 256
	IF ( ndlta .gt. 127 ) ndlta = ndlta - 256
	icnt = icnt + 2
        xlat = xlat + mdlta
        ylon = ylon + ndlta
	IF ( dbug .eq. 'y' ) THEN
	    write(flun,*)'DELTA M COORD      DELTA N COORD '
	    write(flun,60)mdlta, ndlta
	END IF
C
