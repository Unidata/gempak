	SUBROUTINE AW_GCUV ( bdata, irept, lenbyt, iff, iret ) 
C************************************************************************
C* AW_GCUV                                                              *
C*                                                                      *
C* This subroutine is the curved vectors block.  This block contains	*
C* joined continuous arcs.  Each arc consists of three coordinate 	*
C* points which must be curve fitted when displayed.  This subroutine   *
C* represents mode 4, submode 10.	                        	*
C* 								        *
C* AW_GCUV ( BDATA, IREPT, LENBYT, IFF, IRET )                        	*
C*								        *
C* Input parameters: 						        *
...
C
 ...
 86             format(5x,i3,15x,i1,10x,f10.2,11x,f10.2)
            END IF 
            IF ( ( lmbit .eq. 1 ) .and. ( kptrk .ge. 3 ) ) THEN 
...
C
 	IF ( ( lmbit .eq. 0 ) .and. ( kptrk .ge. 3 ) ) THEN 
            CALL GLINE ( 'G', kptrk, xpos, ypos, iret )
        END IF
C
