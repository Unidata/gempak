...
C
	...
	LOGICAL         Clickable, national, regional, site_ref,
     +			site_vel, thumbnail, DoMissing, DoColorBar
	REAL		angle1, angle2, angle3, dlatll, dlonll, dlatur,
     +			dlonur, tsize/.857/, xsize(3)/620.,61.,640./,
     +			ysize(3)/620.,46.,380./
...
C
	...
	CALL GSTEXT ( 2, 2, tsize, 1, 111, 1, 1, ier )  !font=2,hw=on,size=tsize,width=1
	IF  ( ier .ne. 0 )  THEN
...
C
	...
	ELSE IF  ( Clickable ) THEN 
   	     CALL GG_MAP2 ('mepowo.gsf',1,1,23,iret )
   	     CALL GSTEXT (2,2,.5,1,111,1,1,ier)    !font=2,hw=on,size=.5,width=1
	...
	ELSE  
	    ...
	    CALL GSTEXT (22,2,0.857,1,111,1,1,ier)  !font=2(+bold),hw=on,size=1,width=1
	    CALL GG_SPLT2 (31,0,0,0.0,0,'cities.rad',iret) !txtcol = 31,mrkr=off,mtyp=0,msiz=0,mwid=0
	    IF  ( iret .ne. 0 )  print *,
     +			"RADMAP:  GG_SPLT2 - iret = ", iret
	ENDIF	

C Add the title if needed.
	IF  ( .not. thumbnail ) THEN 
  	    CALL GSTEXT (22,2,0.857,1,111,1,1,ier) !font=2(+bold),hw=on,size=0.875,width=1
	    CALL GSCOLR ( 1, iret )
