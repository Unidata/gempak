	SUBROUTINE GPNTXT ( ictyp, line, loci, info, itxtp, iret )
C************************************************************************
C* GPNTXT								*
C*									*
C*									*
...
C* T. Lee/SAIC		01/06	Bug fix in text(2) index		*
C* C. Bailey		10/06	Changed call to IN_LINE			*
C* S. Danz		02/07	Build \n string so Linux doesn't convert*
C*				it into a newline character		*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C
	    ...
	    IF ( itxtp .eq. 2 ) THEN
	      CALL ST_NOCC  ( nchar(2), '/', 2, ipoint, iret )
            ...
            END IF
	    ipoint = ipoint + 1
	    ibs = INDEX ( nchar(2)(ipoint+ipos:), nl )
	    DO WHILE ( ibs .ne. 0 ) 
     ...
     +                       ihbrdr, ihrrotn, ihjust, iret )
	        ...
	        CALL GTEXT ( 'M', xm, ym, ctext, rotat, ixoff,
     +			   iyoff, ier )
              ELSE
	        CALL GTEXT ( 'N', xcnt, ycnt, ctext, rotat, ixoff,
     +			   iyoff, ier )
              END IF
	    ELSE IF ( itxtp .eq. 2 ) THEN
	      CALL ST_NOCC  ( nchar(2), '/', 2, ipoint, iret )
	      IF ( iret .eq. 0 ) THEN
		CALL ST_ILST ( nchar(2)(:ipoint-1), '/', IMISSD, 2, iarr,
     +			       num, ier )
