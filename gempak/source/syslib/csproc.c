 ...
 * R. McTaggart-Cowan/UAlb 1/05	speedPatch changes                   	*
 ...
 ***********************************************************************/
{
    ...
    int    msqid, iproc, smchan, semid, initmq;
    key_t  key;
    union  gemsemun arg;
    size_t ii;
 ...
 */
    initmq = 1;
    cgetmq ( iproc, idebug, initmq, &jsatty, &key, mbchan, &smchan, &ierr );

/*
 *  Now that the message queue is established create a corresponding
 *  semaphore and initialize it (unlocked).
 */
