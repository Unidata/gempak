	SUBROUTINE CLDRIV  ( nlvl, clvl, clbl, lincol, lintyp, linwid, 
     +			     linlbl, scflag, zgrid, jhline, jvline, 
     +                       iret )
C************************************************************************
C* CLDRIV								*
C*									*
...
C
	    DO  jj = 1, jsize
		DO  ii = 1, isize 
		    jhline ( (jj-1)*isize+ii ) = 0
		    jvline ( (jj-1)*isize+ii ) = 0
		END DO
	    END DO
C
...
C
		...
		DO WHILE ( i .lt. itop )
		    i = i + 1
		    idire = 3
		    IF ( jhline ( (j)*isize+i ) .eq. 0 ) THEN
			CALL CCCRSS  ( 2, 3, i, j, cval, zgrid, iret )
			IF  ( iret .eq. -6 ) THEN
...
C
		...
		DO WHILE ( j .gt. 1 )
		    j = j - 1
		    idire = 4
		    IF ( jvline ( (j-1)*isize+i+1 ) .eq. 0 ) THEN
			CALL CCCRSS  ( 3, 4, i, j, cval, zgrid, iret )
			IF  ( iret .eq. -6 ) THEN
			    CALL CCDRAW ( cval, clabel, ilabel, 
     +					  intror, i, j, idire, 
     +					  scflag, zgrid, jhline, 
...
C
		...
		DO WHILE ( i .lt. itop )
		    i = i + 1
		    idire = 1
		    IF ( jhline ( (j-1)*isize+i ) .eq. 0 ) THEN
			CALL CCCRSS  ( 1, 4, i, j, cval, zgrid, iret )
			IF  ( iret .eq. -6 ) THEN
			    CALL CCDRAW ( cval, clabel, ilabel, 
     +					  intror, i, j, idire, 
     +					  scflag, zgrid, jhline, 
...
C
		...
		DO WHILE ( j .gt. 1 )
		    j = j - 1
		    idire = 2
		    IF ( jvline ( (j-1)*isize+i ) .eq. 0 ) THEN
			CALL CCCRSS  ( 1, 2, i, j, cval, zgrid, iret )
			IF  ( iret .eq. -6 ) THEN
			    CALL CCDRAW ( cval, clabel, ilabel, 
     +					  intror, i, j, idire, 
     +					  scflag, zgrid, jhline, 
