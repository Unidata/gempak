	SUBROUTINE CCGTSB  ( i, j, ill, jll, cval, box, boxmod, 
     +                       zgrid, iret )
C************************************************************************
C* CCGTSB								*
C*									*
...
C*									*
C* CCGTSB  ( I, J, ILL, JLL, CVAL, BOX, BOXMOD, ZGRID, IRET )		*
C*									*
C* Input parameters:							*
...
C* S. Gilbert/NCEP      03/07   Added zgrid array to call sequence      *
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
	INCLUDE		'CONTUR.CMN'
C*
...
C
	IF  ( ( i .eq. 1 ) .and. ( j .eq. 1 ) )  THEN
	    box (1) = zgrid ( (jll-1)*isize+ill ) - cval
	END IF
	IF  ( ( i .eq. 1 ) .and. ( j .eq. numsub - 1 ) )  THEN
	    box (2) = zgrid ( (jll)*isize+ill ) - cval
	END IF
	IF  ( ( i .eq. numsub - 1 ) .and.
     +		     ( j .eq. numsub - 1 ) )  THEN
	    box (3) = zgrid ( (jll)*isize+ill+1 ) - cval
	END IF
	IF  ( ( i .eq. numsub - 1 ) .and. ( j .eq. 1 ) )  THEN
	    box (4) = zgrid ( (jll-1)*isize+ill+1 ) - cval
	END IF
C
