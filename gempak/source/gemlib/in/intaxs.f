	SUBROUTINE IN_TAXS  ( taxis, maxlbl, npts, times, x, xstrt,
...
C*									*
C* IN_TAXS  ( TAXIS, MAXLBL, NPTS, TIMES, X, XSTRT, XSTOP, XTLBL,	*
...
C*	TIMES (NPTS)	CHAR*		GEMPAK times			*
C*									*
C* Output parameters:							*
...
C*	blanks.  TIMES holds valid date/times.  
C
	IF  ( tarr (1) .eq. ' ' )  THEN
	    tstrt = times (1) ( 1:11 )
	  ELSE
	    CALL TI_STAN  ( tarr (1), times (1), tstrt, ier )
	    IF  ( ier .ne. 0 )  THEN
		tstrt = times (1) ( 1:11 )
	    END IF
	END IF
C
	IF  ( tarr (2) .eq. ' ' )  THEN
	    tstop = times (npts) ( 1:11 )
	  ELSE
	    CALL TI_STAN  ( tarr (2), times (npts), tstop, ier )
	    IF  ( ier .ne. 0 )  THEN
		tstop = times (npts) ( 1:11 )
	    END IF
	END IF
...
C*	  take the closest "nice" value by sorting through KINTVL (the 
C*	  default is 7 days).
C
          kmin = ntrang / 7
          IF ( kmin .eq. 0 ) kmin = 1
...
C
	  ...
	  IF ( itest .gt. maxlbl ) THEN
	    ...
	    RETURN
	  END IF
	  CALL TG_RINC  ( lbl1, tstop, tarr (3), 2, nxlbl, ctlbl, ier )
C*
...
C
     ...
     +             imonth .gt. ilastt (2) .and. iyear .eq. ilastt (1)
     +             .or. il .eq. LLAXIS ) notdon = .false.
	      itnow (1) = iyear
	      itnow (2) = imonth
C
C*	      Make the label.
...
C
C*        Default interval is quarters; input increment is not ignored.
C
	  IF ( itest .gt. 0 .and. itest .lt. maxlbl ) THEN
	    lbl1 = tstrt ( 1:7 ) // '00' // tstrt ( 10: )
	    CALL TG_RINC  ( lbl1, tstop, tarr (3), 2, nxlbl, ctlbl,
...
C
        DO i = 1, npts
          CALL TI_DIFD ( times (i), tstrt, x (i), ier )
        END DO
C
...
C
	...
	IF ( npts .gt. 1 ) THEN
	  DO i = 2, npts
	    CALL TI_DIFD ( times (i), times (i-1), test, ier )
	    test = ABS ( test )
...
C
	IF ( iarr (1) .eq. IMISSD .and. nxlbl .gt. 11 ) 
     +     ilbfrq = nxlbl / 11 + 1
C
