...
void
vwind (char *wind, int *lens, int *ier)
{
  if ( ! vwindinit ) vwindinit = !0;
  in_wind (wind, &wintyp, &winuni, &winclr, ier, *lens, sizeof (wintyp),
	   sizeof (winuni));
}

void
vwind_init (char *wind, int *lens, int *ier)
{
if ( ! vwindinit )
   vwind (wind, lens, ier);
}
...
void
im_nids_symbol (unsigned char *buf, int symbol_off)
{
  int i, ij, ier;
  int ival;
  off_t k;
...
printf("look vad_symbol block %d %d\n",
  ...
  for (i = 0; i < block_len; i += 2)
    {
      signed short *sbytes = (signed short *) (buf + (symbol_off * 2) + i);
  ...
  for (i = 0; i < nlayers; i++)
      ...
      while (icnt < ival)	/*block_len ) */
	{
	  ...
	  switch (packet_code)
	    ...
	    case 20:		/* TVS */
	      /*printf("packet code %d (TVS) found\n",packet_code);*/
	      pblen = packet_code_20 (buf + boff + 6);
	      break;
	    ...
	    case 2:
	      ...
	      break;
	    case 16:		/* Digital Radial Array */
	      pblen = packet_code_16 (buf + boff + 6);
	      pblen -= 4;	/* Digital Radial Array length plus offset+2 */
	      break;
	    case 17:		/* DPA */
