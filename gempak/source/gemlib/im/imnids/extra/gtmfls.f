...
C*
	...
	CHARACTER	carr(3)*(LLMXLN)
	CHARACTER       filnam*(MXNMFL*MXFLSZ), sep*1
	CHARACTER       tmplt*48, gtime*20, ndttm*40
	CHARACTER	strtim*20, endtim*20, tmpfil*160
	CHARACTER	path*48, rtemplt*48, lpath*48, tmpfil2*160,
     +			savfil*160
	LOGICAL		found
C------------------------------------------------------------------------
	iret = 0
...
C
	CALL ST_CLST ( filtyp, '|', ' ', 3, carr, num, ier)
        ...
        CALL FL_TMPL  ( carr(1), found, path, tmplt,
     +			ic, is, if, ir, ii, ier )
C
	IF  ( .not. found )  THEN
C
