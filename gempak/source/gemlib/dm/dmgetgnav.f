...
C* DM_GETGNAV                                                           *
C*                                                                      *
C* This subroutine gets grid navigation information from A2DB db and	*
c* returns it as an array.
C*                                                                      *
...
C**                                                                     *
C* Log:                                                                 *
C* m.gamazaychikov	04/11	Created					*
C************************************************************************
        INCLUDE         'GEMPRM.PRM'
        INCLUDE         'dmcmn.cmn'
        INCLUDE         'dbcmn.cmn'
C
        ...
        REAL            rheadr (*)
C
        CHARACTER  gridid*20, qtype*8, src*10, navstr*256,
     +             gridnav(10)*60, proj*3, dbdatasrc2*20
C------------------------------------------------------------------------
        nword = 0
        sign = 1.
        dangl1 = 90.
C
C* Check that indeed A2DB grid navigation is needed
C
        IF ( dbread ) THEN
     ...
     +          INDEX(fhdnam, 'NAVB') .gt. 0 ) THEN
              ...
              CALL ST_LCUC ( dbdatasrc, dbdatasrc2,ier )
              CALL ST_NULL ( dbdatasrc2, src, lstr, ier )
              CALL ST_NULL ( dbmodel, gridid, lstr, ier )

              CALL DB_GETGNAV ( qtype, src, gridid,
     +                          navstr, lnavstr, iret)
              IF ( iret .ne. 0 ) THEN
                 iret = -7
                 RETURN
...
C
C*  TODO - Set analysis block.  Currently analysis block is not set.
C
