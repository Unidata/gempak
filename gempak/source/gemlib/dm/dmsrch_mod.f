...
C
	...
	DO WHILE (( .not. done ) .and. ( i .le. istop ) )
		...
		IF  ( kheadr ( keyloc (j), i, iflno ) .ne. keyval (j) ) THEN 
     			done = .false.
                        IF  ( ( i .gt. 340 ) .and. ( i .lt. 370 ) ) THEN
                        WRITE (6,*) 'done = ', done 
                        WRITE (6,*) 'j = ', j
                        WRITE (6,*) 'i = ', i
                        WRITE (6,*) 'iflno = ', iflno
                        WRITE (6,*) 'keyloc(j) = ', keyloc(j)
                        WRITE (6,*) 'k = ', kheadr(keyloc(j),i,iflno)
                        WRITE (6,*) 'keyval(j) = ', keyval(j)
                        END IF
                ELSE
                        IF  ( ( i .gt. 340 ) .and. ( i .lt. 370 ) ) THEN 
                        WRITE (6,*) 'j = ', j
