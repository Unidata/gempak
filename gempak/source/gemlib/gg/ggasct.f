...
C* ASCAT data.								*
C*									*
C* GG_ASCT ( FILTYP, DATTIM, KWNINC, KCOLRS, KCOLRS2, NUMC, BRBSIZ,	*
C*	     IBWID, HDSZ, ITYP, ISKIP, INTERV, ITMCLR, ITMWID, IFLGS,	*
C*	     IRET )							*
C*                                                                      *
C* Input parameters:							*
C*	FILTYP		CHAR*		File type; e.g., 'ASCT'		*
C*	DATTIM		CHAR*		Ending time for ASCAT data	*
C*	KWNINC(*)	INTEGER		Wind speed increments		*
C*	KCOLRS(*)	INTEGER		Color for each speed increment  *
C*      KCOLRS2(*)      INTEGER         Colors for KNMI quality control *
C*					fail-flagged winds	 	*
...
C**									*
C* Log:									*
C* G. McFadden/SAIC     01/07   Adapted from GG_QSCT                    *
C* F. J. Yen/NCEP       07/07   Use start&end min in prefs.tbl for range*
C* G. McFadden/SAIC     07/07   Replaced rain-flagged colors parameter  *
C* 				description with KNMI QC fail-flagged   *
C* 				colors description			*
C* G. McFadden/SAIC     07/09   Reduced number of low-res (50km) ambig- *
C*				uities to two... added two hi-res       *
C*				(25km) ambiguities.			*
...
C*
	...
	CHARACTER	label*12, cstmin*(20), cval*(20)
...
C
C*	Set the wind attributes for the ASCAT data.
C
...
C
     ...
     +				       dtime, ier )
C
C*			Read and plot the ASCAT data.
C
...
C
			CALL GG_ASRD ( filtyp, tfile, stime, dattm2,
...
C
	  label = filtyp
	  IF ( filtyp .eq. 'AAMBG1' .or.
     +	       filtyp .eq. 'EXAAMBG1' ) THEN
	    yy = .97 * ( yt - yb ) + yb
    	    CALL GTEXT ( 'N', xx, yy, label, 0., 0, 0, ier )
	    CALL GG_CBAR ( '1/H/UR/.98;.97/.2;.007/1|.714/21//111///hw',
     +			numclr, fwninc, icolrs, ier )
	   ELSE IF ( filtyp .eq. 'AAMBG2' .or.
     +		     filtyp .eq. 'EXAAMBG2' ) THEN
	    yy = .935 * ( yt - yb ) + yb
    	    CALL GTEXT ( 'N', xx, yy, label, 0., 0, 0, ier )
	    CALL GG_CBAR ( '1/H/UR/.98;.935/.2;.007/1|.71/21//111///hw',
     +			numclr, fwninc, icolrs, ier )
           ELSE IF ( filtyp .eq. 'AAMBG1_HI' .or.
     +		     filtyp .eq. 'EXAAMBG1_HI' ) THEN
            yy = .9 * ( yt - yb ) + yb
            CALL GTEXT ( 'N', xx-.02, yy, label, 0., 0, 0, ier )
            CALL GG_CBAR ( '1/H/UR/.98;.90/.2;.007/1|.714/21//111///hw',
     +                  numclr, fwninc, icolrs, ier )
           ELSE IF ( filtyp .eq. 'AAMBG2_HI' .or.
     +		     filtyp .eq. 'EXAAMBG2_HI' ) THEN
            yy = .865 * ( yt - yb ) + yb
            CALL GTEXT ( 'N', xx-.02, yy, label, 0., 0, 0, ier )
            CALL GG_CBAR ( '1/H/UR/.98;.865/.2;.007/1|.72/21//111///hw',
     +                  numclr, fwninc, icolrs, ier )
           ELSE IF ( filtyp .eq. 'AAMBG3_HI' .or. 
     +		     filtyp .eq. 'EXAAMBG3_HI' ) THEN
            yy = .83  * ( yt - yb ) + yb
            CALL GTEXT ( 'N', xx-.02, yy, label, 0., 0, 0, ier )
            CALL GG_CBAR ( '1/H/UR/.98;.83/.2;.007/1|.72/21//111///hw',
     +                  numclr, fwninc, icolrs, ier )
           ELSE IF ( filtyp .eq. 'AAMBG4_HI' .or. 
     +		     filtyp .eq. 'EXAAMBG4_HI' ) THEN
            yy = .795 * ( yt - yb ) + yb
            CALL GTEXT ( 'N', xx-.02, yy, label, 0., 0, 0, ier )
