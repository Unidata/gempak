	SUBROUTINE GG_OSCT ( filtyp, dattim, kwninc, kcolrs, kcolrs2,
...
C* OSCAT data.								*
C*									*
C* GG_OSCT ( FILTYP, DATTIM, KWNINC, KCOLRS, KCOLRS2, NUMC, BRBSIZ,	*
C*	     IBWID, HDSZ, ITYP, ISKIP, INTERV, ITMCLR, ITMWID, IFLGS,	*
C*	     IRET )							*
C*                                                                      *
C* Input parameters:							*
C*	FILTYP		CHAR*		File type; e.g., 'OSCT'		*
C*	DATTIM		CHAR*		Ending time for OSCAT data	*
C*	KWNINC(*)	INTEGER		Wind speed increments		*
C*	KCOLRS(*)	INTEGER		Color for each speed increment  *
C*      KCOLRS2(*)      INTEGER         Colors for rained winds		*
...
C*
	...
	CHARACTER	label*6, label1*9, cstmin*(20), cval*(20)
...
C
C*	Set the wind attributes for the OSCAT data.
C
...
C
C*	Scan the directory for all of the OSCAT data files.
C
...
C
     ...
     +				       dtime, ier )
C
C*			Read and plot the OSCAT data.
C
...
C
			CALL GG_OSRD ( filtyp, tfile, stime, dattm2,
