	SUBROUTINE GG_QSCT ( filtyp, dattim, kwninc, kcolrs, kcolrs2,
...
C* QuikScat data.							*
C*									*
C* GG_QSCT ( FILTYP, DATTIM, KWNINC, KCOLRS, KCOLRS2, NUMC, BRBSIZ,	*
C*	     IBWID, HDSZ, ITYP, ISKIP, INTERV, ITMCLR, ITMWID, IFLGS,	*
C*	     IRET )							*
C*                                                                      *
C* Input parameters:							*
C*	FILTYP		CHAR*		File type; eg., 'QSCT', 'AMBG1'	*
C*	DATTIM		CHAR*		Ending time for QSCT data	*
...
C**									*
C* Log:									*
C* S. Jacobs/NCEP	 7/01						*
C* A. Hardy/SAIC         2/02   Changed call FL_SCND			*
C* M. Li/SAIC		 4/03	Added second color array kcolrs2	*
C* M. Li/SAIC		 5/03	Added time stamp interval		*
C* M. Li/SAIC		 8/03	Added new format and SEAWND data	*
C* B. Yin/SAIC	 	 3/04	Changed SS_GTIM to CSS_GTIM		*
C* F. J. Yen/NCEP	 6/04	Added arrows and removed SEAWND. (CSC)	*
C* F. J. Yen/NCEP	 7/04	Added ambiguities; allowed lower case	*
C*				filtyp; initialized some kcolrs2 values.*
C* T. Lee/SAIC		 9/04	Replaced FL_TMPL with CTB_DTGET		*
C* M. Li/SAIC		10/04	Changed the definition of NUMC		*
C* A. Hardy/NCEP	11/04	Added calls to ST_RNUL			*
C* S. Chiswell/Unidata  11/04   Added QBUF type                       *
C* m.gamazaychikov/SAIC 12/04   Added ion flag to CTB_DTGET CS          *
C* S. Gilbert/NCEP	0/05	ADDED GQSKY and QSSKY			*
...
C
C*	Set the wind attributes for the QuikScat data.
C
...
C
C*	Scan the directory for all of the QuikScat data files.
C
...
C
     ...
     +				       dtime, ier )
C
C*			Read and plot the QuikScat data.
C
...
C
	IF ( filtyp .eq. 'QSCT' .or. filtyp .eq. 'QSCT_HI' .or. 
     +          filtyp .eq. 'QBUF' ) THEN
