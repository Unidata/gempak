	SUBROUTINE TI_MTCH ( match, dattim, times, ntime, minute,
     +			     ipos, iret )
C************************************************************************
C* TI_MTCH								*
C*									*
...
C*									*
C* TI_MTCH  ( MATCH, DATTIM, TIMES, NTIME, MINUTE, IPOS, IRET )		*
C*									*
C* Input parameters:							*
...
C* R. Curtis/EAI	10/00	Changed LLMXTM to MXNMFL in tsort array *
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
	CHARACTER*(*)	dattim, times (*)
C*
	CHARACTER	tsort(MXNMFL+1)*20, ttemp*20, ttemp2*20
C------------------------------------------------------------------------
	iret = 0
C
...
C
	CALL ST_FIND ( dattim, times, ntime, ipos, ier )
C
...
C
	    CALL ST_FIND ( dattim, tsort, nsort, jpos, ier )
C
...
C
		    CALL TI_DIFF ( dattim, ttemp, nmin, ier )
...
C
		IF  ( jpos .ne. 1 )  THEN
		    ttemp = tsort (jpos-1)
		    CALL TI_DIFF ( dattim, ttemp, mmin, ier )
		  ELSE
		    mmin = 610000
		END IF
C
...
C
		IF  ( jpos .ne. nsort )  THEN
		    ttemp2 = tsort (jpos+1)
		    CALL TI_DIFF ( dattim, ttemp2, nmin, ier )
		  ELSE
		    nmin = 610000
		END IF
C
