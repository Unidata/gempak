	SUBROUTINE TI_ADDM  ( idtarr, minuts, jdtarr, iret )
C************************************************************************
C* TI_ADDM								*
C*									*
C* This subroutine adds a specified number of minutes to the time in	*
C* an integer array.  The input and output arrays may be the same array.*
C*									*
C* TI_ADDM  ( IDTARR, MINUTS, JDTARR, IRET )				*
C*									*
C* Input parameters:							*
C*	IDTARR (5)	INTEGER		Time array (YYYY,MM,DD,HH,MM)	*
C*	MINUTS		INTEGER		Number of minutes to add	*
C*									*
C* Output parameters:							*
...
C**									*
C* Log:									*
C* J. Whistler/AWC	 8/95	Based on TI_ADDD			*
