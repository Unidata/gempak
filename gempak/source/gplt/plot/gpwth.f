	SUBROUTINE GPWTH ( sys, np, pwcod, x, y, ixoff, iyoff, iret )
C************************************************************************
C* GPWTH								*
C*									*
C* This subroutine plots past weather symbols in any coordinate system.	*
C* The symbols are drawn using attributes defined by GSPWTH.		*
C*									*
C* GPWTH ( SYS, NP, PWCOD, X, Y, IXOFF, IYOFF, IRET )			*
C*									*
C* Input parameters:							*
...
C* K. Brill/NMC		 7/91	Documentation				*
C* M. desJardins/NMC	 3/92	Clean up				*
C* M. Linda/GSC		 6/96	Bypass buffer /PWTHBF/ when NP > 1	*
C* M. Linda/GSC		12/96	Removed /PWTHBF/, passing reals on down	*
...
C
	    CALL DPWTH ( iwndw, num, pwcod (ib), gx, gy, ixoff (ib),
     +			 iyoff (ib), iret )
	    ib = ie + 1
	END DO
C*
