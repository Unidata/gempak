...
C**									*
C* Log:									*
C* J. Vilardo/RDS	 7/84	GEMPLT Version 3.0			*
...
C* M. Linda/GSC		 6/96	Bypass buffer /MARKBF/ when NP > 1	*
C* M. Linda/GSC		12/96	Removed /MARKBF/, passing reals on down	*
...
C
     ...
     +			  gx, gy, ier )
C
C*	    Send markers to device driver.
C
	    CALL DMARK ( iwndw, num, gx, gy, iret )
C
	    ib = ie + 1
	END DO
C*
