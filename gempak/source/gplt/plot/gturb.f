	SUBROUTINE GTURB ( sys, np, tucod, x, y, ixoff, iyoff, iret )
C************************************************************************
C* GTURB								*
C*									*
C* This subroutine plots turbulence symbols in any coordinate system.	*
C* The symbols are drawn using attributes defined by GSTURB.		*
C*									*
C* GTURB ( SYS, NP, TUCOD, X, Y, IXOFF, IYOFF, IRET )			*
C*									*
C* Input parameters:							*
...
C* M. Linda/GSC		12/96	Removed /TURBBF/, passing reals on down	*
...
C
	    CALL DTURB ( iwndw, num, tucod (ib), gx, gy, ixoff (ib),
     +			 iyoff (ib), iret )
	    ib = ie + 1
	END DO
C*
