	SUBROUTINE GARC  ( sys, xpt, ypt, xcrm, ycrm, np, ang1, ang2,
     +                     endpts, iret )
C************************************************************************
C* GARC									*
...
C*
	CHARACTER	sysuc*1, sysout*1
     ...
     +			xsave(2), ysave(2), xnew(2), ynew(2)
        REAL            rang1, rang2, xarry(2), yarry(2)
C------------------------------------------------------------------------
...
C
		...
		CALL GTRANS  ( 'L', sysout, 2, xlin, ylin, xd, yd, ier ) 
                IF  ( ( sysuc .eq. 'M' ) .and.
     +		      ( ddev .ne. 'VG' ) ) THEN
	            CALL GTRANS  ( sysout, 'M', 2, xd, yd, xnew, ynew, 
     +                             ier )
	            CALL GP_AZDR ( ang1, xnew(1), ynew(1), rang1, ier )
	            CALL GP_AZDR ( ang2, xnew(1), ynew(1), rang2, ier )
                    rang1 = 90. - rang1
                    rang2 = 90. - rang2
                  ELSE
                    rang1 = ang1
                    rang2 = ang2
        	END IF
                CALL DARC  ( iwndw, xd(1), yd(1), xd(2), yd(2), np, 
     +			     ang1, ang2, endpts, ier )
C
		xlin (1) = xsave (1)
		xlin (2) = xsave (2)
		ylin (1) = ysave (1)
...
C
        IF  ( ( sysuc .eq. 'M' ) .and.
     +		      ( ddev .ne. 'VG' ) ) THEN
	    CALL GTRANS  ( sysout, 'M', 2, xd, yd, xnew, ynew, ier )
	    CALL GP_AZDR ( ang1, xnew(1), ynew(1), rang1, ier )
	    CALL GP_AZDR ( ang2, xnew(1), ynew(1), rang2, ier )
            rang1 = 90. - rang1
            rang2 = 90. - rang2
...
C
        ...
        yarry (2) = endpts(4)
C
        CALL GTRANS ( 'D', sysuc, 2, xarry, yarry, xarry, yarry, ier)
C
        endpts(1) = xarry(1)
        endpts(2) = yarry(1)
        endpts(3) = xarry(2)
