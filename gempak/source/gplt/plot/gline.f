...
C* GLINE								*
C*									*
C* This subroutine draws lines in any coordinate system.  Each line	*
C* is a series of straight segments connecting the given array of	*
C* points.  The lines are drawn using attributes defined by GSLINE.	*
C*									*
...
C* M. desJardins/GSFC	 2/88	Fixed buffer problems			*
C* M. desJardins/GSFC	 6/88	Documentation				*
C* K. Brill/NMC		11/91	Documentation and indentation		*
C* M. Linda/GSC		 6/96	Bypass buffer /LINEBF/			*
...
C* S. Jacobs/NCEP	 5/01	Do not check for breakpoint if sat proj	*
C* S. Jacobs/NCEP	 6/01	Added check for missing pts before DLINE*
C* S. Jacobs/NCEP	 6/01	Removed check against veps		*
C* D.W.Plummer/NCEP	 1/04	Changes for cylindrical processing	*
C* D.W.Plummer/NCEP	 7/04	Add check for missing before DLINE call	*
...
C
     ...
     +			      gx, gy, iret )
		IF ( iret .ne. 0 ) RETURN
		CALL DLINE ( iwndw, ip, gx, gy, iret )
	      ELSE
...
C
     ...
     +			      ( .not. ERMISS ( gy ( i ) ) ) ) THEN
			    ...
			    gy ( im ) = gy ( i )
			  ELSE
			    CALL DLINE ( iwndw, im, gx, gy, iret )
			    IF ( iret .ne. 0 ) RETURN
			    im = 0
			END IF
		    END DO
...
C
     ...
     &                        ( .not. ERMISS ( fy ( ii ) ) ) )  THEN
			  ...
			  fy ( im ) = fy ( ii ) 
			ELSE
			  CALL DLINE ( iwndw, im, fx, fy, iret)
			  IF ( iret .ne. 0 ) RETURN
			  im = 0
     		        END IF
		      END DO
