	SUBROUTINE DSPCL ( iwndw, np, spcod, x, y, ixoff, iyoff, iret )
C************************************************************************
C* DSPCL								*
C*									*
C* This subroutine draws special symbols on the current graphics	*
C* device.								*
C*									*
C* DSPCL ( IWNDW, NP, SPCOD, X, Y, IXOFF, IYOFF, IRET )			*
C*									*
C* Input parameters:							*
...
C
	IF  ( ddev .eq. 'VG' )  THEN
	    CALL HSYMB ( 7, np, spcod, x, y, ixoff, iyoff, iret )
	    RETURN
	END IF
C
C*	Save line type and line width.
C
	IF  ( ( mltyp .ne. 1 ) .or. ( mlwid .ne. mspwid ) ) THEN
	    jltyp = mltyp
	    jlwid = mlwid
	    CALL DSLINE ( 1, 0, mspwid, 0, i1, i2, i3, i4, ier )
	  ELSE
	    jltyp = 0
	    jlwid = 0
	END IF
...
C
		...
		ix    = NINT (     x (i) )
		iy    = NINT (     y (i) )
		CALL ISPCL ( icode, ix, iy, ixoff(i), iyoff(i),
     +			     wsize, ier )
	    END DO
	END IF
C
