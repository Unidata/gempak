...
void	frt2tag ( VG_DBStruct *el, char *buffer, int *iret )
{
    ...
    sprintf( buffer, "%s          <Line pgenCategory=\"%s\" lineWidth=\"%d\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" pgenType=\"%s\">\n", 
...
fill_pattern(el), pgenType );
	...
	if (el->elem.frt.info.fpipdr >=0) {
	    for ( ii = 0; ii < npts; ii++ )  {
	    	blen = strlen(buffer);
	    	sprintf( &(buffer[blen]), "%s            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", indent, el->elem.frt.latlon[ii], el->elem.frt.latlon[ii+npts] );
	    }
	}
	else {
...
void	lin2tag ( VG_DBStruct *el, char *buffer, int *iret )
{
    ...
    sprintf( buffer, "%s          <Line pgenCategory=\"%s\" lineWidth=\"%d\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" pgenType=\"%s\">\n", 
    indent, category(el), el->elem.lin.info.width, 1.0, el->hdr.smooth, el->hdr.closed==0 ?"false":"true",
    el->hdr.filled==0 ?"false":"true", fill_pattern(el), pgenType 
	);
	...
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "%s            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", indent, el->elem.lin.latlon[ii], el->elem.lin.latlon[ii+npts] );
	}
...
char *indent = "";
    ...
    sprintf( buffer, "%s          <Line pgenCategory=\"%s\" lineWidth=\"%d\" sizeScale=\"%4.1f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" pgenType=\"%s\">\n", 
indent, category(el), el->elem.spl.info.splwid, el->elem.spl.info.splsiz,
el->hdr.smooth, el->hdr.closed==0 ?"false":"true", el->hdr.filled==0 ?"false":"true",
fill_pattern(el), pgenType );
	...
	if (el->elem.spl.info.spldir >=0) {
	    for ( ii = 0; ii < npts; ii++ )  {
	    	blen = strlen(buffer);
	    	sprintf( &(buffer[blen]), "%s            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", indent, el->elem.spl.latlon[ii], el->elem.spl.latlon[ii+npts] );
	    }
	}
	else {
	    for ( ii = npts-1; ii >=0; ii-- )  {
...
el->elem.wbx.info.w_mrkwid, mktype, el->hdr.filled==0 ?"false":"true", shape );
    ...
    if (npts > 0) {
    	for ( ii = 0; ii < npts; ii++ )  {
            blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "                <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", el->elem.wbx.latlon[ii], el->elem.wbx.latlon[ii+npts]);
	}
    }
    else {
	blen = strlen(buffer);
...
/*---------------------------------------------------------------------*/
    ...
    *iret = 0;
    ...
    sprintf( buffer, "%s          <Symbol pgenCategory=\"%s\"" " pgenType=\"%s\"" " lineWidth=\"%d\"" " sizeScale=\"%f\"" " clear=\"%s\">\n",
...
el->elem.sym.info.size, (el->elem.sym.info.width)>=100 ?"true":"false");
	...
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "%s            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", indent, el->elem.sym.data.latlon[ii], el->elem.sym.data.latlon[ii+npts] );
	}
    }
    else {
	blen = strlen(buffer);
...
category(el), pgenType, el->elem.wnd.info.width, el->elem.wnd.info.size, (el->elem.wnd.data.spddir[1]), (el->elem.wnd.data.spddir[0]), el->elem.wnd.info.hdsiz, (pgenType=="Directional" ?"true":"false"), el->elem.wnd.info.wndtyp==114 ?"true":"false" ); //112, 1: false
	...
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", el->elem.wnd.data.latlon[ii], el->elem.wnd.data.latlon[ii+npts] );
	}
    }
    else {
	blen = strlen(buffer);
...
char * symbolPatternName = "Not Applicable";
char * avnTextType = "Not Applicable";
char * mask = "false";
char * outline = "false";
char * font;
char * style;
...
getIalign(el->elem.spt.info.ialign), cloudAmn, cloudType, "","","","", tstormLevel, tstormType);
	...
	else if (el->hdr.grptyp ==2) { //change general text to avn text for turb
...
indent, category(el), pgenType, symbolPatternName, getSztext(el->elem.spt.info.sztext), //symbolPatternName is default
font, style, getIalign(el->elem.spt.info.ialign), result2, result, "LOW_LEVEL_TURBULENCE"); //avnTextType is default
	}

	else {
	    /* (el->elem.spt.info.rotn -1000) is to keep drawing the same */
	    sprintf( buffer, "%s          <Text pgenCategory=\"%s\"" " pgenType=\"%s\"" "  rotation=\"%f\"" " fontSize=\"%s\"" " fontName=\"%s\"" " style=\"%s\"" " justification=\"%s\"" " rotationRelativity=\"%s\""  " mask=\"%s\""  " outline=\"%s\""  " yOffset=\"%d\""  " xOffset=\"%d\">\n",
...
el->elem.spt.info.rotn>=1000 ?"NORTH_RELATIVE" :"SCREEN_RELATIVE", mask, outline, 
...
// points, text and ending tag
    ...
    blen = strlen(buffer);
    sprintf( &(buffer[blen]), "%s            <Point Lat=\"%10.6f\"" " Lon=\"%11.6f\"/>\n", indent, el->elem.spt.info.lat, el->elem.spt.info.lon );
...
el->hdr.smooth, el->hdr.closed==0 ?"false":"true", el->hdr.filled==0 ?"false":"true", fill_pattern(el)
	...
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "%s            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", indent, el->elem.cir.data.latlon[ii], el->elem.cir.data.latlon[ii+npts] );
	}
    }
    else {
	blen = strlen(buffer);
...
//extra points
	...
	for ( ii = 0; ii < showSize; ii++ )  {
	    ...
	    sprintf( &(buffer[blen]), "              <location latitude=\"%10.6f\"", el->elem.trk.latlon[ii+ipts] );
            blen = strlen(buffer);
            sprintf( &(buffer[blen]), " longitude=\"%11.6f\"/>\n            </extrapPoints>\n", el->elem.trk.latlon[ii+ipts+npts] );
	}
...
/*---------------------------------------------------------------------*/
    ...
    *iret = 0;
    ...
    if (el->hdr.vg_type == 27) { //sigInt
    	sprintf( buffer, "          <Sigmet pgenCategory=\"%s\" pgenType=\"%s\" width=\"%f\" type=\"%s\" lineWidth=\"%d\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" editableAttrFromLine=\"%s\" editableAttrLevelText2=\"%s\" editableAttrLevelText1=\"%s\" editableAttrLevelInfo2=\"%s\" editableAttrLevelInfo1=\"%s\" editableAttrLevel=\"%s\" editableAttrPhenomDirection=\"%s\" editableAttrPhenomSpeed=\"%d\" editableAttrTrend=\"%s\" editableAttrPhenom=\"%s\" editableAttrPhenom2=\"%s\" editableAttrPhennam=\"%s\" editableAttrPhenomLat=\"%s\" editableAttrPhenomLon=\"%s\" editableAttrPhenomPressure=\"%d\" editableAttrPhenomMaxWind=\"%d\" editableAttrMovement=\"%s\" editableAttrRemarks=\"%s\" editableAttrEndTime=\"%s\" editableAttrStartTime=\"%s\" editableAttrSeqNum=\"%d\" editableAttrId=\"%s\" editableAttrStatus=\"%d\" editableAttrArea=\"%s\">\n",
category(el), pgenType, el->elem.sig.info.distance*1852, subType,
...
el->elem.sig.info.status, trimwhitespace(el->elem.sig.info.area) ); 
    	...
    	sprintf( buffer, "          <Sigmet pgenCategory=\"%s\" pgenType=\"%s\" width=\"%f\" type=\"%s\" lineWidth=\"%d\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" editableAttrFromLine=\"%s\"  editableAttrSeqNum=\"%d\" editableAttrId=\"%s\" editableAttrArea=\"%s\">\n",
category(el), pgenType, el->elem.sig.info.distance*1852, subType,
el->elem.sig.info.linwid,  2.0, el->hdr.smooth,
el->hdr.closed==0 ?"false":"true", el->hdr.filled==0 ?"false":"true", fill_pattern(el), " ",  
...
trimwhitespace(el->elem.sig.info.area) );
	...
	el->elem.sig.latlon[ii], el->elem.sig.latlon[ii+npts] );
	}
    }
    else {
	blen = strlen(buffer);
...
int	ii, npts;
size_t	blen;
char    *filled, *cover, *tops, *prob, *growth, *stype, *dir;
...
/*---------------------------------------------------------------------*/
	...
	filled = "FILL_PATTERN_1";	
    else if (el->hdr.filled == 4)
	filled = "FILL_PATTERN_3";
    else if (el->hdr.filled == 6)
	filled = "FILL_PATTERN_5";
    else 
	filled = "SOLID";
...
/*printf("Text %s\n", el->elem.ccf.spt.text);//null
printf("TextLayout %s\n", el->elem.ccf.textLayout); //IBDR|221;TEXT|TOPS::ETR;TEXT|GWTH::GWTH;TEXT|CONF::CONF;TEXT|CVRG::CVG*/


    sprintf( buffer, "          <Sigmet pgenCategory=\"%s\" pgenType=\"%s\" width=\"%f\" type=\"%s\" lineWidth=\"%d\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" editableAttrFromLine=\"%s\" editableAttrPhenomDirection=\"%s\" editableAttrPhenomSpeed=\"%d\" editableAttrFreeText=\"%s\" editableAttrPhenomLon=\"%s\" editableAttrPhenomLat=\"%s\" editableAttrPhenom2=\"%s\" editableAttrPhenom=\"%s\" editableAttrEndTime=\"%s\" editableAttrStartTime=\"%s\" editableAttrLevelText2=\"%f\" editableAttrLevelText1=\"%f\" editableAttrLevelInfo2=\"%f\" editableAttrLevelInfo1=\"%f\">\n",
...
/*---------------------------------------------------------------------*/
    ...
    sprintf( buffer, "          <Sigmet editableAttrFreeText=\"%s\" pgenCategory=\"%s\" pgenType=\"%s\" width=\"%f\" type=\"%s\" lineWidth=\"%d\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\">\n",
freeText, category(el), "VACL_SIGMET", el->elem.ash.info.distance*1852, subtype,
...
el->hdr.closed==0 ?"false":"true", el->hdr.filled==0 ?"false":"true", "FILL_PATTERN_2" ); 
    ...
    if (npts > 0) {
	    ...
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "            <Point Lat=\"%10.6f\" Lon=\"%11.6f\"/>\n",
	el->elem.ash.latlon[ii], el->elem.ash.latlon[ii+npts] );
    	}
    }
    else {
	blen = strlen(buffer);
...
el->elem.vol.info.number, el->elem.vol.info.name, "false" 
    ...
    for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", el->elem.vol.latlon[ii], el->elem.vol.latlon[ii+npts] );
...
int	ii, npts;
size_t	blen;
char *mask, *outline, *result, *pgenType;
...
//sub2 -- barb
	...
	blen = strlen(buffer);
    	sprintf( &(buffer[blen]), "                  <Text pgenCategory=\"Text\" pgenType=\"General Text\"  rotation=\"%10.6f\" fontSize=\"%s\" fontName=\"%s\" style=\"%s\" justification=\"%s\" rotationRelativity=\"%s\" mask=\"%s\" outline=\"%s\" yOffset=\"%d\" xOffset=\"%d\" hide=\"%s\" auto=\"%s\">\n",
...
mask, outline, el->elem.jet.barb[ii].spt.info.offset_y, el->elem.jet.barb[ii].spt.info.offset_x, "", "");
...
el->elem.jet.barb[ii].wnd.info.width >=800 ?"true" :"false"
    	...
    	blen = strlen(buffer);
    	sprintf( &(buffer[blen]), "                    <Point Lat=\"%10.6f\" Lon=\"%11.6f\"/>\n", el->elem.jet.barb[ii].wnd.data.latlon[0], el->elem.jet.barb[ii].wnd.data.latlon[1] );
...
//sub1
    ...
    sprintf( &(buffer[blen]), "              <Line pgenCategory=\"Lines\"" " lineWidth=\"%d\"" " sizeScale=\"%f\"" " smoothFactor=\"%d\"" " closed=\"%s\"" " filled=\"%s\"" " fillPattern=\"%s\"" " pgenType=\"%s\">\n",
...
fill_pattern(el), "FILLED_ARROW" 
	...
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "                <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", el->elem.jet.line.spl.latlon[ii], el->elem.jet.line.spl.latlon[ii+npts] );
	}
    }
    blen = strlen(buffer);
...
"Hash", el->elem.jet.hash[ii].wnd.info.width, el->elem.jet.hash[ii].wnd.info.size, (180 - el->elem.jet.hash[ii].wnd.data.spddir[1]), el->elem.jet.hash[ii].wnd.data.spddir[0], el->elem.jet.hash[ii].wnd.info.hdsiz, ((pgenType=="Directional" || pgenType=="Hash") ?"true":"false"), el->elem.jet.hash[ii].wnd.info.wndtyp ==114 ?"true":"false"
    	...
    	blen = strlen(buffer);
    	sprintf( &(buffer[blen]), "                <Point Lat=\"%10.6f\" Lon=\"%11.6f\"/>\n", el->elem.jet.hash[ii].wnd.data.latlon[0], el->elem.jet.hash[ii].wnd.data.latlon[1] );
       	blen = strlen(buffer);
    	sprintf( &(buffer[blen]), "              </Vector>\n");
    }
...
*(tag +1) ='\0';
    	...
    	sprintf( buffer, "          <Gfa pgenCategory=\"%s\" pgenType=\"%s\" lineWidth=\"%s\" sizeScale=\"%f\" smoothFactor=\"%d\" closed=\"%s\" filled=\"%s\" fillPattern=\"%s\" latText=\"%s\"  lonText=\"%s\" hazard=\"%s\" fcstHr=\"%s\" tag=\"%s\"  desk=\"%s\" issueType=\"%s\" cycleDay=\"%s\"  cycleHour=\"%s\" area=\"%s\" rg=\"%s\"  beginning=\"%s\" ending=\"%s\" states=\"%s\" type=\"%s\" top=\"%s\" bottom=\"%s\" coverage=\"%s\"  fzlTopBottom=\"%s\" level=\"%s\"  fzlRange=\"%s\" contour=\"%s\" severity=\"%s\"  dueto=\"%s\" speed=\"%s\" intensity=\"%s\" category=\"%s\"  frequency=\"%s\" isOutlook=\"%s\" textVor=\"%s\">\n",
"MET", "GFA", lineWidth, 1.0, el->hdr.smooth, el->hdr.closed==0 ?"false":"true", 
el->hdr.filled==0 ?"false":"true", fill_pattern(el), 
lat, lon, areaType, fcstHr, tag, (tagDesk+1), status, "", cycle, areas, rg, condsBegin, condsEnd, states,
type, top, bottom, coverage, fzlTop, level, fzlRange, contour, severity, dueto, speed, intensity, category, frequency, "", "" );
	...
	for ( ii = 0; ii < npts; ii++ )  {
	    blen = strlen(buffer);
	    sprintf( &(buffer[blen]), "            <Point Lat=\"%10.6f\" Lon=\"%10.6f\"/>\n", el->elem.gfa.latlon[ii], el->elem.gfa.latlon[ii+npts] );
	}
    }
    else {
	blen = strlen(buffer);
