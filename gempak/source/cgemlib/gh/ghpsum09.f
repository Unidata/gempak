	SUBROUTINE GH_PSUM09 ( advise, lenw, slat, slon,
     +                       wdir, wsped, wpres, wwnd, gust, iret )
C************************************************************************
C* GH_PSUM09 								*
C*									*
C* This subroutine decodes the SUMMARY Section of a single public     *
C* advisory report- 2009 format.      						*
C*                                                                      *
C* GH_PSUM09 ( ADVISE, LENW,  SLAT, SLON, WDIR, WSPED,                    *
C*           WPRES, WWND, IRET )                                        *
C*									*
C* Input parameters:							*
...
C* X .Guo/CWS		02/10           copy from GH_PSUM		*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
	INCLUDE		'BRIDGE.PRM'
C*
...
C
	ipos = INDEX( advise(irpt:lenw), 'MAXIMUM SUSTAINED WIND') 
        irpt = irpt + ipos + 21	
	DO i = 1, 10
	    CALL ST_ALNM( advise(irpt:irpt), ityp, iret )
	    IF ( ityp .eq. 1 ) exit
