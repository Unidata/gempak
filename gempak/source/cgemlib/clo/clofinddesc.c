...
void clo_finddesc ( char *locnam, char *xdesc, char *xstate, int srchtyp, 
 ...
 *									*
 * clo_finddesc (locnam, xdesc, xstate, srchtyp, maxlen, 		*
 * 		 nret, info, iret)					*
 *									*
 * Input parameters:							*
 *      *locnam         char	Data location name			*
 *	*xdesc		char	Station name or substring		*
...
char	tstr[128];
char	qdesc[64], qstate[16];

int	done, chkst, eqdesc;
...
/*---------------------------------------------------------------------*/
    ...
    *nret = 0;
    ...
    clo_sortstn ( locnam, STN_DESC, &ier );
     ...
     */
    cst_lcuc( xdesc, qdesc, iret );
    for ( i = 0 ; i < (int)strlen(qdesc) ; i++ )
        if ( qdesc[i] == ' ' )  qdesc[i] = '_';
    cst_lcuc( xstate, qstate, iret );

    chkst = strlen( qstate ) != (size_t)0;

    if ( qdesc[0] == stn->station[0].desc[0] || srchtyp == INDEX )  {
	stptr = 0;
    }
    else  {
	...
	iptr = stptr;
	    ...
	    if ( stn->station[iptr].desc[0] == qdesc[0] )  {
	        ipt1 = stptr - 1;
		while ( stn->station[iptr].desc[0] == 
			stn->station[ipt1].desc[0] )  {
		    stptr--;
		    iptr = stptr;
		    ipt1 = stptr - 1;
		}
	        ...
	        if ( qdesc[0] < stn->station[iptr].desc[0] )  ihi = stptr-1;
	        if ( qdesc[0] > stn->station[iptr].desc[0] )  ilo = stptr+1;
    	        stptr = (ihi+ilo) / 2;
		iptr = stptr;
	    }
        }
	...
	switch ( srchtyp )  {
		...
		eqdesc = strncmp( stn->station[iptr].desc, 
			 qdesc, strlen(qdesc) );
		...
		if ( strstr( stn->station[iptr].desc, qdesc ) != 
			(char *)NULL )  {
		        eqdesc = 0;
		}
		else  {
		...
		eqdesc = strcmp( stn->station[iptr].desc, qdesc );
	...
	if ( ( eqdesc == 0 && !chkst )  ||
	    ...
	    len = (int)strlen(info) + (int)strlen(tstr) + 2;
	    if ( len < maxlen )  {
		    /*
	...
	switch ( srchtyp )  {
		...
		if ( stptr < stn->nstn-1 )  {
		    if ( stn->station[iptr].desc[0] != 
			stn->station[stptr-1].desc[0] ) done = 1;
		}
