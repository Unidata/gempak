	SUBROUTINE G2T_DWDT ( lunb, nt, iret )
C************************************************************************
C* G2T_DWDT								*
C*									*
C* This subroutine creates OFF text wave trending.			*
C*									*
C* G2T_DWDT ( LUNB, NT, IRET )						*
C*									*
C* Input parameters:							*
...
C* T. Lee/SAIC		06/07	Updated for new trending matrix		*
C* T. Lee/SAIC		11/07	Added NT for combining periods		*
C* T. Lee/SAIC		03/08	Removed case numbers			*
C************************************************************************
	INCLUDE		'goftxt.cmn'
	PARAMETER	( ktype = 1 )
	CHARACTER	end*20, dum(3)*2
	CHARACTER	wid(3)*(MAXIDS)
...
C
	...
	nt2 = nt + 2
C
C*	Get data. X(3): stores max wave from nth to (n+2)th time step.
C*		  N(3): stores min wave 
C*		  E(3): stores max wave in the EXCEPT portion.
C
	CALL G2T_GET (  ktype, nt, dum, dum, dum, dum, X, N, E, 
     +			wid, iret )
C
	Rcase_8 = .false.
	IF  ( X ( 2 ) .ge. 5 )  THEN
...
C
	    IF ( nt .le. 7 )  THEN
C
C*		All equals -> Ra -> Rb early -> Rc late...Ea through 
C*		period.
C
...
C
		  ...
		  ELSE IF ( WID ( 1 ) .ne. WID ( 2 ) )  THEN
C
C*		  Period A BECOMING B. Location changes only.
C
		    CALL G2T_A2RBEB ( ktype, lunb, nt, end, ier )	
		  ELSE IF ( WID ( 1 ) .ne. WID ( 3 ) )  THEN
C
