	SUBROUTINE G2T_A2RAEC ( ktype, lunb, nt, end, iret )
C************************************************************************
C* G2T_A2RAEC								*
C*									*
C* This subroutine creates G2T trending text, period A becoming RaEc.	*
C*									*
C* G2T_A2RAEC ( KTYPE, LUNB, NT, END, IRET )				*
C*									*
C* Input parameters:							*
...
C* T. Lee/SAIC		11/07	Added lesser values in EXCEPT		*
...
C
	...
	nt2 = nt + 2
C
C*	Store RcEc in trending block 1, RaEa in 2.
C
	itrnd = 2
...
C
C*	Replace Ea with Ec so block 2 becomes RaEc. Reset the EXCEPTION
C*	and PORTION flags.
C
	...
	END IF
C
C*	Store RaEa in trending block 1 and simplify/trim the wave text.
C
	itrnd = 1
	CALL G2T_ABC ( ktype, nt, itrnd,ier )
