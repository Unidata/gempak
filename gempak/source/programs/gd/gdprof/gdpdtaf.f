	SUBROUTINE GDPDTAF  ( iflno, gdatim, gvcord, gfunc, gpoint, 
...
C*									*
C* GDPDTAF  ( IFLNO, GDATIM, GVCORD, GFUNC, GPOINT, TIME, IVCORD,	*
C*           YSTRT, YSTOP, GRID, RGX, RGY, RLAT, RLON, NPTS, X, Y,  	*
C*	     PARM, IRET )						*
C*									*
C* Input parameters:							*
...
C* S. Gilbert/NCEP	 8/07	From GDPDTA; added work array args      *
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C*	to the desired grid point first within DG_GRIDN before the quantity is
...
C*	vector components
C
	CALL GR_MNAV ( 'CED', 3, 3, rlat - .01, rlon - .01,
...
C
	    CALL GD_GNAV  ( iflno, savblk, isavsz, iret )
