	SUBROUTINE GDPTTM ( igdfln, gdatim, npts, timfnd, trange,
     +			    iret )
C************************************************************************
C* GDPTTM								*
C*									*
C* This subroutine sets time information for a specified GDATTIM.	*
C*									*
C* GDPTTM ( IGDFLN, GDATIM, NPTS, TIMFND, TRANGE, IRET )		*
C*									*
C* Input parameters:							*
...
C* T.W.Barker/WR/MSO     2/97   Created from GDTXTM                     *
C* S. Chiswell/Unidata	 5/05	Changed NTIME to LLMXTM			*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
	...
	LOGICAL		respnd
C*
	CHARACTER*36	time, timlst (LLMXTM), tstrt, tstop, ttt, timout
C------------------------------------------------------------------------
...
C
	CALL GD_GTIM ( igdfln, LLMXTM, timlst, ntimin, ier )
	IF ( ntimin .eq. 0 ) THEN
	    iret = -13
	    CALL ER_WMSG ( 'GDHAIL', iret, ' ', ier )
	    RETURN
	END IF
C
