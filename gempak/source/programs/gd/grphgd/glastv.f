	SUBROUTINE GLASTV ( i, j, n, grid, hist, kx, ky,
     +		    	    fi, fj, val, iret )
C************************************************************************
C* GLASTV                                                               *
C*                                                                      *
C* This subroutine returns the last available value in a given		*
C* direction.								*
C*                                                                      *
...
C
	ii = i + kinc(n,1)
	jj = j + kinc(n,2)
	DO WHILE ( ii .gt. 0 .and. ii .lt. kx+1 .and.
     +		   jj .gt. 0 .and. jj .lt. ky+1 ) 
C
	    IF ( hist(ii,jj) .eq. 0 .or. hist(ii,jj) .eq. 1 )  THEN
C
		RETURN
C
	    ELSE
