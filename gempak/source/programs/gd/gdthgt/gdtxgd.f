	SUBROUTINE GDTXGD ( gridin, nx, nv, ivcord, iyaxis, rlvls,
...
C* type.								*
C*									*
C* GDTXGD ( GRIDIN, NX, NV, IVCORD, IYAXIS, RLVLS, YBOT, YTOP, LVLFLG,	*
...
C*      NVOUT		      INTEGER   Number of vert lvls post	*
C*					interp				*
...
C* S. Gilbert/NCEP	08/07		Removed reference to LLMXGD	*
C************************************************************************
	INCLUDE 	'GEMPRM.PRM'
C*
...
C
	    ...
	    nvout = INT ( rnolv + .5 ) + 1
	    nvchk = nvout * nx
	    maxsize = LLMXLV * LLMXGT
	    IF ( nvchk .gt. maxsize ) nvout = maxsize / nx
