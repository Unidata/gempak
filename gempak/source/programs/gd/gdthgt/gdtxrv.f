	SUBROUTINE GDTXRV ( time, gvcord, gvecx, levt, levb, lavflg,
     +			    rgx, rgy, inttyp, lvert, uu, vv, iret )
C************************************************************************
C* GDTXRV								*
C*									*
C* This subroutine actually gets the wind data at a particular level	*
C*									*
C* GDTXRV ( TIME, GVCORD, GVECX, LEVT, LEVB, LAVFLG, RGX, RGY, INTTYP,	*
C*	    LVERT, UU, VV, IRET)					*
C*									*
C* Input parameters:							*
...
C
	...
	END IF
	if ( .not. lvert ) then
	    CALL DG_VECT ( time, glevel, gvcord, gvecx, pfunc, gridu,
     +			   gridv, kx, ky, dattim, lev, ivc, parmu, 
     +			   parmv, iret )
	ELSE
	    CALL DG_VECR ( time, glevel, gvcord, gvecx, pfunc, gridu,
     +			   gridv, kx, ky, dattim, lev, ivc, parmu,
     +			   parmv, iret )
	END IF
	IF ( iret .eq. 0 ) THEN
