	SUBROUTINE GDTXRI ( tbegin, tend, tinc, itftyp, ntime, times, 
     +			      iret )
C************************************************************************
C* GDTXRI 								*
C*									*
...
C* increment.								*
C*									*
C* GDTXRI ( TBEGIN, TEND, TINC, ITFTYP, NTIME, TIMES, IRET )		*
C*									*
C* Input parameters:							*
...
C
	    ...
	    DO WHILE ( ( idtbeg (1) .lt. idtend (1) ) .or.
     +			( ( idtbeg (1) .eq. idtend (1) ) .and.
...
C
	...
	END IF
C*
C*	if combo...check the forecast times
C*
	IF ( itftyp .eq. 3 ) THEN
	    jhour  = idtbeg (3) / 100
	    jminut = idtbeg (3) - jhour * 100
