...
void png_header(memheap,length)
char *memheap;
int length;
{
...
void unidata_output_flush_fn(png_p)
png_structp png_p;
{
...
void unidata_write_data_fn(png_p,data,length)
png_structp png_p;
png_bytep data;
png_uint_32 length;
{
png_header(data,length);
}

void png_set_memheap(memheap)
char *memheap;
{
prodmap = memheap;
prodoff = 0;
