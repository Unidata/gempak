...
C
        ELSE IF ( pname .eq. 'BOXLIN' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., bli, nbli, iret )
C
        ELSE IF ( pname .eq. 'REGION' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., reg, nreg, iret )
C
        ELSE IF ( pname .eq. 'TXTCOL' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., tco, ntco, iret )
C
        ELSE IF ( pname .eq. 'TXTYPE' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., tty, ntty, iret )
C
        ELSE IF ( pname .eq. 'TXTFIL' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., tfl, ntfl, iret )
C
        ELSE IF ( pname .eq. 'TXTLOC' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., tlo, ntlo, iret )
C
        ELSE IF ( pname .eq. 'COLUMN' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., cln, ncln, iret )
C
        ELSE IF ( pname .eq. 'SHAPE' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., sha, nsha, iret )
C
        ELSE IF ( pname .eq. 'INFO' )  THEN
            CALL GDINST ( s, '<', MAXB,  .false., inf, ninf, iret )
C
        ELSE IF ( pname .eq. 'LOCI' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., loc, nloc, iret )
C
        ELSE IF ( pname .eq. 'ANOTLN' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., ali, nali, iret )
C
        ELSE IF ( pname .eq. 'ANOTYP' )  THEN
            CALL GDINST ( s, '<', MAXB,  .true., aty, naty, iret )
C
	ELSE 
C
	    iret = -1
