...
C* F. J.Yen/NCEP	 7/08	Reverse axis for HYBL (ivcord = 6) 	*
C************************************************************************
...
C
	IF  ( ( ivcord .eq. 1 .or. ivcord .eq. 4 .or. ivcord .eq. 6 )
     +		.and. ( ystrt .lt. ystop ) )  THEN
	    ysave = ystrt
	    ystrt = ystop
	    ystop = ysave
	  ELSE IF ( ( ivcord .ne. 1 .and. ivcord .ne. 4 .and.
     +		      ivcord .ne. 6 ) .and. ( ystrt .gt. ystop ) ) THEN
	    ysave = ystrt
	    ystrt = ystop
	    ystop = ysave
	END IF
