    	SUBROUTINE GDXDVV  ( iflno, gdatim, gvcord, ystrt,
     +                       ystop, gvecx, time, ivcord, 
     +			     rgx, rgy, nhxs, rlvl, ugrd, vgrd, ponth,
     +			     nvxs, parmu, parmv, lvert, lscal, iret )
C************************************************************************
C* GDXDVV								*
C*									*
C* This subroutine gets the wind data to display in a cross section.	*
C*									*
C* GDXDVV  ( IFLNO, GDATIM, GVCORD, YSTRT, YSTOP, GVECX, TIME, IVCORD,	*
C*           RGX, RGY, NHXS, RLVL, UGRD, VGRD, PONTH, NVXS,		*
C*	     PARMU, PARMV, LVERT, LSCAL, IRET )				*
C*									*
C* Input parameters:							*
...
C
		...
		ELSE
		  ...
		  CALL ST_INCH  ( intlvl, glevel, ier )
	        END IF
	        IF ( .not. lvert ) THEN
		  CALL DG_VECT ( gdatim, glevel, gvcord, gvect, 
     +				pfunc,  gridu, gridv, kx, ky, dattim,
     +			        lev, jvcord, parmu, parmv, ier )
		ELSE
		  CALL DG_VECR ( gdatim, glevel, gvcord, gvect, 
     +				pfunc,  gridu, gridv, kx, ky, dattim,
     +			        lev, jvcord, parmu, parmv, ier )
	        END IF
		IF  ( ier .eq. 0 )  THEN
