 ...
 * E. Saffors/SAIC	11/07	use wgem_gqmprj()			*
 ***********************************************************************/
{
char	map[25];
char	proj[25];
 ...
 */
	...
	if ( gflag ) {
	    ...
	    wgem_gqmprj( prjtmp, &angle[0], &angle[1], &angle[2],
				&mapb->x[0], &mapb->y[0], 
				&mapb->x[1], &mapb->y[1],
				iret );
/* 
 ...
 * E. Safford/SAIC	11/07	use wgem_* wrapper functions		*
 ***********************************************************************/
{
 ...
 */
	wgem_gsline(&ltype, &ltyhw, &lwidth, &ltyhw, &iret);
 ...
 * set line color
 */	
	      wgem_gscolr(&(contours->line[j].color), &iret);

/*
 * draw the line
 */
	      wgem_gline(sys_M, &(contours->line[j].npt), 
			 contours->line[j].lat, 
			 contours->line[j].lon, &iret );

/*
 * plot contour label
 */
	      wgem_gtextc(sys_M, &(contours->line[j].lat[0]), 
 ...
 ***********************************************************************/
{
	...
	char		endtim[21], endtm2[21], expr[21]="\0", exptim[21], 
			found, vald[21]="\0", valtim[21], tmzn[4];
 ...
 * set the line type, width... attributes
 */
	wgem_gqline ( &jtype, &jtyhw, &jwidth, &jwdhw, &iret );
	wgem_gsline ( &ltype, &ltyhw, &lwidth, &lwdhw, &iret );

	wgem_gqtext ( &jtxfn, &jtxhw, &siztx, &jtxwid,
		 &jbrdr, &jrrotn, &jjust, &iret );
	wgem_gstext ( &ltxfn, &ltxhw, &sztext, &ltxwid,
 ...
 * Set line color.
 */	
		wgem_gscolr ( &(wtchbox->winfo[j].color), &iret );

/*
 * Draw the box.
 */
		wgem_gline ( sys_M, &(wtchbox->winfo[j].npt), 
			     wtchbox->winfo[j].lat, 
			     wtchbox->winfo[j].lon, &iret );

/*
 * Find the lower left corner for plotting the label.
 */
		wgem_gg_wlbl ( &(wtchbox->winfo[j].npt),
                          wtchbox->winfo[j].lat,
                          wtchbox->winfo[j].lon,
			  &xlat, &ylon, &iret );
 ...
 */
		...
		wgem_gtext ( sys_M, &xlat, &ylon, wlabel, &rotat,
			 &ixoff, &iyoff, &iret );
 ...
 */
	wgem_gsline ( &jtype, &jtyhw, &jwidth, &jwdhw, &iret );

	wgem_gstext ( &jtxfn, &jtxhw, &siztx, &jtxwid,
		 &jbrdr, &jrrotn, &jjust, &iret );
