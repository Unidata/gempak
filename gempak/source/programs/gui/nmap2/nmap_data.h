 ...
 *  R. Tian/SAIC	10/02	add save_auto_updt in loopstate_t	*
 *  J. Wu/SAIC		07/03	add time interval/range to dsrc_t	*
 *  T. Lee/SAIC		02/04	added base time to dominfo_t		*
 *  T. Lee/SAIC		04/04	added delta reference time to dsrc_t	*
 *  J. Wu/SAIC		10/04	add bfhr/afhr to dsrc_t for time binning*
 *  J. Wu/SAIC		12/04	add time binning on/off flag in dsrc_t	*
 *  m.gamazaychikov/SAIC 4/06	add domtmmtch flag in dsrc_t		*
 *  H. Zeng/SAIC	07/07   increased MAX_FRMSRC value to 26	*
 *  E. Safford/SAIC	12/07	temporarily added proto_nmaplib.h and   *
 *				  proto_xw.h includes			*
 *  F. J. Yen/NCEP	 4/08	Added bfmn, afmn, mstrctf to dsrc_t	*
 *  S. Jacobs/NCEP	12/10	Increased MAXDSRC_OF_DCAT from 40 to 60	*
 ***********************************************************************/
...
typedef struct {
    ...
    int		ionoff;			/* time bin - on/off flag       */
    int		bfhr;			/* time bin - hours before      */
    int		bfmn;	          /* minutes portion of time bin before */
    int		afhr;			/* time bin - hours after       */
    int		afmn;	          /* minutes portion of time bin after  */
    int		mstrctf;          /* only most recent flag for time bin */
    int		delta_rt;		/* delta reference time	        */	
    int         domtmmtch;	  /* time matching scheme for dom src   */
...
typedef struct {
    ...
    dttms_t	base_tm;		/* base time for dom source*/
} dominfo_t;
...
typedef struct {			/* loop state information      */
    ...
    Boolean	save_auto_updt;		/* copy of auto-update state   */
    Boolean	data_chngd;		/* data changed flag           */
} loopstate_t;



#define MAXDSRC_OF_DCAT  60	/* maximum data source in one category */
