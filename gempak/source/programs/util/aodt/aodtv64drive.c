...
#define DEFYFILE        "AODTDUMP.AODTv64"
/* Dummy date                                                   */
...
static char scenetype[][16] = { "CLEAR", "PINHOLE", "LARGE_CLEAR",
                         "LARGE_RAGGED", "RAGGED", "OBSCURED",
...
/*
 *  Prototypes for aodtv64_drive and aodtv64_exit
 */
 ...
 **									*
 * Log:									*
 * M. Li/SAIC		12/05	Modified from aodtv63drive.c		*	
 ...
 ***********************************************************************/
{
    ...
    int 	s_neweye, s_newcloud;
    int         g_neweye, g_newcloud, g_oldeye, g_oldcloud; 
...
/*---------------------------------------------------------------------*/
  ...
  */
     iaodt=aodtv64_setmiscoptions ( landflag, searchflag );
 /*
  ...
  */
     iaodt=aodtv64_setstartstr ( inclassflag, inclassvalu );
 /*
  ...
  */
     ...
     **/
     if ( lodt ) {
         ...
         */
	 cursat = -1;
         iaodt = aodtv64_setIRimageinfo ( curdate, curtime, cursat );
        /*
         *  Get image date/time info from AODT
         */
         iaodt = aodtv64_getIRimageinfo ( &g_imdate, &g_imtime, &g_sat, g_csat );
	...
	/*
	 ...
	 posm = 0;
	 if ( lauto == G_TRUE ) {
	     aodtv64_runautomode( nauto, fauto, imagefile, &cenlat, &cenlon, &posm  );
	 }

        /*
         *  Set center location in AODT
         */
         iaodt=aodtv64_setlocation ( cenlat, cenlon, posm );
         if ( iaodt != 0 ) {
            aodtv64_qmessage (iaodt, 0, NULL, retmsg);
            if ( iaodt < 0 ) {
               aodtv64_exit (retmsg);
         ...
         */
         iaodt = aodtv64_setdomain ( idomain );
         if ( iaodt != 0 ) {
            aodtv64_qmessage (iaodt, 0, NULL, retmsg);
            if ( iaodt < 0 ) {
               aodtv64_exit (retmsg);
         ...
         */
         ...
         if ( iaodt != 0 ) {
            aodtv64_qmessage (iaodt, g_domain, NULL, retmsg);
            if ( iaodt < 0 ) {
               aodtv64_exit (retmsg);
            }
         }
	 ...
	 free( lons  );
         ...
         */
         if ( loverride ) {
            ...
            */
	    iaodt=aodtv64_setscenetypes (s_neweye, s_newcloud, origeye, origcloud );
            if ( iaodt != 0 ) {
               aodtv64_qmessage (iaodt, 0, NULL, retmsg);
               if ( iaodt < 0 ) {
                  aodtv64_exit (retmsg);
     ...
     **/
	 ltmp = (ldelete || lcomment) ? 1 : 0;
         iaodt=aodtv64_setdatetime ( time1, time2, date1, date2, ltmp );
        /*
         ...
         */
         if ( llist) {
             ...
             */
             if ( ldump ) {
                ...
                if ( fdump != 0 ) {
                   aodtv64_qmessage (-71, 0, dumpfile, retmsg);
                   if ( iaodt < 0 ) {
                      aodtv64_exit (retmsg);
                   }
                }
	     ...
	     itype = -1;
             ...
             */
             iaodt = aodtv64_historygetnextrec ( 0, &historyrec);
             if ( iaodt != 0 ) {
                aodtv64_qmessage (iaodt, 0, NULL, retmsg);
                if ( iaodt < 0 ) {
                   aodtv64_exit (retmsg);
             ...
             */
             if ( ldump ) {
                fclose (fdump);
                aodtv64_qmessage (13, 0, dumpfile, retmsg);
             }
         }
        /*
         ...
         */
	     ...
	     cst_rxbl ( comment, comment, &len, &ier );
             iaodt = aodtv64_historyaddcomment ( comment, &n_comm );

	     if ( iaodt != 0 ) {
	         aodtv64_qmessage (iaodt, n_comm, historyfile, retmsg);
                 if ( iaodt < 0 ) {
                   aodtv64_exit (retmsg);
                 }
             }
