 ...
 **									*
 * Log:									*
 * M. Li/SAIC		12/06	Modified from testaodtv64.c		*
 ***********************************************************************/
{
    ...
    int         g_neweye, g_newcloud, g_oldeye, g_oldcloud, g_eyesize, ldelete; 
    int         indx, n_records, h_deleted, h_modified, h_records, eyeSize;
    ...
    char        latlon[32], device[32], clrbar[80], srcID[512], strmID[512];
...
/*---------------------------------------------------------------------*/
		...
		eyeSize = -99;
                iaodt=aodtv72_setmiscoptions ( s_oland, s_osearch, eyeSize );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 3 ) {
                ...
                iaodt=aodtv72_setstartstr ( s_classf, s_class );
...
/*---------------------------------------------------------------------*/
		...
		s_irsat = -1;
                iaodt = aodtv72_setIRimageinfo ( s_irdate, s_irtime, s_irsat );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 8 ) {
                ...
                iaodt = aodtv72_getIRimageinfo ( &g_irdate, &g_irtime, &g_sat, g_csat );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 9 ) {
                ...
                iaodt=aodtv72_setlocation ( s_lat, s_lon, s_posf );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 11 ) {
                ...
                iaodt = aodtv72_setdomain ( s_domain );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 13 ) {
                ...
                iaodt = aodtv72_setsstvalue ( s_sstvalue );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 14 ) {
                ...
                iaodt=aodtv72_getsstvalue ( &g_sstvalue );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 15 ) {
                ...
                iaodt = aodtv72_settopovalue ( s_topof );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 16 ) {
                ...
                iaodt=aodtv72_gettopovalue ( &g_topof );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 21 ) {
                ...
                iaodt=aodtv72_setscenetypes (s_neweye, s_newcloud,
                                            s_oldeye, s_oldcloud );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 30 ) {
                ...
                iaodt=aodtv72_setdatetime ( time1, time2, date1, date2, ldelete );
...
/*---------------------------------------------------------------------*/
            if ( numsub == 33 ) {
                ...
                iaodt = aodtv72_historygetnextrec ( recnumber, &historyrec);
...
/*---------------------------------------------------------------------*/
		...
		printf ("Enter comment to add: \n");
	 	scanf ( "%s", comment );
                iaodt=aodtv72_historyaddcomment( comment, &h_modified );

                printf("Number of records modified:     %d \n", h_modified);

                printf ( "\nAODT_HistoryAddcomment iaodt =     %d\n\n", iaodt );
                if ( iaodt != 0 ) {
                   aodtv72_qmessage (iaodt, h_modified, historyfile, retmsg);
                   if ( iaodt < 0 ) {
                      aodtv72_exit (retmsg);
                   } 
                }
