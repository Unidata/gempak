...
C* coordinates.								*
C* NOTE:  THIS version of the function is to be used with MKELEV only	*
C*									*
...
C* m.gamazaychikov/SAIC 06/06	Removed checks for nlcl > 7		*
C* C. Bailey/HPC	 8/06	Added contour label as input parameter  *
...
C
	...
	DO WHILE  ( more )
     ...
     +			     nlcl, xlcl, ylcl, more, ier )
	    ...
	    END IF
C
C*	    Only plot contours if scflag = .false. or 
C*	    if scflag = .true. and nlcl > 7
C
