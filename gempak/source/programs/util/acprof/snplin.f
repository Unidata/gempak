	SUBROUTINE SNPLIN  ( border, thtaln, thteln, mixrln, ibordr,
...
C* drawn.								*
C*									*
C* SNPLIN  ( BORDER, THTALN, THTELN, MIXRLN, IBORDR, ITHTAL, ITHTEL,	*
C*           IMIXRL, IRET )						*
C*									*
C* Input parameters:							*
C*	BORDER		CHAR*		Border input			*
...
C* S. Schotz/GSC	10/90	Call IN_LINE for border, called 	*
C*				IN_COLOR for thta, thte, mix lines	*
...
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
	CHARACTER*(*)	border, thtaln, thteln, mixrln
	INTEGER		ibordr (*), ithtal (*), ithtel (*), imixrl (*)
C*
	CHARACTER	color*28
	LOGICAL		scflag
C----------------------------------------------------------------------
	...
	values = 0.
C*
	CALL IN_LINE  ( border, values, 1, ibordr(1), ibordr(2), 
     +                  ibordr(3), iblab, smth, fltr, scflag, ier )
C*
C
