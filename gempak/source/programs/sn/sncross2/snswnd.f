...
C* Chiz/Unidata		 8/00	Rolled in variable color wind plotting	*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C
	    ...
	    IF  ( wintyp .eq. 'A' )  THEN
		DO m=1,nlv
                 iclr = iwnclr( min( ( int(s(m))/winc ) + 1, nc ) )
...
C
		IF  ( filtfc .eq. 0. )  THEN
		    DO m=1,nlv
		      ...
		      CALL GBARB  ( 'V', 1, vx(m), vy(m), s(m), d(m), iret )
		    END DO
		  ELSE
C
...
C
		    yold = vy (1)
		    iclr = iwnclr( min( ( int(s(1))/winc ) + 1, nc ) )
                    call GSCOLR( iclr, ier )
                    lclr = iclr
