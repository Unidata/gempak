...
C* Chiz/Unidata		 8/00	CReated SNCROSS2 from SNCROSS		*
C* T. Lee/GSC            8/00   Added calls to GSTANM and GENANM        *
C* Chiz/Unidata		 3/02	Added contour fill types		*
C************************************************************************
	INCLUDE 	'GEMPRM.PRM'
C*
...
C
     ...
     +				   times, ntime, iret )
			...
			END IF
		      ELSE
			iret  = -6
			CALL ER_WMSG  ( 'SNCROSS2', iret, dattim, ier )
			proces = .false.
			datold = ' '
		    END IF
		END IF
...
C
c		CALL IN_WIND  ( wind, wintyp, winuni, iwnclr, ier )
		CALL SNCWND  ( wind, wintyp, winuni, iwnclr, nc, ier )
C
...
C
		...
		IF  ( ( parm .eq. ' ' ) .and. 
     +                ( ( iwnclr(1) .eq. 0 ) .and. (nc.le.1) ) ) THEN
		    iret = -13
		    CALL ER_WMSG  ( 'SNCROSS2', iret, snparm, ier )
		    proces = .false.
		    newprm = .true.
...
C
     ...
     +				   iytmfr, iret )
			...
			iret = -4
			CALL ER_WMSG  ( 'SNCROSS2', iret, ' ', ier )
			proces = .false.
		      ELSE
...
C
		    IF ( wndexs )  THEN
		        IF  ( (iwnclr(1) .gt. 0 ) .or. (nc.gt.0) ) THEN
C			    CALL GSCOLR  ( iwnclr, ier )
