...
C* S. Lilly/NCEP	 8/10	Changed prog name in error messages	*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C
		...
		IF  ( .not. cmpflg (i) )  THEN
		    CALL ER_WMSG  ( 'SNDSLIST', -4, prmlst (i), ier )
		  ELSE IF  ( chrflg (i) )  THEN
		    CALL ER_WMSG  ( 'SNDSLIST', -5, prmlst (i), ier )
		  ELSE
		    np = np + 1
		    prmlst (np) = prmlst (i)
		    prmcnd (np) = prmcnd (i)
...
C
		...
		CALL ER_WMSG  ( 'SNDSLIST', -6, vc, ier )
		nparms = 0
...
C
	    ...
	    DO  i = 1, np
		IF  ( .not. cmpflg (i) )  THEN
		    CALL ER_WMSG  ( 'SNDSLIST', -7, tmprm (i), ier )
		  ELSE IF  ( chrflg (i) )  THEN
		    CALL ER_WMSG  ( 'SNDSLIST', -8, tmprm (i), ier )
		  ELSE
		    nstnp = nstnp + 1
