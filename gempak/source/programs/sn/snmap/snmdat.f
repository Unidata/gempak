	SUBROUTINE SNMDAT  ( dattim, iflno, newfil, datcur, 
     +			     ntime,  times, iret )
C************************************************************************
C* SNMDAT								*
C*									*
...
C*									*
C* SNMDAT  ( DATTIM, IFLNO, NEWFIL, DATCUR, NTIME, TIMES,		*
C*           IRET )							*
C*									*
C* Input parameters:							*
...
C* T. Piper/SAIC	 4/02	Increased timdst to 20 for consistency	*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
...
C
	    CALL SN_GTIM  ( iflno, LLMXTM, ntimf, timdst, iret )
	    IF  ( iret .ne. 0 )  THEN
...
C
     ...
     +			    times,  ier )
	    ...
	    ELSE 
		IF  ( ier .gt. 0 )
     +              CALL ER_WMSG  ( 'TI', ier, dattim, iret )
		iret   = -6
		datcur = ' '
	    END IF
	END IF
