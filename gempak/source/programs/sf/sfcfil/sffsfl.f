	SUBROUTINE SFFSFL  ( stnfil, nstn, stid, isnm, slat, slon, selv,
     +			     stat,   coun, ispri, iret )
C************************************************************************
C* SFFSFL								*
C*									*
C* This subroutine gets station information from the station table.	*
C*									*
C* SFFSFL  ( STNFIL, NSTN, STID, ISNM, SLAT, SLON, SELV, STAT, COUN,	*
C*           ISPRI, IRET )						*
C*									*
C* Input parameters:							*
...
C*	ISPRI (NSTN)	INTEGER		Station priorities		*
...
C* S. Jacobs/EAI	 7/92	Added FL_CLOS to close station file	*
...
C
	...
	IF  ( ier .ne. 0 )  THEN
	    ...
	    CALL ER_WMSG  ( 'SFCFIL', -3, stnfil, ierr )
	    iret = -1	
	    RETURN
	END IF
C
