...
C*
	...
	CHARACTER	carr(2)*(LLMXLN), ctmpl*(LLMXLN)
	CHARACTER       filnam*(MXNMFL*MXFLSZ), sep*1
	CHARACTER       path*25, tmplt*(MXTMPL), gtime*20, ndttm*40
	CHARACTER	strtim*20, endtim*20, fldtim*20, tmpfil*160
	CHARACTER	npath*(MXTMPL), rtemplt*(MXTMPL), 
...
C
	CALL ST_CLST ( filtyp, '|', ' ', 2, carr, num, ier)
C
	ctmpl = carr(1)
...
C
     ...
     +			ier )
	        CALL ST_RPST ( path, '%SITE%', carr(2), ipos, npath, 
     +			ier )
	    END IF
C
C*	    Get list of files in the current directory that matches the
C*	    template. If none are found, look in the npath directory.
C
	    CALL ST_LSTR ( npath, ilenp, ier )
