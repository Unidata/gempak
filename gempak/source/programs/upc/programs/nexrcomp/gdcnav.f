	SUBROUTINE GDCNAV  ( proj, kxky, gdarea, cprj, kx, ky,  
     +			     grdout, rnvblk, iret )
C************************************************************************
C* GDCNAV								*
C*									*
...
C*									*
C* GDCNAV  ( PROJ, KXKY, GDAREA, CPRJ, KX, KY, GRDOUT, RNVBLK, IRET )	*
C*									*
C* Input parameters:							*
...
C* K. Brill/GSC          4/90	Moved CALL GR_SNAV to GDCANL		*
C* K. Brill/NMC          9/90	Check for case of 360 lon range		*
...
C
	CALL LC_GARE  ( gdarea, rltln, cdproj, centrd, ier )
	 IF ( ier .ne. 0 )  THEN
	    iret = -5
	    CALL ER_WMSG  ( 'GDCFIL', iret, gdarea, ier )
	    RETURN
	END IF
C
...
C
	CALL GR_VNAV  ( cprj, kx, ky, grdout (1), grdout (2),
     +			grdout (3), grdout (4), angle1, angle2,
     +			angle3, angflg, valid, ier )
