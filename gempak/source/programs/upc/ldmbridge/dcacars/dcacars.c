 ...
 *
 *   dcncprof.c
 *      Generic netCDF format ACARS decoder. See usage information
 *   for options.
 */
#include <stdio.h>
#include <stdlib.h>
...
if(infilnam == 0)
   ...
   if(outfilnam == 0) /* create a temporary file name */
      ...
      tempnam[0] = '\0';
      sprintf(tempnam,".tmp_dcacars_netcdf.XXXXXX\0");
      mkstemp(tempnam);
...
head = decode_acars(infilnam,MISS,&iret);
