 ...
 *
 *   dcncprof.c
 *      Generic netCDF format Profiler/RASS decoder. See usage information
 *   for options.
 */
#include <stdio.h>
#include <stdlib.h>
...
opterr = 1;
while ((ch = getopt(argc,argv,"hvxl:f:n:e:")) != EOF)
   {
   switch(ch)
      {
      case 'h':
...
head = decode_ncprof(infilnam,MISS,&iret);
