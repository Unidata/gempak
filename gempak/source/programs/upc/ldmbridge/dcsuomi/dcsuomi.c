 ...
 *
 *   dcsuomi.c
 *      Generic netCDF format SUOMINET decoder. See usage information
 *   for options.
 */
#include <stdio.h>
#include <stdlib.h>
...
(void)fprintf(stderr,
	"\t-d filename  Log output (default uses local0, '-' to stdout\n");
(void)fprintf(stderr,
	"\t-v           Verbose logging\n");
(void)fprintf(stderr,
        "\t-x           Debug logging\n");
...
opterr = 1;
while ((ch = getopt(argc,argv,"hvxd:f:n:e:")) != EOF)
   {
   switch(ch)
      {
      case 'h':
...
head = decode_suomi(infilnam,MISS,&iret);
