	SUBROUTINE DG_GRID ( gdattm, glevel, gvcord, gfunc, pfunc, grid,
...
C* input by the user.							*
C*									*
C* DG_GRID  ( GDATTM, GLEVEL, GVCORD, GFUNC, PFUNC, GRID, 		*
...
C* R. Tian/SAIC		 1/07	Added check for LLMXGD			*
C************************************************************************
        INCLUDE         'GEMPRM.PRM'
C*
...
C------------------------------------------------------------------------
C
C*	Check for KXYD > LLMXGD
C
	CALL DG_KXKY ( kx, ky, ier )
	IF ( kx * ky .gt. LLMXGD ) THEN
	    iret = -47
	    RETURN
	END IF
C
