 ...
 * dg_g2gs								*
 *									*
 * This subroutine remaps sines of data from one grid to another grid.	*
 *									*
 ...
 *									*
 * dg_g2gs ( inttyp, glbwi, kxi, kyi, grdi, kxo, kyo, gixo, giyo, grdo,	*
 *	     iret )							*
 *									*
 * Input parameters:							*
 ...
 *	*gixo		float		Input grd rltv x on output grid *
...
/*----------------------------------------------------------------------*/
	...
	*iret = -68;
	...
	for ( j = 1; j < (*kyi); j += (*kyi) - 1 ) {
		...
		if ( rflct == G_TRUE ) {
		    if ( G_ABS ( xchk - sin ( grdi[cidx] ) ) <  RDIFFD ) {
		        rflct = G_FALSE;
		    }
		}
		i++;
