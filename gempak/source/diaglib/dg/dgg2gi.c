 ...
 * dg_g2gi								*
 *									*
 * This subroutine remaps data from one grid to another grid.	  	*
 *									*
 ...
 *									*
 * dg_g2gi ( inttyp, glbwi, kxi, kyi, grdi, kxo, kyo, gixo, giyo, grdo,	*
 *	     iret )							*
 *									*
 * Input parameters:							*
 ...
 *	*gixo		const float	Input grd rltv x on output grid *
 *	*giyo		const float	Input grd rltv y on output grid *
 *									*
 * Output parameters:							*
 ...
 * Log:									*
 * K. Brill/HPC		10/02						*
 * K. Brill/HPC		12/02	Multiply by radx2--divide outside loop	*
 * K. Brill/HPC		 3/03	Simplify/fix global grid wrap checks	*
 * K. Brill/HPC		 8/03	Do linear interpolation in the area	*
 *				average preserving code when r2 > rad2	*
 * K. Brill/HPC		 9/03	Fix problem with linear interp caused by*
 *				using NINT;  use MAX in computing rad2	*
 * K. Brill/HPC		 4/04	IF (k==0) k=1; add .005 to rad		*
 * S. Jacobs/NCEP	 6/05	Added check for out-of-bound points	*
 * R. Tian/SAIC		 2/06	Recoded from Fortran			*
 ************************************************************************/
{
    float rkio2, rdtst, xchk, rad2m, rad2, dx, dy, d2, x, y, xi, omx, omy,
        rad, radx2, sum, sumw, dr2, dr22, wt, ri, r2, tmp;
    int ir[4], jr[4], kio2, cidx, cidx1, i, j, k, io, jo, ip, ityp, in, jn,
        idx1, idx2, idx3, idx4, ib, jb, ie, je, ii, jj, jdif, npts;
    int rflct;
/*----------------------------------------------------------------------*/
    *iret  = 0;
	...
	*iret = -68;
	...
	for ( j = 1; j <= (*kyi); j += (*kyi) - 1 ) {
		...
		if ( rflct == G_TRUE ) {
		    if ( ! ( G_ABS ( xchk - grdi[cidx] ) < RDIFFD ) ) {
		        rflct = G_FALSE;
		    }
		}
		i++;
    ...
    /*
     * Set out of bounds points to missing.
     */
    for ( jo = 1; jo <= (*kyo); jo++ ) {
	for ( io = 1; io <= (*kxo); io++ ) {
	    cidx = ( jo - 1 ) * (*kxo) + io - 1;
	    if ( ( gixo[cidx] < 1 ) || ( gixo[cidx] > (*kxi) ) ) {
		gixo[cidx] = RMISSD;
	    }
	    if ( ( giyo[cidx] < 1 ) || ( giyo[cidx] > (*kyi) ) ) {
		giyo[cidx] = RMISSD;
	    }
	}
    }
	...
	for ( io = 1; io <= (*kxo); io++ ) {
		...
		jn = G_NINT ( giyo[cidx] );
		if ( in >= 1 && in <= (*kxi) && jn >= 1 && jn <= (*kyi) ) {
		    /*
		     * Nearest point value assignment.
		     */
		    cidx1 = ( jn - 1 ) * (*kxi) + in - 1;
		    grdo[cidx] = grdi[cidx1];
		}
	    } else if ( (*inttyp) == 2 ) {
		ityp = 1;
	    } else {
