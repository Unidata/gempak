 ...
 * in COMMON / DGOVEC /.						*
 *									*
 *     TNGV = ( V dot ( normalized orientation vector ) ) times		*
 *		   ( normalized orientation vector )			*
 *									*
 * TNGV generates a vector field.					*
 *									*
 * dv_tngv  ( iret )							*
 *									*
 * Output parameters:							*
 ...
 ************************************************************************/
{
	...
	float		orntv [ 2 ], ornang, du1, dv1, du2, dv2, orn1, orn2;
        float           rnm;
	...
	*iret = 0;
	...
	for ( i = ksub1 - 1; i < ksub2; i++ ) {
		...
		rnm = sqrt ( du2 * du2 + dv2 * dv2 );
	        orn1 = du2 / rnm;
	        orn2 = dv2 / rnm;
		gru[i] = ( du1 * orn1 + dv1 * orn2 ) * orn1;
		grv[i] = ( du1 * orn1 + dv1 * orn2 ) * orn2;
	    }
	}

        /*
         *	Make a name of the form 'TANGV'//u,v and update header;
         *	update stack.
         */
	dg_updv  ( "TANGV", &numu, &numv, &numu1, &numv1, iret );
