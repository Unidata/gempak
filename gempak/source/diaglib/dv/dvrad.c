 ...
 * dv_rad								*
 *									*
 * This routine computes the radial component of the wind.  A unit	*
 * vector is determined using oblique spherical triangles.  Once the	*
 * x and y components of the unit vectors are computed the radial	*
 ...
 ************************************************************************/
{
	...
	int		numu, numv, ilat, ilon, nvecu, nvecv;
	float		*gru, *grv, *grvecu, *grvecv, vdotr;
	int		ix, iy, idir, ispd, iazst;
	float		*grx, *gry, *grdir, *grspd, *grazst;

	char    	grid[13], parm[13], time1[21], time2[21];
	int		level1, level2, ignum, ivcord, kxyd, idlun;
