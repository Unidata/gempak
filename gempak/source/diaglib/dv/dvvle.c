 ...
 * dv_vle								*
 *									*
 * This subroutine finds values of the magnitude of V which are less    *
 * than or equal to S.							*
 *									*
 *     VLE (V, S) IF |V| <= S THEN V ELSE RMISSD			*
 *									*
 * dv_vle  ( iret )							*
 *									*
 * Output parameters:							*
	...
	*iret = 0;
	...
	for ( i= ksub1 - 1; i < ksub2; i++ ) {
	   if ( ERMISS ( grmag[i]) || ERMISS ( grnum1[i]) ) {
        ...
        /*
         *	Make a name of the form 'VLE'//V//S and 
         *	update both grid headers; update the stack.
         */
	dg_updv ( "VLE", &nu, &nv, &numu, &num1, iret );
	dg_putv ( &nu, &nv, iret );
