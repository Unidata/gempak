 ...
 * dv_mtng								*
 *									*
 * This routine computes the magnitude of the tangential component of	*
 ...
 * The tangential component of the wind is determined using the		*
 * equation:								*
 *		TNG =   V dot (k x r).					*
 * Cyclonic flow around the storm is set to be positive and 		*
 * anti-cyclonic flow is set to be negative.				*
 ...
 ************************************************************************/
{
	...
	int		level1, level2, ignum, ivcord, kxyd;

        int             numu, numv, ilat, ilon;
        int             ix, iy, numout, iazst, idir, ispd;
...
/*------------------------------------------------------------------------*/
	*iret = 0;
	...
	for ( i = ksub1 - 1; i < ksub2; i++ ) {
         ...
         */
	dg_updh  ( "MTNG", &numout, &numu, &zero, &ier );
