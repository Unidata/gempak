 ...
 * df_sge								*
 *									*
 * This subroutine finds values of S1 which are greater than or equal	*
 * to S2.								*
 *									*
 *     SGE (S1, S2) IF S1 >= S2 THEN S1 ELSE RMISSD			*
 *									*
 * df_sge ( iret )							*
 *									*
 * Output parameters:							*
...
/*----------------------------------------------------------------------*/
    *iret = 0;
    ...
    /*
     * Get a name of the form 'SGE'//S1//S2 and update header;
     * update stack.
     */
    dg_updh ( "SGE", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
