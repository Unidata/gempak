 ...
 * df_quo								*
 *									*
 * This subroutine computes the quotient of two scalar grids:		*
 *									*
 *     QUO (S1, S2) = S1 / S2						*
 *									*
 * df_quo ( iret )							*
...
/*----------------------------------------------------------------------*/
    *iret = 0;
    ...
    /*
     * Get a new grid number and divide the grids.
     */
    dg_nxts ( &num, iret );
    if ( *iret != 0 ) return;
	...
	dg1 = gnum1[im1];
	dg2 = gnum2[im1];
	if ( ERMISS ( dg1 ) || ERMISS ( dg2 ) || ( G_DIFFT(dg2, 0.0F, GDIFFD) ) ) {
	    gnum[im1] = RMISSD;
	} else {
	    gnum[im1] = dg1 / dg2;
	}
    ...
    /*
     * Make a name of the form 'QUO'//S1//S2 and update header;
     * update header.
     */
    dg_updh ( "QUO", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
