 ...
 * df_ncge								*
 *									*
 * This subroutine computes NCGE (S,T,ROI), the neighborhood count      *
...
/*----------------------------------------------------------------------*/
    ...
    */
	...
	for ( col = jgxmin; col <= jgxmax; col++ ) {
            ...
            nearby[indx] = 0;
            if ( ERMISS ( gnum1[indx] ) || ( thresh <= rmin ) ) {
		gnumn[indx] = RMISSD;
            } else {
                gnumn[indx] = 0.0;
            }
...
/* Calculate count if neighboring point is defined and within radius of influence. */
    ...
    /*
     * Make a name of the form 'NCGE'//S and update header;
     * update stack.
     */
    dg_updh ( "NCGE", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
