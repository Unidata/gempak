 ...
 * df_relh								*
 *									*
 * This subroutine computes the relative humidity from the temperature	*
 * and dewpoint temperature.						*
 *									*
 *     RELH  ( TEMP, DWPT ) = PD_RELH ( TEMP, DWPT )			*
 *									*
 * df_relh ( iret )							*
 *									*
 * Output parameters:							*
 ...
 * M. desJardins/GSFC	 2/90	Correct calling sequence to DF_RELH	*
...
/*----------------------------------------------------------------------*/
    *iret = 0;
    ...
    kxyd = kxd * kyd;
    pd_relh ( gnum1, gnum2, &kxyd, gnum, &ier );

    /*    
     * Make a name of the form 'RELH'//S1//S2 and update header;
     * update stack.
     */
    dg_updh ( "RELH", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
