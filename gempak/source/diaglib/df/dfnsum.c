 ...
 * df_nsum								*
 *									*
 * This subroutine computes NSUM (S,ROI), the neighborhood              *
 *  sum of a scalar field (S) within some radius of influence	        *
 ...
 ************************************************************************/
{
    int num1, num2, num3, num4, num, kxd, kyd, ksub1, ksub2, zero, indx, ier;
...
/*----------------------------------------------------------------------*/
    ...
    */
    ...
    dg_getg ( &num4, &nearby, &kxd, &kyd, &ksub1, &ksub2, &ier );
...
/*           if ( ERMISS ( gnum1[indx] ) ) {
    ...
    /*
     * Loop over all grid points to determine neighborhood sum for each grid point.
     */
...
/* Calculate sum if neighboring point is defined and within radius of influence. */
                        if ( (dist[boxindx] <= radius) ) { 
                           if  (! ERMISS ( gnum1[boxindx] ) ) { 
                                 gnumn[indx] = gnum1[boxindx] + gnumn[indx];
                           } else {
                            /* Grid point is nearby one that is defined  */
                                nearby[boxindx] = 1;
                           }   
                        }
                    }
    ...
    for ( row = jgymin; row <= jgymax; row++ ) {
       for ( col = jgxmin; col <= jgxmax; col++ ) {
         indx=(row-1)*kxd+(col-1);
/* Consider undefined points within radius of earlier defined points */
...
/* Calculate sum if neighboring point is defined and within radius of influence. */
                        if ( (dist[boxindx] <= radius) && (! ERMISS ( gnum1[boxindx] ) ) ) {
                                gnumn[indx] = gnum1[boxindx] + gnumn[indx];
    /*                   printf("%d %d %f %f\n",indx,boxindx,gnum1[boxindx],gnumn[indx]); */
                        }   
                    }
                }
             }
    ...
    /*
     * Make a name of the form 'NSUM'//S and update header;
     * update stack.
     */
    dg_updh ( "NSUM", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
