 ...
 * df_thta								*
 *									*
 * This subroutine computes the potential temperature in Kelvin		*
 * from the temperature and pressure:					*
 *									*
 *     THTA  ( TMPC, PRES ) = PD_THTA  ( TMPC, PRES )			*
 *									*
 * df_thta ( iret )							*
 *									*
 * Output parameters:							*
 ...
 * M. desJardins/GSFC	 2/90	Correct call to PD_THTA			*
...
/*----------------------------------------------------------------------*/
    *iret = 0;
    ...
    /*
     * Get a new grid number and compute theta.
     */
    dg_nxts ( &num, iret );
    if ( *iret != 0 ) return;
    ...
    kxyd = kxd * kyd;
    pd_thta ( gnum1, gnum2, &kxyd, gnum, &ier );

    /*
     * Make a name of the form 'THTA'//S1//S2 and update header;
     * update stack.
     */
    dg_updh ( "THTA", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
