 ...
 * df_slt								*
 *									*
 * This subroutine finds values of S1 which are less than S2.		*
 *									*
 *     SLT (S1, S2) IF S1 < S2	THEN S1 ELSE RMISSD			*
 *									*
 * df_slt ( iret )							*
 *									*
 * Output parameters:							*
...
/*----------------------------------------------------------------------*/
    *iret = 0;
    ...
    /*
     * Get a name of the form 'SLT'//S1//S2 and update header;
     * update stack.
     */
    dg_updh ( "SLT", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
