 ...
 * df_sle								*
 *									*
 * This subroutine finds values of S1 which are less than or equal 	*
 * to S2.								*
 *									*
 *     SLE (S1, S2) IF S1 <= S2 THEN S1 ELSE RMISSD			*
 *									*
 * df_sle ( iret )							*
 *									*
 * Output parameters:							*
...
/*----------------------------------------------------------------------*/
    *iret = 0;
    ...
    /*
     * Get a name of the form 'SLE'//S1//S2 and update header;
     * update stack.
     */
    dg_updh ( "SLE", &num, &num1, &num2, iret );
    dg_puts ( &num, iret );
