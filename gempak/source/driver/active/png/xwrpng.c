 ...
 * xwrpng								*
 *									*
 * This subroutine writes out a png file from the current graphics 	*
 * window.								*
 *									*
 * xwrpng ( iret )							*
 *									*
 ...
 * S. Chiswell/Unidata	 7/03	Modified for PNG			*
 ***********************************************************************/
{
    XColor		colors[256];
    XImage		*image ;
...
/*---------------------------------------------------------------------*/
	...
	*iret = -1;
	...
	if  ( gemwindow[i].name[0] != '\0' )
	{
	    ...
	    ier = WritePNG( fp, imgdatptr, cwin->width,
