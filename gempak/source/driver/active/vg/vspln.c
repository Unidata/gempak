 ...
 * vspln								*
 *									*
 * This subroutine draws a special line to the device by constructing a *
 * VG record for the line and writing the record to the file at the     *
 * proper location.	                                                *
 *									*
 * vspln ( np, x, y, iret )						*
 *									*
 ...
 * Log:									*
 * D. Keiser/GSC	 4/97						*
 * D.W.Plummer/NCEP      7/97   Added filled and closed                 *
 * S. Law/GSC		04/98	Added smooth				*
 * S. Jacobs/NCEP	12/98	Added check for closed lines		*
...
{
    VG_DBStruct	el;
    int		npts, lclspt, filled, closed, recsize;
...
/*
 *  Initialize a special line element.
 */
 ...
 */
	...
	for ( ii = 0; ii < npblk; ii++ ) {
	    if ( iblk <= nblk - 1 - lclspt ) {
	        ...
	        /*
                 ...
                 */
		el.elem.spl.latlon[ii      ] = x[ipt1 + ii];
		el.elem.spl.latlon[ii+npblk] = y[ipt1 + ii];
	    }
                 ...
                 */
		el.elem.spl.latlon[1      ] = x[0];
		el.elem.spl.latlon[1+npblk] = y[0];
		el.elem.spl.latlon[0      ] = x[npts - 1];
		el.elem.spl.latlon[0+npblk] = y[npts - 1];
	    }

            /* 
             *  Establish max and mins if they apply. 
             */
	    if (el.elem.spl.latlon[ii+npblk] < el.hdr.range_min_lon )
		    el.hdr.range_min_lon = el.elem.spl.latlon[ii+npblk];
	    if (el.elem.spl.latlon[ii      ] < el.hdr.range_min_lat )
		    el.hdr.range_min_lat = el.elem.spl.latlon[ii      ];
	    if (el.elem.spl.latlon[ii+npblk] > el.hdr.range_max_lon )
		    el.hdr.range_max_lon = el.elem.spl.latlon[ii+npblk];
	    if (el.elem.spl.latlon[ii      ] > el.hdr.range_max_lat )
	...
	el.elem.spl.info.spltyp = ksltyp;
	el.elem.spl.info.splstr = kslstr;
	el.elem.spl.info.spldir = ksldir;
	el.elem.spl.info.splsiz = rslsiz;
	el.elem.spl.info.splwid = kslwid;
