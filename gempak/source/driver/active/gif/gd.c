...
/*---------------------------------------------------------------------*/
	...
	im->pixels = (unsigned char **) malloc (sizeof 
			(unsigned char *) * sx);
	im->polyInts = 0;
	im->polyAllocated = 0;
	im->brush = 0;
...
{
	int	dx, dy, incr1, incr2, d, x, y, xend, yend, xdirflag,
	        ydirflag;
/*---------------------------------------------------------------------*/
	dx = abs (x2 - x1);
	dy = abs (y2 - y1);
...
void gdImageDashedLine ( gdImagePtr im, int x1, int y1, int x2, 
						int y2, int color )
{
	int	dx, dy, incr1, incr2, d, x, y, xend, yend, xdirflag,
	        ydirflag;
...
void gdImageCharUp ( gdImagePtr im, gdFontPtr f, int x, int y, char c, 
								int color )
{
	int             cx, cy;
	int             px, py;
...
void gdImageString ( gdImagePtr im, gdFontPtr f, int x, int y, char *s, 
								int color )
{
	int             i;
	int             l;
...
void gdImageStringUp ( gdImagePtr im, gdFontPtr f, int x, int y, char *s, 
		 						 int color )
{
	int             i;
	int             l;
...
void gdImageArc ( gdImagePtr im, int cx, int cy, int w, int h, int s, int e, 
								     int color )
/*
 * s and e are integers module 360 (degrees), with 0 degrees being the
 * rightmost extreme and degrees changing clockwise. cx and cy are the 
 * center in pixels; w and h are the horizontal and vertical diameter 
 * in pixels.  Nice interface, but slow, since I don't yet use Bresenham
 * (I'm using an inefficient but simple solution with too much work 
 * going on in it; generalizing Bresenham to ellipses and partial arcs 
 * of ellipses is non-trivial, at least for me) and there are other 
 * inefficiencies (small circles do far too much work).
 */
{
	int             i;
	int             lx = 0, ly = 0;
...
/*
 * a code_int must be able to hold 2**GIFBITS values of type int, and 
 * also -1
 */
...
/*---------------------------------------------------------------------*/
{
	...
	init_statics ();
	/* All set, let's do it. */
	GIFEncode ( out, im->sx, im->sy, interlace, 0, transparent, 
 ...
 *		Transparent, BitsPerPixel, Red, Green, Blue,
 *		gdImagePtr )				
 */
 ...
 */
{
	 ...
	 * beginning If we are interlaced, bump the cury to the 
	 * appropriate spot, otherwise, just increment it.
	 */
	if (curx == Width)
	{
		curx = 0;
 ...
 * Jim McKie               (decvax!mcvax!jim) Steve Davies
 * (decvax!vax135!petsd!peora!srd) Ken Turkowski
 * (decvax!decwrl!turtlevax!ken) James A. Woods
 ...
 * those used by decompress().  The tab_prefix table is the same size 
 * and type as the codetab.  The tab_suffix table needs 2**GIFBITS 
 * characters.  We get this from the beginning of htab.  The output 
 * stack uses the rest of htab, and contains characters.  There is 
 * plenty of room for any possible stack (stack used to be 8000 
 * characters).
 */
 ...
 * to a faster exclusive-or manipulation.  Also do block compression 
 * with an adaptive reset, whereby the code table is cleared when the 
 * compression ratio decreases, but after the table fills.  The 
 * variable-length output codes are re-sized at this point, and a 
 * special CLEAR code is generated for the decompressor.  Late addition:
 * construct the table according to file size for noticeable speed 
 * improvement on small files.  Please direct questions about this 
...
static void init_statics ( void )
{
	 ...
	 * of statics doesn't break when the code gets called more than 
	 * once.
	 */
	Width = 0;
	Height = 0;
...
#define        ReadOK(file,buffer,len) (fread(buffer, len, 1, file) != (size_t)0)
...
*/
static int      ReadColorMap (FILE * fd, int number, unsigned char 
		(*buffer)[256]);
static int      DoExtension (FILE * fd, int label, int *Transparent);
static int      GetDataBlock (FILE * fd, unsigned char *buf);
static int      GetCode (FILE * fd, int code_size, int flag);
static int      LWZReadByte (FILE * fd, int flag, int input_code_size);
static void     ReadImage (gdImagePtr im, FILE * fd, int len, 
		int height, unsigned char (*cmap)[256], int interlace, 
		int ignore);
...
*/
	...
	for (;;)
	{
		...
		{		/* GIF terminator */
			...
			/*
			 * Check for open colors at the end, so we can 
			 * reduce colorsTotal and ultimately BitsPerPixel
			 */
...
/*---------------------------------------------------------------------*/
{
	...
	for (y = srcY; (y < (srcY + h)); y++)
	{
		...
		for (x = srcX; (x < (srcX + w)); x++)
		{
			...
			if (colorMap[c] == (-1))
			{
				/* If it's the same image, mapping is 
				   trivial */
				if (dst == src)
				{
					nc = c;
	...
	for (y = srcY; (y < (srcY + srcH)); y++)
	{
		for (ydest = 0; (ydest < sty[y - srcY]); ydest++)
		{
			...
			for (x = srcX; (x < (srcX + srcW)); x++)
			{
				...
				/*
				 * Have we established a mapping for this
				 * color?
				 */
				if (colorMap[c] == (-1))
				{
					...
					if (nc == (-1))
					{
					    /* No, so try to allocate 
					     * it
					     */
					    nc = gdImageColorAllocate (
						 dst, src->red[c], 
						 src->green[c],
						 src->blue[c]);

					    /*
					     * If we're out of colors, 
					     * go for the closest color
					     */
					    if (nc == (-1))
					    {
						nc = gdImageColorClosest
						     (dst, src->red[c], 
						     src->green[c], 
						     src->blue[c]);
					    }
...
/*---------------------------------------------------------------------*/
{
	...
	for (y = y1; (y <= y2); y++)
	{
		...
		for (i = 0; (i <= n); i++)
		{
			...
			int             dir;
			int             ind1, ind2;
			/* int lastInd1 = 0; !!! DBA moved out of scope*/
			if ((i == n) || (!i))
			{
				ind1 = n - 1;
				ind2 = 0;
			...
			if ((y >= y1) && (y <= y2))
			{
				...
				/*
				 * Only count intersections once except 
				 * at maxima and minima. Also, if two
				...
				if (!interFirst)
				{
				  if  (( ( p[ind1].y == p[lastInd1].y ) 
				      ...
				      || (( p[ind2].y == p[lastInd1].y )
				      && ( p[ind2].x != p[lastInd1].x) ))
				  {
					if (dir == dirLast)
					{
					    if (inter > interLast)
		...
		/* if the number of intersections in the line
		 * is an odd number, then an error will result
		 * due to  miscounted intersections. To
		 * correct this, we check for oddness, then
		 * delete the last point in the intersection 
		 * array if we have an odd number of them,
		 * by subtracting one from the 'ints' variable.
		 */
