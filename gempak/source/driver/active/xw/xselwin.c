...
void xselwin ( char win_name[], int *len, float xsize, float ysize, 
 ...
 *									*
 * xselwin ( win_name, len, xsize, ysize, ixsize, iysize, iret )	*
 *									*
 * Input parameters:							*
...
/*---------------------------------------------------------------------*/
	...
	*iret = G_NIWNAM;
	...
	for ( ii = 0; ii < MAX_WINDOW; ii++ ) {
		...
		xopenw (win_name, ii, xsize, ysize, ixsize, iysize, &ierr);
		window_set = 1;
		current_window = ii;
		*iret = G_NEWWIN;
		break;
