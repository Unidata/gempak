 ...
 * proto_gemlib.h                                                       *
 *                                                                      *
 * NOTE:  When adding entries, please maintain alphabetical order.      *
 *                                                                      *
 **                                                                     *
 * Log:                                                                 *
 ...
 * X. Guo/CWS           04/10   Added im_nexbzwmo function              *
 * m.gamazaychikov/CWS  03/11   Remove db_checkConnection, db_gradhdr,  *
 *                              db_gradimg, db_gsathdr, db_gsatimg,     *
 *                              db_getBinaryData, db_gNextColumn.       *
 *                              Add db_gtim, db_getduri, db_getgnav,    *
 ...
 * S. Gilbert/NCEP      2/16    Added cycle and fcst args to            *
 *                              da_getheader                            *
...
/* CV library */
void cv_mdfy(int *npo, float *xo, float *yo, int *npc, float *xc, float *yc,
        int *sm_lvl, int *oline, int *maxpts, int *np, float *x, float *y,
        int *is, int *ie, int *iret);

void cv_prmt(int*, float*, float*, float*, int*, float*, int*, int*, int*,
        float*, float*, int *iret);

void cv_rduc(int *npts, float *xpts, float *ypts, float *filt, int *nout,
        float *xout, float *yout, int *iret);

/* DG library */
void dgc_grid(const char *gdattm, const char *glevel, const char *gvcord,
        const char *gfunc, char *pfunc, float *grid, int *igx, int *igy,
        char *time1, char *time2, int * level1, int *level2, int *ivcord,
        char *parm, int *iret);
void dgc_ndtm(const char *gdatm, int *iret);
void dgc_nfil(const char *gdfile, const char *gdoutf, int *iret);
void dgc_ntim(const int *chngnv, const int *coladd, char *time1, char *time2,
        int *nxtok, int *iret);
...
void er_wmsg(char *errgrp, int *numerr, char *errstr, int *iret, size_t,
        size_t);

/* DB library */
void da_getdti(char *station, char *time, int *partnum, int *iflno, int *idthdr,
        int *idata, int *nval, int *iret);
void da_getdtc(char *station, char *time, int *partnum, int *iflno, int *idthdr,
        char *cdata, int *nchr, int *iret);
void da_getdtr(char *station, char *time, int *partnum, int *iflno, int *idthdr,
        float *rdata, int *nval, int *iret);
void da_getdtrgd(char *cycle, char* forecast, char *level1, char *level2,
        char *gvcord, char *gparm, int *iflno, int *idthdr, float *rdata,
        int *nval, int *iret);

void da_getfilhdc(int *iflno, char *fhdnam, int *mxchar, char *cheadr,
        int *nchar, int *iret);
void da_getfilhdi(int *iflno, char *fhdnam, int *mxword, int *iheadr,
        int *nword, int *iret);
void da_getfilhdr(int *iflno, char *fhdnam, int *mxword, float *rheadr,
        int *nword, int *iret);

void da_getfhdr(int *iflno, int *nfhdr, int *type, int *fhlen, int *iret);
void da_getheader(int *iflno, char *hdrtyp, char *cycle, char* forecast,
        int *nval, int *values, int *iret);
...
void da_getnums(int *iflno, int *rows, int *cols, int *prts, int *fhds,
        int *iret);
void da_getparm(int *iflno, int *npart, int *nparm, int *scale, int *offset,
        int *bits, int *iret);
...
void da_insertParam(char* paramName, char* paramSource, int typeVal,
        char* paramReport, float valuef, int valuei, char* valuec, void** root);
void da_insertParamArray(char* paramName, char* paramSource, int typeVal,
        char* paramReport, int size, float* valuef, int* valuei, char** valuec, void** root);
void da_insertStation(char* station, void* paramTree, void** root);
void da_insertStationParams(void* station, void* newParamTree);
void da_insertTimesParam(void** root, char* param, int typeVal,
        char* parmSource, char* paramReport);
void da_insertTimestamp(char* timestamp, void* stationRoot,
        void** timestampRoot);
void da_insertTimestampParams(void* timestampRoot, char** params, int parmSize,
        int type, char* parmSource, char* paramReport);
void da_paramcmp(char* param1, char* source1, char* report1, char* param2,
        char* source2, char* report2, int* iret);
void da_paramExistsStation(void* root, char* param, char* source, char* report, int* iret,
        void** node);
void da_paramExistsTimestamp(void* timestampRoot, char** params, int parmSize,
        char* parmSource, char* paramReport, int* iret);
...
void da_returnParamValue(char* param, void* root, int* iRetType, float* fValue,
...
void dm_pkgd(float*, int*, int*, int*, int*, int *, int*, int*, float*, float*,
        float*, int *iret);
void dm_wpkg(int*, float*, int*, int*, int*, int*, int *, int*, int*, int*,
...
void dp_udif(int*, int*, int*, float*, float*, float*, int*, int*, float*,
        int *iret);
...
void gb_gbdh(int *length, int *bdflag, int *scale, float *ref, int *nbits,
        int *iret);

void gb_ggds(float *gdsarr, int *irltv, int *nrmflg, int *iret);

void gb_gpbd(int *kx, int *ky, int *iuscal, float *ref, float *scale,
        int *nbits, int *igrid, int *lengrd, int *iret);

void gb_gpds(int *itime, int *iaccm, int *ilevel, int *ivcord, int *iparm,
        int *igrdnm, int *icodtbl, int *icenter, int *isgds, int *isbms,
        int *ispdse, char *cpds, int *iclen, int *iret);

void gb_gubd(int *kx, int *ky, int *iuscal, int *misflg, float *rmsval,
        int *nrmflg, int *irltv, int *nbits, float *fgrid, int *iret);

void gb_next(int *ivers, int *iret);

void gb_open(char *filnam, int *lenfil, char *idxnam, int *lenidx, int *iret);

void gb_read(int *mxgrib, int *ipack, int *iret);

/* GD library */
void gdc_open(char *gdfile, int *wrtflg, int *mxanl, int *mxnav, int *iacss,
        float *anl, float *rnav, int *msxgrd, int *iret);
...
void gd_open(char *, int *, int *, int *, int *, float *, float *, int *,
        int *iret, size_t);
void gd_wpgd(int*, float*, int*, int*, int*, char*, int*, int*, char*, int*,
        int*, int*, int *iret, size_t, size_t);

/* GG library */
void gg_asrd(char *filtyp, char *filnam, char *stime, char *etime, char *dtime,
        float fwninc[], int icolrs[], int icolrs2[], int *numclr, int *iskip,
        int *interv, int *itmclr, int iflags[], int *ityp, int *iret);
...
void gg_osrd(char *filtyp, char *filnam, char *stime, char *etime, char *dtime,
        float fwninc[], int icolrs[], int icolrs2[], int *numclr, int *iskip,
        int *interv, int *itmclr, int iflags[], int *ityp, int *iret);
void gg_panl(char *panel, int *iret, size_t);
void gg_proj(const char*, char*, float*, float*, int*, int*, size_t, size_t);
void gg_qsrd(char *filtyp, char *filnam, char *cstime, char *etime, char *dtime,
        float fwninc[], int icolrs[], int icolrs2[], int *numclr, int *iskip,
        int *interv, int *itmclr, int iflags[], int *ityp, int *iret);
...
void gg_wfps(int*, char*, char*, int*, int wcnty[], int*, size_t, size_t);
void gg_wlbl(int*, float*, float*, float*, float*, int *iret);
void gg_wlso(int *npts, int nfips[], int *mxp, int *nunion, int *nv, float xp[],
        float yp[], float *xc, float* yc, float* area, int *iret);
void gg_wsrd(char *filtyp, char *filnam, char *stime, char *etime, char *dtime,
        float fwninc[], int icolrs[], int icolrs2[], int *numclr, int *iskip,
        int *interv, int *itmclr, int iflags[], int *ityp, int *iret);
void gg_wstr(char *string, int *line, int *iret, size_t);
void gg_wtch(char*, int*, float*, int*, int*, int *iret, size_t);
void gg_wusc(char *ugcin, char *stin, char *namin, int *numb, char *cday,
        char *chour, int *vtecln, char *prdcod, char *actn, char *offid,
        char *phen, char *sigcd, char *etn, int vtime[], int etime[],
        char *cntystr, int *ilenout, int *iret);
void gg_wwfo(char*, char*, int *iret, size_t, size_t);
void gg_wwtp(int*, int*, char*, char*, char*, char*, char*, char*, int*, char*,
        char*, char*, int*, char*, char*, char*, char*, int*, int*, int*, int*,
...
void gh_wwbk(int (*)[4], int[], int (*)[4], int[], int*, char (*)[1000],
        char (*)[1000], int (*)[3], char (*)[605], int*, char*, int*, size_t,
        size_t, size_t, size_t);
void gh_wwbl(int (*)[4], int[], int (*)[4], int[], int*, char (*)[1000],
        char (*)[1000], int (*)[3], char (*)[605], int*, char*, int*, size_t,
        size_t, size_t, size_t);
void gh_wwtx(char*, char*, char*, char*, char*, int*, char*, int*, char*, char*,
        char (*)[1000], char (*)[1000], int (*)[3], char (*)[605], int*, size_t,
        size_t, size_t, size_t, size_t, size_t, size_t, size_t, size_t, size_t,
        size_t);
void gh_0bkp(char*, char*, int*, float*, float*, int*, float*, int*, int*,
        size_t, size_t);
...
/* GU library */
void gcboxx(const int *, const int *, const float *, const int *, const int *,
...
void gr_gtim(char *, char *, char *, char *, char *, int *, size_t, size_t,
        size_t, size_t, size_t);
void gr_levl(char *, int *, int *, int *iret, size_t);
void gr_ltln(int *, int *, float *, float *, int *iret);
void gr_mbn2(const float*, const int*, const float*, const float*, float*,
        int*);
void gr_mnav(char*, int*, int*, float*, float*, float*, float*, float*, float*,
        float*, int*, float*, int *iret, size_t);
void gr_pack(char *, int *, int *, int *iret, size_t);
void gr_plin(char *, int *, int *, float *, float *, float *, float *,
        int *iret, size_t);
void gr_ploc(char *, float *, float *, float *, float *, int *iret, size_t);
void gr_rban(float *, float *, float *, float *, float *, float *, float *,
        int *, int *iret);
void gr_rnav(float *, char *, int *, int *, int *, size_t);
void gr_setr(int *, int *, int *, int *iret);
void gr_snav(int *, float *, int *iret);
void gr_sscl(int*, const int*, const int*, const int*, const int*, const int*,
        const int*, float*, float*, float*, int*);
void gr_stat(float *, int *, int *, int *, int *, int *, int *, float*, float*,
        float*, float*, int *iret);
void gr_suba(const char*, const int*, float*, float*, int*, int*, int*, int*,
        size_t);
void gr_vnav(char*, int*, int*, float*, float*, float*, float*, float*, float*,
        float*, int*, int*, int *iret, size_t);
void gr_wgb2(int*, int*, int*, int*, char*, int*, int*, char*, int*, int*, int*,
        int*, int*, int*, int*, size_t, size_t);

/* GRC library */
void grc_geni(const char *gdfile, FILE **fps, const int *nfps, int *iret);
void grc_wnmc(FILE **fps, const int *nfps, const int *title, const int *num,
        const char *gdattm1, const char *gdattm2, const int *level1,
        const int *level2, const int *ivcord, const char *parm,
        const int *nmcprm, const int *nmcvco, const int *nmcgdn, int *iret);

/* IN library */
void in_bdta(int *iret);
void in_cint(char*, float*, int *npts, float *gmin, float *gmax, float*, int*,
        float *frint, int *iret, size_t);
...
void in_txtn(char *text, int *ifont, int *ihwsw, float *siztxt, int *itxwid,
        int *ibrdr, int *irrotn, int *ijust, int *iret, size_t);
...
void inc_outt(const char *output, const char *def, int *termflg, int *fileflg,
        char *filnam, int *iret);
...
void mv_itob(int *ivalue, int *istart, int *nbytes, unsigned char *barray,
        int *iret);
...
void sf_rspc(int *isffln, char *string, int *nchar, int *ihhmm, int *nrep,
        int *iret, size_t);
void sf_rstr(int *isffln, char *string, int *ihhmm, int *nchar, int *iret,
        size_t);
...
void tb_nids(const int *, const int *, char *, int *, char *, float *, char *,
        int *iret);
...
void cti_mtch(int match, const char *dattim, const char **times, int ntime,
        int minute, int *ipos, int *iret);
void ti_mtch(int*, char *dattim, char*, int*, int*, int*, int *iret, size_t,
        size_t);
...
void cti_tmln(char **timin, int nin, int mrange, int intrvl, int idir,
        int iflag, int iauto, char *basetm, char *endtim, int *idelrt,
        char **timout, int *ntime, int *iret);
...
void gcbar(char *clrbar, int *nflvl, float *flvl, int *ifcolr, int *iret,
        size_t);
...
void ggapsm(float*, float*, int*, int*, int*, int*, int*, int*, float*, float*,
        int*, int *iret);
void ggdriv(float *grid, float *grid1, int *kx, int *ky, float *hist,
        float *work1, float *work2, float *work3, float *buffer, int *rspflg,
        int *iret);
...
void gqgprj(char *proj, float*, float*, float*, int*, int*, float*, float*,
        float*, float*, int *iret, size_t);
void gqmprj(char *proj, float*, float*, float*, float*, float*, float*, float*,
        int *iret, size_t);
...
void gsgprj(char *, float *, float *, float *, int *, int *, float *, float*,
        float*, float*, int *iret, size_t);
...
void gsmprj(char *proj, float*, float*, float*, float*, float*, float*, float*,
        int *iret, size_t);
...
void gsbrgb(int *icbank, int *ncolr, int *icolrs, int *reds, int *igrns,
        int *iblus, int *iret);
...
void gplbnd(char*, int*, float*, int*, float*, int*, int*, int*, int*, char*,
        int*, int*, float*, int*, int*, size_t, size_t);
...
void gtext(char*, float*, float*, char*, float*, int*, int*, int*, size_t,
        size_t);
void gtextc(char *sys, float *x, float *y, char *cchar, float *rotat,
        int *ixoff, int *iyoff, int *iret, size_t, size_t);
void gtrans(char*, char*, int*, float*, float*, float*, float*, int*, size_t,
        size_t);
...
void oabsdr(float *gelat, float *gelon, float *coslat, float *data,
        int *infoflg, int *iret);
