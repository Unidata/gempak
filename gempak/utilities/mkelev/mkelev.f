...
C* PROGRAM MKELEV							*
C*									*
C* This program produces VGF elevation file.				*
C**									*
C* Log:									*
C* m.gamazaychikov/SAIC 06/06	Created from GDCNTR			*
C* m.gamazaychikov/SAIC 07/06	Fixed a bug on the border of teh area	*
C* C. Bailey/HPC	 8/06	Modify IN_INTC, GCLGRN, GCNTLN calling	*
C*				seq. to include contour label array	*
C* C. Bailey/HPC	10/06	Add scflag to IN_LINE, GCLGRN, & GCNTLN *
...
C*
	CHARACTER	time (2)*20, ttlstr*72, garout*72, parm*12,
     +			pfunc*72, prjout*72, dev*72, carr (4)*72, 
     +			filnam*72, clbl(LLCLEV)*24
...
C
		...
		IF ( ier .ne. 0 ) THEN
		    ier = 2
		    CALL ER_WMSG ( 'MKELEV', ier, time(1), irr )
		END IF
...
C
		      DO i = 1, nclvl
			CALL GR_LABL(clvl(i), 0, 0, clbl(i), 
     +				       nchar, ier)
		      END DO
C*
		      CALL IN_LINE ( line, clvl, nclvl, icolor, 
...
C
		    IF  ( cflag )  THEN
                        DO iksub = 1, kxsub*kysub 
                           IF (  subgrd ( iksub ) .eq. RMISSD) 
     +                         subgrd ( iksub ) = -1.
                        END DO
C
C*                      southern and northern borders
C
                        DO iksub = 1, kxsub 
                           iksubb = iksub + kxsub * ( kysub -1 )
                           subgrd ( iksub )  = -1.
                           subgrd ( iksubb ) = -1.
                        END DO
C
C*                      eastern and western borders
C
                        ifirst = kxsub + 1
                        ilast  = kxsub * ( kysub - 2) + 1
                        DO iksub = ifirst, ilast, kxsub 
                           iksubb = iksub + kxsub - 1
                           subgrd ( iksub )  = -1.
                           subgrd ( iksubb ) = -1.
                        END DO
		      IF  ( lflag )  THEN
...
C
	IF ( iperr .ne. 0 ) CALL ER_WMSG ( 'MKELEV', iperr, ' ', ier )
C
