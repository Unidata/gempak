...
C* GDBIINT								*
C*									*
C* This program interpolates one grid to another			*
C*									*
C**									*
C* Log:									*
C************************************************************************
	INCLUDE		'GEMPRM.PRM'
C*
	CHARACTER*(LLMXLN)	gdfile, gdoutf, gfunc, glev, gvco, 
     +				gdtm, gdnum, path, filnam
C*
...
C
	  CALL GDLINP  ( gdfile, gdoutf, gfunc, glev, gvco, gdtm, 
     +			 gdnum, ier)
C
...
C
	    ...
	    CALL GD_OPNF(filnam,.false.,igdfln,navsz,rnvblk,ianlsz,
     +                   anlblk,ihdrsz,maxgrd,iret)
	    CALL GD_OPNF(gdoutf,.true.,igdfln2,navsz2,rnvblk2,ianlsz2,
     +                   anlblk2,ihdrsz2,maxgrd2,ier)
            iret = iret + ier
	    IF  ( iret .ne. 0 )  THEN
		proces = .false.
	    END IF
C
	    if(proces) then
               CALL GDINTERP(igdfln,navsz,rnvblk,igdfln2,navsz2,
     +                       rnvblk2, gfunc, glev, gvco, gdtm,
     +			     gdnum ) 
	       CALL GD_CLOS(igdfln,iret)
	       CALL GD_CLOS(igdfln2,iret)
	    endif
C
...
C
	IF  (iperr .ne. 0)  CALL ER_WMSG ( 'GDBIINT', iperr, ' ', ier )
C
