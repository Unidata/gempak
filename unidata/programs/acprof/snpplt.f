...
C*
	...
	REAL		filtfc
C*
	REAL		wind (LLMXLV,3), temp (LLMXLV,3)
	REAL		dwpt  (LLMXLV,3), xwind (LLMXLV), ywind (LLMXLV)
	CHARACTER	wt*1
	INTEGER		mrks (15)
	PARAMETER       ( NMRK = 15 )
	DATA            mrks  / 1, 2, 3, 4, 5, 6, 11, 12, 13, 14,
     +                            17, 18, 19, 20, 21 /
C*
C----------------------------------------------------------------------
	iret  = 0
...
C
	IF  ( ( ip1arr (1) .ge. 0 ) .and. ( ntemp .gt. 0 ) )  THEN
C
C*	    Set the color and set the line type.
C
            if(ip1arr(1).gt.0) then
...
C
	    IF  ( mkcolr .gt. 0 )  THEN
c	        CALL GMARK   ( 'M', ntemp, temp (1,2), temp (1,1), ier )
	        CALL GQMRKR  (im1,im2,sm1,im3,ier)
                do i = 1, ntemp
                  imark = mod(NINT(temp(i,3)) - 1,NMRK) + 1
                  CALL GSCOLR (mkcolr + (NINT(temp(i,3))/NMRK), ier)
                  CALL GSMRKR  (mrks(imark),im2,sm1,im3,ier)
	          CALL GMARK   ( 'M', 1, temp (i,2), temp (i,1), ier )
                end do
	    END IF
	END IF
C
