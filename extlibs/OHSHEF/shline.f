...
C           READ(LUNI,'(A)',IOSTAT=LSTAT) LINE(LBEG:MAXEND-1)

            CALL SHN_GLIN  ( LINE(LBEG:MAXEND-1), LSTAT )
...
C             WRITE(LUNC,'(1X,A)',IOSTAT=LSTAT) LINE(LBEG:LEND)

              IF ( ( LEND - LBEG ) .LT. 80 ) THEN
                LLST = LEND 
              ELSE
                LLST = LBEG + 79
              END IF

C             Print bulletin line to decoder log unless the first
C             character is <CTRL-A>

              IF ( MOVA2I( LINE(LBEG:LBEG) ) .NE. 1 ) THEN
                CALL DC_WLOG ( 2, 'DC', 2, LINE(LBEG:LLST), IERWLG )
              END IF
...
C             WRITE(LUNE,'(1X,A,''   (line'',I8,'')'')',IOSTAT=LSTAT)
C    $         LINE(LBEG:LEND),LNUM

              IF ( ( LEND - LBEG ) .LT. 60 ) THEN
                LLST = LEND 
              ELSE
                LLST = LBEG + 59
              END IF
              WRITE(LMSG,'(A,''   (line'',I8,'')'')',IOSTAT=LSTAT)
     $         LINE(LBEG:LLST),LNUM
...
C             WRITE(LUNE,FMAT,IOSTAT=LSTAT)
          ENDIF

      ELSEIF (CMD.EQ.'WRITE_NUM_LINES ') THEN
          IF (LUNE .GE. 0) THEN
C          WRITE(LUNE,'(''    TOTAL NUMBER OF LINES ..'',I8)',
C    $      IOSTAT=LSTAT) LNUM
          ENDIF
