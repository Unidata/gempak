/*
 * EXAMPLE 8: Delayed Replication within data set with a replication count
 * of zero.  Encodes a data set using delayed replication to repeat a data 
 * sequence within the data set as in Example 6, however, a sequence is also
 * included with a replication count of zero.  The FXYs corresponding to the
 * sequence with zero replication are included in Section 3 and the replication
 * count of zero is included in section 4.  This has the effect of skiping over 
 * these descriptors when decoding.  
 *
 * This program is the same as for example 6.
 */
...

{
    ...
    printf(" *      within dataset     *\n");
    printf(" *  replication count of 0 *\n");    
    printf(" *                         * \n"); 
    printf(" *   FXY_Pack_Dec          *\n"); 
    ...
    strcpy(data_file,"exam8.dat");
    strcpy(fxy_file,"exam8.fxy");
    strcpy(encode_file,"ex8.enc");
    /********************************************************/
    /*             read FXYs and data                       */
    /********************************************************/
