/*
 * EXAMPLE 15: This example is similiar to Example 14 except the first approach
 * (PREDEFINED) is used for defining templates. The following paragraphs are 
 * repeated from Example 13.
 * 
 ***********************************
 * Copied form Example 13
 *
 * This example illustrates the use of "templates"  Templates is the
 ...
 *************
 *
 * This example is based on Example 13 and Example 14 and uses the first
 * approach for defining templates.  This approach predefines the FXYs
 * for the data subset.
 *
 * For an example of the second (ON_THE_FLY) approach, see Example 14.
 *   
 */
...
#endif
{
    ...
    strcpy(data_file,"exam15.dat");
    strcpy(fxy_file,"exam15.fxy");
    strcpy(encode_file,"ex15.enc");
    /********************************************************/
    /*             read FXYs and data                       */
    /********************************************************/
    ...
    /* define data subset */
    BUFR_Define_Dataset( fxy,  NUM_FXYS);
    ...
    printf(" data set  %d\n", j+1);
	  ...
	  fscanf(fd, "%s", tstr);
          printf("%s\n",tstr);
	  if (BUFR_Put_Array(tstr, strlen(tstr), DT_CHAR, NULL, 1) ){
	...
	/* Enter array of data into bufr message */
        ...
        if ( BUFR_Put_Array(values, num_num, DT_FLOAT, NULL, num_num) ){
