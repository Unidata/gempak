/*
 * BUFR_Put_S4Data - VERSION: %I%  %E% %T%
 */
/*
 * BUFR_Put_S4Data - Add data to Section 4's bit stream.
...
int BUFR_Put_S4Data( char* Data, int DataBits )
...
#endif
{
    ...
    if( Data == NULL )
    {
        BUFR_Err_Set( "BUFR_Put_S4Data", "NULL pointer for data" );
    ...
    if( DataBits < 1 )
    {
        BUFR_Err_Set( "BUFR_Put_S4Data", "data length < 1" );
        fprintf(BUFR_Cntl.bufr_log,"%s: %s\n", "BUFR_Put_S4Data", 
            "data length < 1" );
        return 1;
    }
...
#if TRACE_PRINT
    if( BUFR_TraceLevel() )
        fprintf(BUFR_Cntl.bufr_log, "Adding Section 4 Data\n" );
#endif

    /* Add each complete byte to the Section 4 bit stream. */
    ...
    for( i=0, dp=(uchar_t*)Data; i < num_bytes; i++, dp++ )
    {
        ...
        if( BitStream_Put( &BUFR_Msg.Section4_Data, ev ) )
        {
            BUFR_Err_Log( "BUFR_Put_S4Data" );
            return 1;
        }
    }
