/*
 * BUFR_Get_S3Data - VERSION: %I%  %E% %T%
 */
/*
 * BUFR_Get_S3Data - Get data from Section 3's bit stream.
...
int BUFR_Get_S3Data( char* Data, int DataBits )
...
#endif
{
    ...
    if( Data == NULL )
    {
        BUFR_Err_Set( "BUFR_Get_S3Data", "NULL pointer for data" );
        fprintf(BUFR_Cntl.bufr_log,"%s\n", 
           "BUFR_Get_S3Data: NULL pointer for data");
        return 0;
    }

    if( DataBits < 1 )
    {
        BUFR_Err_Set( "BUFR_Get_S3Data", "data length < 1" );
        fprintf(BUFR_Cntl.bufr_log,"%s\n", 
           "BUFR_Get_S3Data: data length < 1");
        return 0;
    }

    if( BUFR_ProcType() == TYPE_ENCODE )
    {
        BUFR_Err_Set( "BUFR_Get_S3Data",
            "Decoding function called while encoding data" );
        fprintf(BUFR_Cntl.bufr_log,"%s\n", 
            "BUFR_Get_S3Data: Decoding function called while encoding data");
        return 0;
    }
...
#if TRACE_PRINT
    if( BUFR_TraceLevel() )
        fprintf(BUFR_Cntl.bufr_log, "Geting Section 3 Data\n" );
#endif

    /* Get each complete byte from the Section 3 bit stream. */
    ...
    {
        if( BitStream_Get( &BUFR_Msg.Section3_Data, &ev, BITS_IN_BYTE ) )
        {
            BUFR_Err_Log( "BUFR_Get_S3Data" );
