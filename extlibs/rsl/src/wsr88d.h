...
#define WSR88D_SITE_INFO_FILE "wsr88d_locations.dat"
#define WSR88D_SITE_INFO_DIR "$GEMTBL/stns"
#endif
/*===============================================================*/
typedef struct {
...
typedef struct {
   ...
   /* Table II message header....same as packet type 1 */
   /* halfword 7 : message header information */
   short    packet_msg_size;  /* # halfwords from here to end of record? */
   short    packet_msg_type;  /* Digital Radar Data.  This message may contain
			* a combination of either reflectivity,
			* aliased velocity, or spectrum width.
			*/
   short    packet_id_seq;    /* I.d. Seq = 0 to 7FFF, then roll over to 0 */
   short    packet_msg_date;  /* modified Julian date from 1/1/70 */
   int      packet_msg_time;  /* packet generation time in ms past midnite */
   short    packet_num_seg;
   short    packet_seg_num;
   ...
   unsigned char spare;
   short radial_length;
   unsigned char azm_res_space;
   unsigned char ray_status;
   unsigned char elev_num;
   unsigned char sector_num;
   float elev_angle;
   unsigned char spot_blank;
   unsigned char azm_index_mode;
...
typedef struct {
   int message_type;
   Wsr88d_packet1 *type1;
   Wsr88d_packet31 *type31;
   }
Wsr88d_packet;
...
/* modified for 0.5 degree SuperRes data */
/*#define MAX_RAYS_IN_SWEEP 400*/
#define MAX_RAYS_IN_SWEEP 800
typedef struct {
...
int *wsr88d_get_vcp_info(Wsr88d_ray *ray,int el_num);  /* Courtesy of Dan Austin. */
...
void wsr88d_get_station_id(char *site_id_str, float *mess31_lat, float *mess31_lon, float *mess31_elev);
void wsr88d_set_station_id(char *site_id_str, float lat, float lon, float elev);
