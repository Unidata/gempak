/* kb1gvar.f -- translated by f2c (version 19990503).
...
#define gvarxxgvarkb1_1 gvarxxgvarkb1_

struct {
    integer calflg, calarr[128];
} gvrcal1_;

#define gvrcal_1 gvrcal1_

struct {
    char caltyp[4];
} brkpntgvarkb1_;

#define brkpntgvarkb1_1 brkpntgvarkb1_
...
integer kb1inigvar_(cin, cout, iopt, cin_len, cout_len)
char *cin, *cout;
integer *iopt;
ftnlen cin_len;
ftnlen cout_len;
...
/* ----- MAXAREARQSTLEN - max length of area request string */
    ...
    /* Function Body */
    movw_(&c__5, &iopt[1], gvarxxgvarkb1_1.jopt);
    gvarxxgvarkb1_1.jtype = 0;
    gvrcal_1.calflg = 0;
    gvarxxgvarkb1_1.isou = iopt[1];
    gvarxxgvarkb1_1.ides = iopt[2];
    if (s_cmp(cin, "RAW ", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "RAD ", (
	    ftnlen)4, (ftnlen)4) == 0) {
	gvarxxgvarkb1_1.jtype = 1;
    }
...
integer kb1calgvar_(calb, idir, nval, jband, ibuf)
...
shortint *ibuf;
{
    ...
    /* Local variables */
    extern /* Subroutine */ int chkirgvarkb1_();
    static real gain;
    ...
    extern /* Subroutine */ int movw_();
    static real f;
    extern integer chkcodgvarkb1_();
    static integer i__, iband;
    extern doublereal gvatbbgvarkb1_();
    static real scale;
    static integer intab[256];
    extern /* Subroutine */ int exptabgvarkb1_();
    static integer tblhi, hvcod;
    extern /* Subroutine */ int chkvisgvarkb1_();
    static real vbavg, vgavg;
    static integer tbllo, itemp;
    extern integer grysclgvarkb1_();
    extern doublereal tmpsclgvarkb1_();
...
/* L5: */
	...
	for (i__ = 1; i__ <= 2; ++i__) {
	    if (idir[33] == list[i__ * 3 - 3] && iband == list[i__ * 3 - 1] &&
		     gvarxxgvarkb1_1.jtype == list[i__ * 3 - 2]) {
		indx = i__;
		goto L100;
	    }
/* L10: */
...
L16:
    ...
    list[indx * 3 - 1] = iband;
    list[indx * 3 - 2] = gvarxxgvarkb1_1.jtype;
    brktst = gvarxxgvarkb1_1.jtype == 4 && s_cmp(brkpntgvarkb1_1.caltyp, 
...
/* L36: */
	...
	chkvisgvarkb1_(&isat, iarr, newarr, &hvcod);
	vg2avg = (float)0.;
	vgavg = (float)0.;
	vbavg = (float)0.;
	if (iband == 1) {
...
/* L102: */
	...
	g2term = (float)1.;
	if (gvarxxgvarkb1_1.jtype == 3 && dabs(vg2avg) < (float)1e-4) {
...
/* L38: */
	    ...
	    exptabgvarkb1_(intab, &itab[(indx << 15) - 32768], &c__0);
	    goto L100;
	}
	if (*nval > 100) {
...
/* Computing MIN */
	...
	if (gvarxxgvarkb1_1.jtype == 1 || brktst) {
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (i__ - 1) / scale;
...
/* L40: */
	...
	} else if (gvarxxgvarkb1_1.jtype >= 3) {
	    brktst = gvarxxgvarkb1_1.jtype == 4 && s_cmp(
		    brkpntgvarkb1_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 
		    0;
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (i__ - 1) / scale;
...
/* L45: */
	...
	chkirgvarkb1_(&isat, &iband, iarr, &gain, &bias, &hvcod);
	if (isat % 2 == 0) {
	    scale = (float)16.;
	    igvband = iband + 20;
...
/* UNDEFINED CONFIGURATIO */
	...
	if (gvarxxgvarkb1_1.jtype == 3) {
	    for (i__ = 0; i__ <= 255; ++i__) {
		temp = tmpsclgvarkb1_(&i__);
		rad = gvatbbgvarkb1_(&temp, &igvband, &igvdet, &isat, &c__1);
		raw = rad * gain + bias;
...
/* L801: */
	    ...
	    exptabgvarkb1_(intab, &itab[(indx << 15) - 32768], &c__1);
	    goto L100;
	}
	if (*nval > 100) {
	    rad = gvatbbgvarkb1_(&c_b36, &igvband, &igvdet, &isat, &c__1);
	    raw = rad * gain + bias;
	    r__1 = raw * scale + (float)1.;
	    iraw = i_nint(&r__1);
/* Computing MAX */
	    i__1 = iraw + 1;
	    tbllo = max(i__1,1);
	    rad = gvatbbgvarkb1_(&c_b38, &igvband, &igvdet, &isat, &c__1);
	    raw = rad * gain + bias;
	    r__1 = raw * scale + (float)1.;
	    iraw = i_nint(&r__1);
/* Computing MIN */
	...
	if (gvarxxgvarkb1_1.jtype == 1 || gvarxxgvarkb1_1.jtype == 4 && 
		brktst) {
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (real) i__;
...
/* L6: */
	...
	} else if (gvarxxgvarkb1_1.jtype == 2 || gvarxxgvarkb1_1.jtype == 4) {
	    brktst = gvarxxgvarkb1_1.jtype == 4 && s_cmp(
		    brkpntgvarkb1_1.caltyp, "TEMP", (ftnlen)4, (ftnlen)4) == 
		    0;
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (real) i__;
		temp = (f - (float)1.) / scale;
		rad = (temp - bias) / gain;
		temp = gvatbbgvarkb1_(&rad, &igvband, &igvdet, &isat, &c__0);
		if (brktst) {
...
/* L7: */
	...
	} else if (gvarxxgvarkb1_1.jtype > 2) {
	    brktst = gvarxxgvarkb1_1.jtype == 4 && s_cmp(
		    brkpntgvarkb1_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 
		    0;
	    i__1 = tblhi;
...
L100:
    mgvatb_(nval, &gvarxxgvarkb1_1.isou, &gvarxxgvarkb1_1.ides, &ibuf[1], &
	    itab[(indx << 15) - 32768]);
    ret_val = 0;
    return ret_val;
} /* kb1calgvar_ */

integer kb1optgvar_(cfunc, iin, iout, cfunc_len)
char *cfunc;
integer *iin, *iout;
ftnlen cfunc_len;
{
...
/* ----- MAXAREARQSTLEN - max length of area request string */
	...
	if (iband == 1 && isat % 2 == 0 || iband == 19 && isat % 2 == 1) {
		...
		if (brkset_(cfile, brkpntgvarkb1_1.caltyp, (ftnlen)12, (
			ftnlen)4) != 0) {
		    ret_val = -3;
		    return ret_val;
		}
	...
	if (brkset_(cfile, brkpntgvarkb1_1.caltyp, (ftnlen)12, (ftnlen)4) != 
		0) {
	    ret_val = -3;
	    return ret_val;
	}
...
doublereal gvatbbgvarkb1_(val, kch, kdet, isat, switch__)
real *val;
...
doublereal tmpsclgvarkb1_(ibrit)
integer *ibrit;
{
    /* Initialized data */
...
/* Subroutine */ int chkvisgvarkb1_(isat, codarr, visarr, hvcod)
integer *isat, *codarr;
real *visarr;
integer *hvcod;
{
    ...
    extern doublereal pctdifgvarkb1_();
    static integer itemp, codpos, newpos;
    extern doublereal frmtgvarkb1_();
    ...
    if (*isat % 2 == 0) {
	for (itemp = 1; itemp <= 8; ++itemp) {
	    r__1 = frmtgvarkb1_(&codarr[codpos]);
	    if (pctdifgvarkb1_(&ivbias[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L10: */
	...
	for (itemp = 1; itemp <= 8; ++itemp) {
	    r__1 = frmtgvarkb1_(&codarr[codpos]);
	    if (pctdifgvarkb1_(&ivgain[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L20: */
	...
	for (itemp = 1; itemp <= 8; ++itemp) {
	    r__1 = frmtgvarkb1_(&codarr[codpos]);
	    if (pctdifgvarkb1_(&iv2gain[itemp - 1], &r__1) > mxdf && *hvcod ==
		     1) {
		visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L30: */
	...
	r__1 = frmtgvarkb1_(&codarr[codpos]);
	if (pctdifgvarkb1_(&ivalb, &r__1) > mxdf && *hvcod == 1) {
	    visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
	    iflag = 1;
	} else {
	    visarr[newpos] = ivalb;
	...
	for (itemp = 1; itemp <= 4; ++itemp) {
	    r__1 = frmtgvarkb1_(&codarr[codpos]);
	    if (pctdifgvarkb1_(&svbias[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L40: */
	...
	for (itemp = 1; itemp <= 4; ++itemp) {
	    r__1 = frmtgvarkb1_(&codarr[codpos]);
	    if (pctdifgvarkb1_(&svgain[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L50: */
	...
	for (itemp = 1; itemp <= 4; ++itemp) {
	    r__1 = frmtgvarkb1_(&codarr[codpos]);
	    if (pctdifgvarkb1_(&sv2gain[itemp - 1], &r__1) > mxdf && *hvcod ==
		     1) {
		visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L60: */
	...
	r__1 = frmtgvarkb1_(&codarr[codpos]);
	if (pctdifgvarkb1_(&svalb, &r__1) > mxdf && *hvcod == 1) {
	    visarr[newpos] = frmtgvarkb1_(&codarr[codpos]);
	    ++iflag;
	} else {
	    visarr[newpos] = svalb;
...
/* Subroutine */ int chkirgvarkb1_(isat, iband, codarr, gain, bias, hvcod)
...
integer *hvcod;
{
    ...
    /* Local variables */
    static integer iflag;
    extern doublereal pctdifgvarkb1_(), frmtgvarkb1_();
	...
	if (*iband == 2) {
	    *bias = iirbias[2];
	    *gain = iirgain[2];
	    r__1 = frmtgvarkb1_(&codarr[ibiasoff + loc2]);
	    if (pctdifgvarkb1_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb1_(&codarr[ibiasoff + loc2]);
	    }
	    r__1 = frmtgvarkb1_(&codarr[igainoff + loc2]);
	    if (pctdifgvarkb1_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb1_(&codarr[igainoff + loc2]);
	    }
	}
	if (*iband == 3) {
	    *bias = iirbias[3];
	    *gain = iirgain[3];
	    r__1 = frmtgvarkb1_(&codarr[ibiasoff + loc3]);
	    if (pctdifgvarkb1_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb1_(&codarr[ibiasoff + loc3]);
	    }
	    r__1 = frmtgvarkb1_(&codarr[igainoff + loc3]);
	    if (pctdifgvarkb1_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb1_(&codarr[igainoff + loc3]);
	    }
	}
	if (*iband == 4) {
	    *bias = iirbias[0];
	    *gain = iirgain[0];
	    r__1 = frmtgvarkb1_(&codarr[ibiasoff + loc4]);
	    if (pctdifgvarkb1_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb1_(&codarr[ibiasoff + loc4]);
	    }
	    r__1 = frmtgvarkb1_(&codarr[igainoff + loc4]);
	    if (pctdifgvarkb1_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb1_(&codarr[igainoff + loc4]);
	    }
	}
	if (*iband == 5) {
	    *bias = iirbias[1];
	    *gain = iirgain[1];
	    r__1 = frmtgvarkb1_(&codarr[ibiasoff + loc5]);
	    if (pctdifgvarkb1_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb1_(&codarr[ibiasoff + loc5]);
	    }
	    r__1 = frmtgvarkb1_(&codarr[igainoff + loc5]);
	    if (pctdifgvarkb1_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb1_(&codarr[igainoff + loc5]);
	    }
	}
    }
    if (*isat % 2 == 1) {
	r__1 = frmtgvarkb1_(&codarr[sgainoff + *iband]);
	if (pctdifgvarkb1_(&sirgain[*iband - 1], &r__1) > mxdf && *hvcod == 1)
		 {
	    iflag = 1;
	    *gain = frmtgvarkb1_(&codarr[sgainoff + *iband]);
	} else {
	    *gain = sirgain[*iband - 1];
	}
	r__1 = frmtgvarkb1_(&codarr[sbiasoff + *iband]);
	if (pctdifgvarkb1_(&sirbias[*iband - 1], &r__1) > mxdf && *hvcod == 1)
		 {
	    iflag = 1;
	    *bias = frmtgvarkb1_(&codarr[sbiasoff + *iband]);
	} else {
...
/* Subroutine */ int dmparrgvarkb1_(array, start, end)
real *array;
integer *start, *end;
{
    /* Parameter adjustments */
...
doublereal pctdifgvarkb1_(value1, value2)
real *value1, *value2;
{
    /* System generated locals */
    real ret_val, r__1;
...
/* Subroutine */ int exptabgvarkb1_(intab, itab, switch__)
