/* kb1visr.f -- translated by f2c (version 19990503).
...
#define visrxxvisrkb1_1 visrxxvisrkb1_

struct {
    char caltyp[4];
} brkpntvisrkb1_;

#define brkpntvisrkb1_1 brkpntvisrkb1_
...
integer kb1inivisr_(cin, cout, iopt, cin_len, cout_len)
char *cin, *cout;
integer *iopt;
ftnlen cin_len;
ftnlen cout_len;
...
/* local variables */
    ...
    /* Function Body */
    movw_(&c__5, &iopt[1], visrxxvisrkb1_1.jopt);
    visrxxvisrkb1_1.jtype = 0;
    visrxxvisrkb1_1.isou = iopt[1];
    visrxxvisrkb1_1.ides = iopt[2];
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "TEMP", (
	    ftnlen)4, (ftnlen)4) == 0) {
	visrxxvisrkb1_1.jtype = 1;
    }
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "MODB", (
	    ftnlen)4, (ftnlen)4) == 0) {
	visrxxvisrkb1_1.jtype = 2;
    }
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "RAW ", (
	    ftnlen)4, (ftnlen)4) == 0) {
	visrxxvisrkb1_1.jtype = 3;
    }
    if (visrxxvisrkb1_1.jtype == 0) {
	goto L900;
    }
    ret_val = 0;
    return ret_val;
...
integer kb1calvisr_(calb, idir, nval, iband, ibuf)
integer *calb, *idir, *nval, *iband, *ibuf;
{
    ...
    extern /* Subroutine */ int temptbvisrkb1_();
    extern integer brkval_();
    /* extern / Subroutine / int mpixel_(), mpixtb_(); */
    extern /* Subroutine */ void mpixel (), mpixtb_();
...
/* initialized variables */
	...
	if ((s_cmp(brkpntvisrkb1_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 0 
		|| s_cmp(brkpntvisrkb1_1.caltyp, "RAW ", (ftnlen)4, (ftnlen)4)
		 == 0) && visrxxvisrkb1_1.jtype == 2) {
	    if (lasday != idir4 || lastyp != visrxxvisrkb1_1.jtype) {
...
/* L100: */
		...
		lasday = idir4;
		lastyp = visrxxvisrkb1_1.jtype;
	    }
	    mpixtb_(nval, &visrxxvisrkb1_1.isou, &visrxxvisrkb1_1.ides, &ibuf[
		    1], itab);
	    return ret_val;
	} else {
	    mpixel (nval, &visrxxvisrkb1_1.isou, &visrxxvisrkb1_1.ides, &ibuf[
		    1]);
	    return ret_val;
	}
    }
    iday = idir4;
    if (lasday != idir4 || lastyp != visrxxvisrkb1_1.jtype) {
	if (visrxxvisrkb1_1.jtype == 1) {
	    temptbvisrkb1_(&iday, itab);
	} else if (visrxxvisrkb1_1.jtype == 3) {
	    for (i__ = 0; i__ <= 255; ++i__) {
/* L105: */
		itab[i__] = i__;
	    }
	} else if (s_cmp(brkpntvisrkb1_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4)
		 == 0 || s_cmp(brkpntvisrkb1_1.caltyp, "RAW", (ftnlen)4, (
		ftnlen)3) == 0) {
	    for (i__ = 0; i__ <= 255; ++i__) {
/* L110: */
		r__1 = (real) i__;
	...
	} else if (s_cmp(brkpntvisrkb1_1.caltyp, "TEMP", (ftnlen)4, (ftnlen)4)
		 == 0) {
	    temptbvisrkb1_(&iday, itab);
	    for (i__ = 0; i__ <= 255; ++i__) {
...
/* L200: */
    ...
    mpixtb_(nval, &visrxxvisrkb1_1.isou, &visrxxvisrkb1_1.ides, &ibuf[1], 
	    itab);
    ret_val = 0;
    return ret_val;
} /* kb1calvisr_ */

integer kb1optvisr_(cfunc, iin, iout, cfunc_len)
char *cfunc;
integer *iin, *iout;
ftnlen cfunc_len;
{
...
/* local variables */
	...
	if (ischar_(&iin[38]) == 1) {
	    ...
	    if (brkset_(cfile, brkpntvisrkb1_1.caltyp, (ftnlen)8, (ftnlen)4) 
		    != 0) {
		ret_val = -3;
		return ret_val;
	    }
	...
	if (brkset_(cfile, brkpntvisrkb1_1.caltyp, (ftnlen)8, (ftnlen)4) != 0)
		 {
	    ret_val = -3;
	    return ret_val;
	}
...
/* Subroutine */ int temptbvisrkb1_(iday, item)
integer *iday, *item;
{
    /* Initialized data */
