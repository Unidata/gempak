/* kb3visr.f -- translated by f2c (version 19990503).
...
#define visrxxvisrkb3_1 visrxxvisrkb3_

struct {
    char caltyp[4];
} brkpntvisrkb3_;

#define brkpntvisrkb3_1 brkpntvisrkb3_
...
integer kb3inivisr_(cin, cout, iopt, cin_len, cout_len)
char *cin, *cout;
integer *iopt;
ftnlen cin_len;
ftnlen cout_len;
...
/* local variables */
    ...
    /* Function Body */
    movw_(&c__5, &iopt[1], visrxxvisrkb3_1.jopt);
    visrxxvisrkb3_1.jtype = 0;
    visrxxvisrkb3_1.isou = iopt[1];
    visrxxvisrkb3_1.ides = iopt[2];
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "TEMP", (
	    ftnlen)4, (ftnlen)4) == 0) {
	visrxxvisrkb3_1.jtype = 1;
    }
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "MODB", (
	    ftnlen)4, (ftnlen)4) == 0) {
	visrxxvisrkb3_1.jtype = 2;
    }
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "RAW ", (
	    ftnlen)4, (ftnlen)4) == 0) {
	visrxxvisrkb3_1.jtype = 3;
    }
    if (visrxxvisrkb3_1.jtype == 0) {
	goto L900;
    }
    ret_val = 0;
    return ret_val;
...
integer kb3calvisr_(calb, idir, nval, iband, ibuf)
integer *calb, *idir, *nval, *iband, *ibuf;
{
    ...
    extern /* Subroutine */ int temptbvisrkb3_();
...
/* initialized variables */
	...
	if ((s_cmp(brkpntvisrkb3_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 0 
		|| s_cmp(brkpntvisrkb3_1.caltyp, "RAW ", (ftnlen)4, (ftnlen)4)
		 == 0) && visrxxvisrkb3_1.jtype == 2) {
	    if (lasday != idir4 || lastyp != visrxxvisrkb3_1.jtype) {
...
/* L100: */
		...
		lasday = idir4;
		lastyp = visrxxvisrkb3_1.jtype;
	    }
	    mpixtb_(nval, &visrxxvisrkb3_1.isou, &visrxxvisrkb3_1.ides, &ibuf[
		    1], itab);
	    return ret_val;
	} else {
	    mpixel (nval, &visrxxvisrkb3_1.isou, &visrxxvisrkb3_1.ides, &ibuf[
		    1]);
	    return ret_val;
	}
    }
    iday = idir4;
    if (lasday != idir4 || lastyp != visrxxvisrkb3_1.jtype) {
	if (visrxxvisrkb3_1.jtype == 1) {
	    temptbvisrkb3_(&iday, itab);
	} else if (visrxxvisrkb3_1.jtype == 3) {
	    for (i__ = 0; i__ <= 255; ++i__) {
/* L105: */
		itab[i__] = i__;
	    }
	} else if (s_cmp(brkpntvisrkb3_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4)
		 == 0 || s_cmp(brkpntvisrkb3_1.caltyp, "RAW", (ftnlen)4, (
		ftnlen)3) == 0) {
	    for (i__ = 0; i__ <= 255; ++i__) {
/* L110: */
		r__1 = (real) i__;
	...
	} else if (s_cmp(brkpntvisrkb3_1.caltyp, "TEMP", (ftnlen)4, (ftnlen)4)
		 == 0) {
	    temptbvisrkb3_(&iday, itab);
	    for (i__ = 0; i__ <= 255; ++i__) {
...
/* L200: */
    ...
    mpixtb_(nval, &visrxxvisrkb3_1.isou, &visrxxvisrkb3_1.ides, &ibuf[1], 
	    itab);
    ret_val = 0;
    return ret_val;
} /* kb3calvisr_ */

integer kb3optvisr_(cfunc, iin, iout, cfunc_len)
char *cfunc;
integer *iin, *iout;
ftnlen cfunc_len;
{
...
/* local variables */
	...
	if (ischar_(&iin[38]) == 1) {
	    ...
	    if (brkset_(cfile, brkpntvisrkb3_1.caltyp, (ftnlen)8, (ftnlen)4) 
		    != 0) {
		ret_val = -3;
		return ret_val;
	    }
	...
	if (brkset_(cfile, brkpntvisrkb3_1.caltyp, (ftnlen)8, (ftnlen)4) != 0)
		 {
	    ret_val = -3;
	    return ret_val;
	}
...
/* Subroutine */ int temptbvisrkb3_(iday, item)
integer *iday, *item;
{
    /* Initialized data */
