/* kb1gms.f -- translated by f2c (version 19990503).
...
#define gmsxxgmskb1_1 gmsxxgmskb1_
...
#define debuggmskb1_1 (debuggmskb1_._1)
#define debuggmskb1_2 (debuggmskb1_._2)

struct {
    char caltyp[4];
} brkpntgmskb1_;

#define brkpntgmskb1_1 brkpntgmskb1_

struct {
    integer ktab[256];
} radgmskb1_;

#define radgmskb1_1 radgmskb1_
...
#define gmspfcgmskb1_1 (gmspfcgmskb1_._1)
#define gmspfcgmskb1_2 (gmspfcgmskb1_._2)
...
integer kb1inigms_(char *cin, char *cout, integer *iopt, ftnlen cin_len, 
	ftnlen cout_len)
{
...
/*  Error flag for byte size, conver */
    ...
    /* Function Body */
    movw_(&c__5, &iopt[1], gmsxxgmskb1_1.jopt);
    ierror = 0;
    gmsxxgmskb1_1.jtype = 0;
    gmsxxgmskb1_1.isou = iopt[1];
    gmsxxgmskb1_1.ides = iopt[2];
    if (gmsxxgmskb1_1.isou != 1 || gmsxxgmskb1_1.ides != 1 && 
	    gmsxxgmskb1_1.ides != 2 && gmsxxgmskb1_1.ides != 4) {
	ierror = 1;
	edestX_("KBX_INI: Invalid byte sizes specified.", &c__0, (ftnlen)38);
	edestX_("Source (must be 1)=", &gmsxxgmskb1_1.isou, (ftnlen)19);
	edestX_("Destination (must be 1,2, or 4)=", &gmsxxgmskb1_1.ides, (
		ftnlen)32);
    }
    if ((s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 || s_cmp(cin, "RAW", (
	    ftnlen)4, (ftnlen)3) == 0) && s_cmp(cout, "TEMP", (ftnlen)4, (
	    ftnlen)4) == 0) {
	gmsxxgmskb1_1.jtype = 1;
    }
    if ((s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 || s_cmp(cin, "RAW", (
	    ftnlen)4, (ftnlen)3) == 0) && s_cmp(cout, "ALB", (ftnlen)4, (
	    ftnlen)3) == 0) {
	gmsxxgmskb1_1.jtype = 2;
    }
    if ((s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 || s_cmp(cin, "RAW", (
	    ftnlen)4, (ftnlen)3) == 0) && s_cmp(cout, "RAD", (ftnlen)4, (
	    ftnlen)3) == 0) {
	gmsxxgmskb1_1.jtype = 3;
    }
    if (s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "MODB", (
	    ftnlen)4, (ftnlen)4) == 0) {
	gmsxxgmskb1_1.jtype = 4;
    }
    if ((s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 || s_cmp(cin, "RAW", (
	    ftnlen)4, (ftnlen)3) == 0) && s_cmp(cout, "RAW", (ftnlen)4, (
	    ftnlen)3) == 0) {
	gmsxxgmskb1_1.jtype = 5;
    }
    if ((s_cmp(cin, "BRIT", (ftnlen)4, (ftnlen)4) == 0 || s_cmp(cin, "RAW", (
	    ftnlen)4, (ftnlen)3) == 0) && s_cmp(cout, "BRIT", (ftnlen)4, (
	    ftnlen)4) == 0) {
	gmsxxgmskb1_1.jtype = 6;
    }
    if (s_cmp(cin, "RAW", (ftnlen)4, (ftnlen)3) == 0 && s_cmp(cout, "MODB", (
	    ftnlen)4, (ftnlen)4) == 0) {
	gmsxxgmskb1_1.jtype = 7;
    }
    if (gmsxxgmskb1_1.jtype == 0) {
	edestX_("KBX_CAL data conversion unrecognized:", &c__0, (ftnlen)37);
/* Writing concatenation */
	i__1[0] = 24, a__1[0] = "          Input  type-->";
	i__1[1] = 4, a__1[1] = cin;
	i__1[2] = 3, a__1[2] = "<--";
	s_cat(ch__1, a__1, i__1, &c__3, (ftnlen)31);
	edestX_(ch__1, &c__0, (ftnlen)31);
/* Writing concatenation */
	...
	edestX_("KBX_INI error, LW file GMSCAL not found", &c__0, (ftnlen)39);
	ierror = 1;
    }
...
integer kb1calgms_(integer *calb, integer *idir, integer *nval, integer *
	iband, integer *ibuf)
{
    /* Initialized data */
    ...
    extern integer rdcalgmskb1_(integer *, integer *, integer *, integer *);
#define itab (equiv_1)
    extern real vplancgmskb1_(real *, integer *, integer *, integer *);
    static real xalb, xrad;
    static integer ichan, irtab;
    extern /* Subroutine */ int edestX_(char *, integer *, ftnlen);
#define itabv (equiv_1)
#define jtabv ((integer *)&debuggmskb1_2 + 2049)
...
/*  IBAND for which tables are curre */
    ...
    if (s_cmp(brkpntgmskb1_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 0 && 
	    gmsxxgmskb1_1.jtype == 4 && lastyp != gmsxxgmskb1_1.jtype || 
	    s_cmp(brkpntgmskb1_1.caltyp, "RAW", (ftnlen)4, (ftnlen)3) == 0 && 
	    gmsxxgmskb1_1.jtype == 7 && lastyp != gmsxxgmskb1_1.jtype) {
	for (i__ = 1; i__ <= 256; ++i__) {
	    r__1 = (real) (i__ - 1);
	    debuggmskb1_2.jtab[i__ - 1] = brkval_(&r__1);
	}
	lastyp = gmsxxgmskb1_1.jtype;
	ret_val = 0;
    } else if (gmsxxgmskb1_1.jtype == 5 && lastyp != gmsxxgmskb1_1.jtype) {
	for (i__ = 1; i__ <= 256; ++i__) {
	    debuggmskb1_2.jtab[i__ - 1] = i__ - 1;
	}
	lastyp = gmsxxgmskb1_1.jtype;
	ret_val = 0;
    } else if (lastyp != gmsxxgmskb1_1.jtype || laschan != ichan || lasband !=
	     *iband) {
	if (ichan == 0 && (ivflg == 0 || lastyp != gmsxxgmskb1_1.jtype || 
		lasband != *iband)) {
	    laschan = ichan;
	    lasband = *iband;
	    if (rdcalgmskb1_(&calb[1], &idir[1], iband, itab) != 0) {
		edestX_("KBX_CAL: RD_CAL call failed.", &c__0, (ftnlen)28);
		ret_val = -1;
	    }
	    ivflg = 1;
	    if (gmsxxgmskb1_1.jtype == 2 || gmsxxgmskb1_1.jtype == 4 || 
		    gmsxxgmskb1_1.jtype == 7) {
		for (i__ = 1; i__ <= 1024; ++i__) {
		    xalb = itab[i__ - 1] / 1e3f;
		    debuggmskb1_2.xtab[i__ - 1] = xalb;
		    if (s_cmp(brkpntgmskb1_1.caltyp, "ALB", (ftnlen)4, (
			    ftnlen)3) == 0 && gmsxxgmskb1_1.jtype != 2) {
/* MODB */
			r__1 = (xalb + .5f) / 10.f;
			debuggmskb1_2.jtab[i__ - 1] = brkval_(&r__1);
		    } else if (gmsxxgmskb1_1.jtype == 2) {
/* ALB */
			debuggmskb1_2.jtab[i__ - 1] = (integer) (xalb + .5f);
		    }
		}
		lastyp = gmsxxgmskb1_1.jtype;
	    }
	    if (gmsxxgmskb1_1.jtype == 6 || gmsxxgmskb1_1.jtype == 7) {
		if (s_cmp(brkpntgmskb1_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4)
			 == 0 && gmsxxgmskb1_1.jtype != 6) {
/* MODB */
		    for (i__ = 1; i__ <= 256; ++i__) {
			r__1 = (real) itab[i__ - 1] / 4e3f;
			k = brkval_(&r__1);
			debuggmskb1_2.jtab[i__ - 1] = k;
			debuggmskb1_2.jtab[i__ + 255] = k;
			debuggmskb1_2.jtab[i__ + 511] = k;
			debuggmskb1_2.jtab[i__ + 767] = k;
		    }
		} else if (gmsxxgmskb1_1.jtype == 6) {
/* BRIT */
		    for (i__ = 1; i__ <= 1024; ++i__) {
			debuggmskb1_2.jtab[i__ - 1] = itab[i__ - 1] / 4e3f;
			debuggmskb1_2.jtab[i__ - 1] = debuggmskb1_2.jtab[i__ 
	...
	} else if (ichan == 1 && lastyp != gmsxxgmskb1_1.jtype || lasband != *
		iband) {
	    laschan = ichan;
	    lasband = *iband;
	    if (rdcalgmskb1_(&calb[1], &idir[1], iband, itab) != 0) {
	    ...
	    ivflg = 1;
	    if (gmsxxgmskb1_1.jtype == 1 || gmsxxgmskb1_1.jtype == 4 || 
		    gmsxxgmskb1_1.jtype == 7) {
		for (i__ = 1; i__ <= 256; ++i__) {
		    if (s_cmp(brkpntgmskb1_1.caltyp, "TEMP", (ftnlen)4, (
			    ftnlen)4) == 0 && gmsxxgmskb1_1.jtype != 1) {
/* MODB */
			r__1 = itab[i__ - 1] / 1e3f;
			debuggmskb1_2.jtab[i__ - 1] = brkval_(&r__1);
		    } else if (gmsxxgmskb1_1.jtype == 1) {
/* TEMP */
			debuggmskb1_2.jtab[i__ - 1] = (integer) (itab[i__ - 1]
				 / 100.f + .5f);
		    }
		    xtemp = (real) itab[i__ - 1] / 1e3f;
		    debuggmskb1_2.xtab[i__ - 1] = xtemp;
		}
		ivisn = 1;
	    }
	    if (gmsxxgmskb1_1.jtype == 3 || gmsxxgmskb1_1.jtype == 4 || 
		    gmsxxgmskb1_1.jtype == 7) {
		for (i__ = 1; i__ <= 256; ++i__) {
		    if (s_cmp(brkpntgmskb1_1.caltyp, "RAD ", (ftnlen)4, (
			    ftnlen)4) == 0 && gmsxxgmskb1_1.jtype != 3) {
/* MODB */
			r__2 = itab[i__ - 1] / 1e3f;
			r__1 = vplancgmskb1_(&r__2, &i__, &idir[3], iband) * 
				10;
			debuggmskb1_2.jtab[i__ - 1] = brkval_(&r__1);
		    } else if (gmsxxgmskb1_1.jtype == 3) {
/* RAD */
			r__2 = itab[i__ - 1] / 1e3f;
			r__1 = vplancgmskb1_(&r__2, &i__, &idir[3], iband) * 
				100.f;
			debuggmskb1_2.jtab[i__ - 1] = i_nint(&r__1);
		    }
		    xtemp = (real) itab[i__ - 1] / 1e3f;
		    xrad = vplancgmskb1_(&xtemp, &i__, &idir[3], iband);
		    debuggmskb1_2.xtab[i__ - 1] = xrad;
		}
		ivisn = 1;
	    }
	    if (gmsxxgmskb1_1.jtype == 6 || gmsxxgmskb1_1.jtype == 7) {
		...
		for (i__ = 1; i__ <= 256; ++i__) {
		    xtemp = (real) itab[i__ - 1] / 1e3f;
		    gryscl_(&xtemp, &ibrit);
		    debuggmskb1_2.xtab[i__ - 1] = (real) ibrit;
		    if (s_cmp(brkpntgmskb1_1.caltyp, "BRIT", (ftnlen)4, (
			    ftnlen)4) == 0 && gmsxxgmskb1_1.jtype != 6) {
/* MODB */
			r__1 = (real) ibrit;
			debuggmskb1_2.jtab[i__ - 1] = brkval_(&r__1);
		    } else if (gmsxxgmskb1_1.jtype == 6) {
/* BRIT */
			debuggmskb1_2.jtab[i__ - 1] = ibrit;
		    }
		}
		ivisn = 1;
	    }
    ...
    mpixtb_(nval, &gmsxxgmskb1_1.isou, &gmsxxgmskb1_1.ides, &ibuf[1], &jtabv[(
	    ivisn << 8) - 256]);
    return ret_val;
} /* kb1calgms_ */
...
integer kb1optgms_(char *cfunc, integer *iin, integer *iout, ftnlen cfunc_len)
{
    /* System generated locals */
    address a__1[3];
    integer ret_val, i__1[3];
...
/*  Validity test variable */
	...
	if (ischar_(&iin[38]) == 1) {
	    ...
	    if (brkset_(cfile, brkpntgmskb1_1.caltyp, (ftnlen)8, (ftnlen)4) !=
		     0) {
		ret_val = -3;
	    }
	}
	...
	ret_val = 0;
	if (brkset_(cfile, brkpntgmskb1_1.caltyp, (ftnlen)8, (ftnlen)4) != 0) 
		{
	    ret_val = -3;
	}
...
real vplancgmskb1_(real *t, integer *index, integer *iss, integer *iband)
{
    /* Initialized data */
...
/* Radiance table for GMS-5 from RD_CAL */
	...
	ret_val = radgmskb1_1.ktab[*index - 1] / 1e3f;
    } else {
	edestX_("V_PLANC:  Unrecognized SS or BAND", &c__0, (ftnlen)33);
	edestX_("              ISS=", iss, (ftnlen)18);
	edestX_("            IBAND=", iband, (ftnlen)18);
	ret_val = 0.f;
    }
    return ret_val;
} /* vplancgmskb1_ */

real gmtradgmskb1_(real *t, integer *iband)
{
    /* System generated locals */
    real ret_val;
...
/*  IR channel array index */
    k = *iband - 1;
    tt = gmspfcgmskb1_1.tc[(k << 1) - 2] + gmspfcgmskb1_1.tc[(k << 1) - 1] * *
	    t;
    expn = exp(gmspfcgmskb1_1.fk2[k - 1] / tt) - 1.f;
    ret_val = gmspfcgmskb1_1.fk1[k - 1] / expn;
    return ret_val;
} /* gmtradgmskb1_ */

/* Subroutine */ int gmpfcogmskb1_(char *cdet, integer *iok, ftnlen cdet_len)
{
    /* Initialized data */
...
/*  Index */
    *iok = 1;
	...
	for (k = 1; k <= 3; ++k) {
	    gmspfcgmskb1_2.cwn[k - 1] = awn[k - 1];
	    gmspfcgmskb1_2.fk1[k - 1] = ak1[k - 1];
	    gmspfcgmskb1_2.fk2[k - 1] = ak2[k - 1];
	    for (j = 1; j <= 2; ++j) {
		gmspfcgmskb1_2.tcc[j + (k << 1) - 3] = acc[j + (k << 1) - 3];
	    }
	}
    } else if (*(unsigned char *)cdet == *(unsigned char *)&cb[0]) {
	for (k = 1; k <= 3; ++k) {
	    gmspfcgmskb1_2.cwn[k - 1] = bwn[k - 1];
	    gmspfcgmskb1_2.fk1[k - 1] = bk1[k - 1];
	    gmspfcgmskb1_2.fk2[k - 1] = bk2[k - 1];
	    for (j = 1; j <= 2; ++j) {
		gmspfcgmskb1_2.tcc[j + (k << 1) - 3] = bcc[j + (k << 1) - 3];
	    }
	}
    } else {
	*iok = 0;
...
integer rdcalgmskb1_(integer *calb, integer *idir, integer *iband, integer *
	itab)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int gmpfcogmskb1_(char *, integer *, ftnlen);
    static integer ltab, ioff;
    extern real gmtradgmskb1_(real *, integer *);
    static integer kend, nlen;
...
/*  Table Name in calibration data b */
    ...
    if (gmsxxgmskb1_1.kopt <= 0 || gmsxxgmskb1_1.kopt > 15) {
	gmsxxgmskb1_1.kopt = ksys_(&c__151);
    }
    if (gmsxxgmskb1_1.kopt <= 0 || gmsxxgmskb1_1.kopt > 15) {
	gmsxxgmskb1_1.kopt = 7;
	if (idir[3] <= 82) {
	    gmsxxgmskb1_1.kopt = 3;
	}
    }
    if (idir[3] <= 82 && gmsxxgmskb1_1.kopt > 3) {
	edestX_("Invalid calibration option for SS=", &idir[3], (ftnlen)34);
	edestX_("     ...was ", &gmsxxgmskb1_1.kopt, (ftnlen)12);
	gmsxxgmskb1_1.kopt = 3;
	edestX_("     ...reset to ", &gmsxxgmskb1_1.kopt, (ftnlen)17);
	edestX_("Options > 3 valid for GMS-5 and later only!", &c__0, (ftnlen)
		43);
    }
    debuggmskb1_2.ival = 0;
    idsen = 0;
    if ((gmsxxgmskb1_1.kopt & 12) != 0) {
...
/*  convert to mW/etc./cm */
		    radgmskb1_1.ktab[j - 1] = (integer) (r__ * 1e3f);
/*     the KTAB table as */
/*  scale by 1000 and put */
		}
		iok = 0;
		for (j = 2; j <= 255; ++j) {
		    if (radgmskb1_1.ktab[j - 1] < 0 || radgmskb1_1.ktab[j - 1]
			     > 190000) {
			++iok;
		    }
		}
...
L80:
	debuggmskb1_2.ival = 8;
	if (iok > 5 && (gmsxxgmskb1_1.kopt & 15) == 8) {
...
L100:
	...
	if (iss == 83) {
	    if (idsen != 2) {
		gmpfcogmskb1_("A", &jok, (ftnlen)1);
	    }
	    if (idsen == 2) {
		gmpfcogmskb1_("B", &jok, (ftnlen)1);
	    }
	    if (jok == 0) {
		goto L300;
	    }
	    for (j = 1; j <= 256; ++j) {
		r__1 = itab[j] * .001f;
		radgmskb1_1.ktab[j - 1] = gmtradgmskb1_(&r__1, iband) * 1e3f;
	    }
	} else {
	    goto L200;
	}
	debuggmskb1_2.ival = 4;
	goto L400;
    }
L200:
    if ((gmsxxgmskb1_1.kopt & 2) != 0) {
	s_copy(cfile, "GMSCALU", (ftnlen)12, (ftnlen)7);
	istat = lwix_(cfile, &c__0, &c__512, dir, (ftnlen)12);
	swbyt4_(dir, &c__512);
	if (istat != 0 && (gmsxxgmskb1_1.kopt & 1) == 0) {
/* Writing concatenation */
	...
	debuggmskb1_2.ival = 2;
	goto L400;
    }
L300:
    if ((gmsxxgmskb1_1.kopt & 1) != 0) {
...
/* Writing concatenation */
	...
	debuggmskb1_2.ival = 1;
	goto L400;
    }
    ret_val = -1;
    goto L500;
L400:
    if (debuggmskb1_2.ival == 0) {
	ret_val = -1;
    } else {
	ret_val = 0;
    }
L500:
    sysin_(&c__152, &debuggmskb1_2.ival);
    for (i__ = 1; i__ <= 1024; ++i__) {
	debuggmskb1_2.itabr[i__ - 1] = itab[i__];
    }
    return ret_val;
} /* rdcalgmskb1_ */
