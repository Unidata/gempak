/* kb3gms.f -- translated by f2c (version 19990503).
...
#define gmsxxgmskb3_1 gmsxxgmskb3_
...
#define debuggmskb3_1 (debuggmskb3_._1)
#define debuggmskb3_2 (debuggmskb3_._2)

struct {
    char caltyp[4];
} brkpntgmskb3_;

#define brkpntgmskb3_1 brkpntgmskb3_

struct {
    integer ktab[256];
} radgmskb3_;

#define radgmskb3_1 radgmskb3_
...
#define gmspfcgmskb3_1 (gmspfcgmskb3_._1)
#define gmspfcgmskb3_2 (gmspfcgmskb3_._2)
...
integer kb3inigms_(char *cin, char *cout, integer *iopt, ftnlen cin_len, 
	ftnlen cout_len)
{
...
real vplancgmskb3_(real *t, integer *index, integer *iss, integer *iband)
{
    /* Initialized data */
...
/* Radiance table for GMS-5 from RD_CAL */
	...
	ret_val = radgmskb3_1.ktab[*index - 1] / 1e3f;
    } else {
...
real gmtradgmskb3_(real *t, integer *iband)
{
    /* System generated locals */
    real ret_val;
...
/*  IR channel array index */
    k = *iband - 1;
    tt = gmspfcgmskb3_1.tc[(k << 1) - 2] + gmspfcgmskb3_1.tc[(k << 1) - 1] * *
	    t;
    expn = exp(gmspfcgmskb3_1.fk2[k - 1] / tt) - 1.f;
    ret_val = gmspfcgmskb3_1.fk1[k - 1] / expn;
    return ret_val;
} /* gmtradgmskb3_ */

/* Subroutine */ int gmpfcogmskb3_(char *cdet, integer *iok, ftnlen cdet_len)
{
    /* Initialized data */
...
/*  Index */
    *iok = 1;
	...
	for (k = 1; k <= 3; ++k) {
	    gmspfcgmskb3_2.cwn[k - 1] = awn[k - 1];
	    gmspfcgmskb3_2.fk1[k - 1] = ak1[k - 1];
	    gmspfcgmskb3_2.fk2[k - 1] = ak2[k - 1];
	    for (j = 1; j <= 2; ++j) {
		gmspfcgmskb3_2.tcc[j + (k << 1) - 3] = acc[j + (k << 1) - 3];
	    }
	}
    } else if (*(unsigned char *)cdet == *(unsigned char *)&cb[0]) {
	for (k = 1; k <= 3; ++k) {
	    gmspfcgmskb3_2.cwn[k - 1] = bwn[k - 1];
	    gmspfcgmskb3_2.fk1[k - 1] = bk1[k - 1];
	    gmspfcgmskb3_2.fk2[k - 1] = bk2[k - 1];
	    for (j = 1; j <= 2; ++j) {
		gmspfcgmskb3_2.tcc[j + (k << 1) - 3] = bcc[j + (k << 1) - 3];
	    }
	}
    } else {
	*iok = 0;
