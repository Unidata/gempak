/* kb3gvar.f -- translated by f2c (version 19990503).
...
#define gvarxxgvarkb3_1 gvarxxgvarkb3_

struct {
    integer calflg, calarr[128];
} gvrcal3_;

#define gvrcal_1 gvrcal3_

struct {
    char caltyp[4];
} brkpntgvarkb3_;

#define brkpntgvarkb3_1 brkpntgvarkb3_
...
integer kb3inigvar_(cin, cout, iopt, cin_len, cout_len)
char *cin, *cout;
integer *iopt;
ftnlen cin_len;
ftnlen cout_len;
...
/* ----- MAXAREARQSTLEN - max length of area request string */
    ...
    /* Function Body */
    movw_(&c__5, &iopt[1], gvarxxgvarkb3_1.jopt);
    gvarxxgvarkb3_1.jtype = 0;
    gvrcal_1.calflg = 0;
    gvarxxgvarkb3_1.isou = iopt[1];
    gvarxxgvarkb3_1.ides = iopt[2];
    if (s_cmp(cin, "RAW ", (ftnlen)4, (ftnlen)4) == 0 && s_cmp(cout, "RAD ", (
	    ftnlen)4, (ftnlen)4) == 0) {
	gvarxxgvarkb3_1.jtype = 1;
    }
...
integer kb3calgvar_(calb, idir, nval, jband, ibuf)
...
shortint *ibuf;
{
    ...
    static real bias;
    extern /* Subroutine */ int chkirgvarkb3_();
    static integer ioff;
    static real valb;
    ...
    extern /* Subroutine */ int movw_();
    static real f;
    extern integer chkcodgvarkb3_();
    static integer i__, iband;
    extern doublereal gvatbbgvarkb3_();
    static real scale;
    static integer intab[256];
    extern /* Subroutine */ int exptabgvarkb3_();
    static integer tblhi, hvcod;
    static real vbavg;
    extern /* Subroutine */ int chkvisgvarkb3_();
    static real vgavg;
    static integer tbllo, itemp;
    extern doublereal tmpsclgvarkb3_();
    extern integer grysclgvarkb3_();
...
/* L5: */
	...
	for (i__ = 1; i__ <= 2; ++i__) {
	    if (idir[33] == list[i__ * 3 - 3] && iband == list[i__ * 3 - 1] &&
		     gvarxxgvarkb3_1.jtype == list[i__ * 3 - 2]) {
		indx = i__;
		goto L100;
	    }
/* L10: */
...
L16:
    ...
    list[indx * 3 - 1] = iband;
    list[indx * 3 - 2] = gvarxxgvarkb3_1.jtype;
    brktst = gvarxxgvarkb3_1.jtype == 4 && s_cmp(brkpntgvarkb3_1.caltyp, 
...
/* L36: */
	...
	chkvisgvarkb3_(&isat, iarr, newarr, &hvcod);
	vg2avg = (float)0.;
	vgavg = (float)0.;
	vbavg = (float)0.;
	if (iband == 1) {
...
/* L102: */
	...
	g2term = (float)1.;
	if (gvarxxgvarkb3_1.jtype == 3 && dabs(vg2avg) < (float)1e-4) {
...
/* L38: */
	    ...
	    exptabgvarkb3_(intab, &itab[(indx << 15) - 32768], &c__0);
	    goto L100;
	}
	if (*nval > 100) {
...
/* Computing MIN */
	...
	if (gvarxxgvarkb3_1.jtype == 1 || brktst) {
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (i__ - 1) / scale;
...
/* L40: */
	...
	} else if (gvarxxgvarkb3_1.jtype >= 3) {
	    brktst = gvarxxgvarkb3_1.jtype == 4 && s_cmp(
		    brkpntgvarkb3_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 
		    0;
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (i__ - 1) / scale;
...
/* L45: */
	...
	chkirgvarkb3_(&isat, &iband, iarr, &gain, &bias, &hvcod);
	if (isat % 2 == 0) {
	    scale = (float)16.;
	    igvband = iband + 20;
...
/* UNDEFINED CONFIGURATIO */
	...
	if (gvarxxgvarkb3_1.jtype == 3) {
	    for (i__ = 0; i__ <= 255; ++i__) {
		temp = tmpsclgvarkb3_(&i__);
		rad = gvatbbgvarkb3_(&temp, &igvband, &igvdet, &isat, &c__1);
		raw = rad * gain + bias;
...
/* L801: */
	    ...
	    exptabgvarkb3_(intab, &itab[(indx << 15) - 32768], &c__1);
	    goto L100;
	}
	if (*nval > 100) {
	    rad = gvatbbgvarkb3_(&c_b36, &igvband, &igvdet, &isat, &c__1);
	    raw = rad * gain + bias;
	    r__1 = raw * scale + (float)1.;
	    iraw = i_nint(&r__1);
/* Computing MAX */
	    i__1 = iraw + 1;
	    tbllo = max(i__1,1);
	    rad = gvatbbgvarkb3_(&c_b38, &igvband, &igvdet, &isat, &c__1);
	    raw = rad * gain + bias;
	    r__1 = raw * scale + (float)1.;
	    iraw = i_nint(&r__1);
/* Computing MIN */
	...
	if (gvarxxgvarkb3_1.jtype == 1 || gvarxxgvarkb3_1.jtype == 4 && 
		brktst) {
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (real) i__;
...
/* L6: */
	...
	} else if (gvarxxgvarkb3_1.jtype == 2 || gvarxxgvarkb3_1.jtype == 4) {
	    brktst = gvarxxgvarkb3_1.jtype == 4 && s_cmp(
		    brkpntgvarkb3_1.caltyp, "TEMP", (ftnlen)4, (ftnlen)4) == 
		    0;
	    i__1 = tblhi;
	    for (i__ = tbllo; i__ <= i__1; ++i__) {
		f = (real) i__;
		temp = (f - (float)1.) / scale;
		rad = (temp - bias) / gain;
		temp = gvatbbgvarkb3_(&rad, &igvband, &igvdet, &isat, &c__0);
		if (brktst) {
...
/* L7: */
	...
	} else if (gvarxxgvarkb3_1.jtype > 2) {
	    brktst = gvarxxgvarkb3_1.jtype == 4 && s_cmp(
		    brkpntgvarkb3_1.caltyp, "BRIT", (ftnlen)4, (ftnlen)4) == 
		    0;
	    i__1 = tblhi;
...
L100:
    mgvatb_(nval, &gvarxxgvarkb3_1.isou, &gvarxxgvarkb3_1.ides, &ibuf[1], &
	    itab[(indx << 15) - 32768]);
    ret_val = 0;
    return ret_val;
} /* kb3calgvar_ */

integer kb3optgvar_(cfunc, iin, iout, cfunc_len)
char *cfunc;
integer *iin, *iout;
ftnlen cfunc_len;
{
...
/* ----- MAXAREARQSTLEN - max length of area request string */
	...
	if (iband == 1 && isat % 2 == 0 || iband == 19 && isat % 2 == 1) {
		...
		if (brkset_(cfile, brkpntgvarkb3_1.caltyp, (ftnlen)12, (
			ftnlen)4) != 0) {
		    ret_val = -3;
		    return ret_val;
		}
	...
	if (brkset_(cfile, brkpntgvarkb3_1.caltyp, (ftnlen)12, (ftnlen)4) != 
		0) {
	    ret_val = -3;
	    return ret_val;
	}
...
doublereal gvatbbgvarkb3_(val, kch, kdet, isat, switch__)
real *val;
...
doublereal tmpsclgvarkb3_(ibrit)
integer *ibrit;
{
    /* Initialized data */
...
/* Subroutine */ int chkvisgvarkb3_(isat, codarr, visarr, hvcod)
integer *isat, *codarr;
real *visarr;
integer *hvcod;
{
    ...
    extern doublereal pctdifgvarkb3_();
    static integer itemp, codpos, newpos;
    extern doublereal frmtgvarkb3_();
    ...
    if (*isat % 2 == 0) {
	for (itemp = 1; itemp <= 8; ++itemp) {
	    r__1 = frmtgvarkb3_(&codarr[codpos]);
	    if (pctdifgvarkb3_(&ivbias[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L10: */
	...
	for (itemp = 1; itemp <= 8; ++itemp) {
	    r__1 = frmtgvarkb3_(&codarr[codpos]);
	    if (pctdifgvarkb3_(&ivgain[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L20: */
	...
	for (itemp = 1; itemp <= 8; ++itemp) {
	    r__1 = frmtgvarkb3_(&codarr[codpos]);
	    if (pctdifgvarkb3_(&iv2gain[itemp - 1], &r__1) > mxdf && *hvcod ==
		     1) {
		visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L30: */
	...
	r__1 = frmtgvarkb3_(&codarr[codpos]);
	if (pctdifgvarkb3_(&ivalb, &r__1) > mxdf && *hvcod == 1) {
	    visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
	    iflag = 1;
	} else {
	    visarr[newpos] = ivalb;
	...
	for (itemp = 1; itemp <= 4; ++itemp) {
	    r__1 = frmtgvarkb3_(&codarr[codpos]);
	    if (pctdifgvarkb3_(&svbias[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L40: */
	...
	for (itemp = 1; itemp <= 4; ++itemp) {
	    r__1 = frmtgvarkb3_(&codarr[codpos]);
	    if (pctdifgvarkb3_(&svgain[itemp - 1], &r__1) > mxdf && *hvcod == 
		    1) {
		visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L50: */
	...
	for (itemp = 1; itemp <= 4; ++itemp) {
	    r__1 = frmtgvarkb3_(&codarr[codpos]);
	    if (pctdifgvarkb3_(&sv2gain[itemp - 1], &r__1) > mxdf && *hvcod ==
		     1) {
		visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
		iflag = 1;
	    } else {
...
/* L60: */
	...
	r__1 = frmtgvarkb3_(&codarr[codpos]);
	if (pctdifgvarkb3_(&svalb, &r__1) > mxdf && *hvcod == 1) {
	    visarr[newpos] = frmtgvarkb3_(&codarr[codpos]);
	    ++iflag;
	} else {
	    visarr[newpos] = svalb;
...
/* Subroutine */ int chkirgvarkb3_(isat, iband, codarr, gain, bias, hvcod)
...
integer *hvcod;
{
    ...
    /* Local variables */
    static integer iflag;
    extern doublereal pctdifgvarkb3_(), frmtgvarkb3_();
	...
	if (*iband == 2) {
	    *bias = iirbias[2];
	    *gain = iirgain[2];
	    r__1 = frmtgvarkb3_(&codarr[ibiasoff + loc2]);
	    if (pctdifgvarkb3_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb3_(&codarr[ibiasoff + loc2]);
	    }
	    r__1 = frmtgvarkb3_(&codarr[igainoff + loc2]);
	    if (pctdifgvarkb3_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb3_(&codarr[igainoff + loc2]);
	    }
	}
	if (*iband == 3) {
	    *bias = iirbias[3];
	    *gain = iirgain[3];
	    r__1 = frmtgvarkb3_(&codarr[ibiasoff + loc3]);
	    if (pctdifgvarkb3_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb3_(&codarr[ibiasoff + loc3]);
	    }
	    r__1 = frmtgvarkb3_(&codarr[igainoff + loc3]);
	    if (pctdifgvarkb3_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb3_(&codarr[igainoff + loc3]);
	    }
	}
	if (*iband == 4) {
	    *bias = iirbias[0];
	    *gain = iirgain[0];
	    r__1 = frmtgvarkb3_(&codarr[ibiasoff + loc4]);
	    if (pctdifgvarkb3_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb3_(&codarr[ibiasoff + loc4]);
	    }
	    r__1 = frmtgvarkb3_(&codarr[igainoff + loc4]);
	    if (pctdifgvarkb3_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb3_(&codarr[igainoff + loc4]);
	    }
	}
	if (*iband == 5) {
	    *bias = iirbias[1];
	    *gain = iirgain[1];
	    r__1 = frmtgvarkb3_(&codarr[ibiasoff + loc5]);
	    if (pctdifgvarkb3_(bias, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*bias = frmtgvarkb3_(&codarr[ibiasoff + loc5]);
	    }
	    r__1 = frmtgvarkb3_(&codarr[igainoff + loc5]);
	    if (pctdifgvarkb3_(gain, &r__1) > mxdf && *hvcod == 1) {
		iflag = 1;
		*gain = frmtgvarkb3_(&codarr[igainoff + loc5]);
	    }
	}
    }
    if (*isat % 2 == 1) {
	r__1 = frmtgvarkb3_(&codarr[sgainoff + *iband]);
	if (pctdifgvarkb3_(&sirgain[*iband - 1], &r__1) > mxdf && *hvcod == 1)
		 {
	    iflag = 1;
	    *gain = frmtgvarkb3_(&codarr[sgainoff + *iband]);
	} else {
	    *gain = sirgain[*iband - 1];
	}
	r__1 = frmtgvarkb3_(&codarr[sbiasoff + *iband]);
	if (pctdifgvarkb3_(&sirbias[*iband - 1], &r__1) > mxdf && *hvcod == 1)
		 {
	    iflag = 1;
	    *bias = frmtgvarkb3_(&codarr[sbiasoff + *iband]);
	} else {
...
/* Subroutine */ int dmparrgvarkb3_(array, start, end)
real *array;
integer *start, *end;
{
    /* Parameter adjustments */
...
doublereal pctdifgvarkb3_(value1, value2)
real *value1, *value2;
{
    /* System generated locals */
    real ret_val, r__1;
...
/* Subroutine */ int exptabgvarkb3_(intab, itab, switch__)
