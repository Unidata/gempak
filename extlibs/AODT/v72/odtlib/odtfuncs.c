...
double aodtv72_calctime( int, int );
float  aodtv72_slopecal( double,int );
float  aodtv72_getpwval( int,float );
float  aodtv72_PWlandsea( float );
float  aodtv72_ptovmax( float );
float  aodtv72_ptotno( float );
int    aodtv72_cmonth2julian( char * );
void   aodtv72_julian2cmonth( int, char * );
void   aodtv72_distance( float,float,float,float,int,float *,float * );
void   aodtv72_distance2( float, float, float, float, float *, float * );
float  aodtv72_atoif( char *, int, int );
void   aodtv72_calcskew( float *, int, float *, float *, float * );
int    aodtv72_idmyyd( int,int,int );
void   aodtv72_yddmy( int,int *,int *,int * );
int    aodtv72_oceanbasin( float,float );
int    aodtv72_sattypes( int,char * );
int    aodtv72_atcffilename( char *,int,char *,char *);
int    aodtv72_initcurrent(int);

float aodtv72_slopecal(double tval,int itype)
...
*/
{
  ...
  while(odthistory!=0) {
    ...
    if((xtime<curtime)&&(xtime>=tlimit)) {
      ...
      if(landcheck) {
        ...
        if(itype==2) yvalue=odthistory->IR.Traw;   /* adjusted Raw T# */
        if(itype==3) yvalue=odthistory->IR.latitude;
        if(itype==4) yvalue=odthistory->IR.longitude;
        sumx=sumx+xvalue;
        sumy=sumy+yvalue;
  ...
  if(itype<=2) {
    ...
    if(itype==1) yvalue=odtcurrent_v72->IR.Tfinal;
    if(itype==2) yvalue=odtcurrent_v72->IR.Traw;
    sumy=sumy+yvalue;
    sumsqy=sumsqy+(yvalue*yvalue);
    icnt++;
  }
  ...
  if(icnt<icntmin) {
    slope=0.0;
    if((itype==3)||(itype==4)) slope=999.99;
    return slope;
  }
...
float aodtv72_getpwval(int ival,float cival)
...
*/
{
  ...
  while((cival>tno_v72[ixx])&&(ixx<82)) { ixx++; }

  /* convert CI value to wind_v72/pressure value */
  if(ival==1) {
    value=wind_v72[ixx];           /* WIND */
  } else {
    value=pres_v72[idomain_v72][ixx];  /* PRESSURE */
  }

  return value;
}

float aodtv72_PWlandsea( float pval )
...
float aodtv72_ptovmax( float pval )
/* derive wind_v72 Vmax value from pressure (MSLP) value
...
*/
{
  ...
  while((pval<pres_v72[idomain_v72][ixx])&&(ixx<82)) { ixx++; }

  if(ixx==2) {
    vmax=wind_v72[2];
  } else {
    xpart=(pres_v72[idomain_v72][ixx-1]-pval)/(pres_v72[idomain_v72][ixx-1]-pres_v72[idomain_v72][ixx]);
    vmax=wind_v72[ixx-1]+((wind_v72[ixx]-wind_v72[ixx-1])*xpart);
  }

  return vmax;
}

float aodtv72_ptotno( float pval )
/* derive T# value from pressure (MSLP) value
   I_v72nputs  : pressure value
...
*/
{
  ...
  if(ixx==2) {
    ...
    xpart=(pres_v72[idomain_v72][ixx-1]-pval)/(pres_v72[idomain_v72][ixx-1]-pres_v72[idomain_v72][ixx]);
    xtno=tno_v72[ixx-1]+((tno_v72[ixx]-tno_v72[ixx-1])*xpart);
  }

  return xtno;
}

int aodtv72_cmonth2julian( char *cval1 )
/* Convert YYYYMMMDD format to julian date.
   Inputs  : cval1 - character repres_v72entation of date in YYYYmonDD format
...
*/
{
  ...
  while((strncmp(cmonx,cmon_v72[imon],3)!=0)&&(imon<12)) { imon++; }

  /* determine julian date from day/month/year */
  if(imon<12) iyyddd=aodtv72_idmyyd(iday,imon+1,iyear);

  return iyyddd;
}

void aodtv72_julian2cmonth( int julian,char *cvalue )
/* convert julian date to YYYYMMMDD format for output.
   Inputs  : julian - Julian date repres_v72entation of date
   Outputs : cvalue - character repres_v72entation of date in YYYYmonDD format
             where DD and YYYY are integers and mon is a three
             character abbreviation for the month (e.g. 2000MAR13)
*/
{
  ...
  (void)aodtv72_yddmy(julian,&iday,&imon,&iyear);

  /* form character string repres_v72entation from various components */
  if(iday<10) {
    (void)sprintf(cvalue,"%4d%3s0%1d",iyear,cmon_v72[imon-1],iday);
  } else {
    (void)sprintf(cvalue,"%4d%3s%2d",iyear,cmon_v72[imon-1],iday);
  }

}

void aodtv72_distance(float rrlat,float rrlon,float pplat,float pplon,int iunit,
...
void aodtv72_distance2(float rlat,float rlon,float xdist,float xang,
...
float aodtv72_atoif(char *ptr,int beg,int end)
...
void aodtv72_calcskew( float *bin, int nbin, float *ave, float *stdv, float *skew )
...
int aodtv72_idmyyd(int day,int mon,int year)
...
void aodtv72_yddmy(int syd,int *day,int *mon,int *year)
...
int aodtv72_sattypes(int isat,char *csat)
...
int aodtv72_oceanbasin(float xlat,float xlon)
/* determine ocean basin given latitude and longitude position of storm
   Inputs  : xlat - latitude
             xlon - longitude
   Outputs : basin type (0=atlantic,1=west pacific,2,east pacific,3=Indian)
*/
{
  int   ibasin;
  float m;
  ...
  if(xlat>=0.0) {
    /* northern hemisphere */
    if(xlon<=-100.0) {
      ibasin=1;      /* West Pacific */
    } else if((xlon>-100.0)&&(xlon<=-20.0)) {
      ibasin=3;      /* Indian */
    } else if(xlon>=100.0) {
      ibasin=2;      /* East Pacific */
    } else {
      /* -20 to ~+100 */
      if(xlat>20.0) {
        ibasin=0;      /* Atlantic */
...
int aodtv72_atcffilename(char *inpath,int itype,char *stormID, char *outfile)
{
    int rawflag,finalflag,basin;
    ...
    lat=odtcurrent_v72->IR.latitude;
    lon=odtcurrent_v72->IR.longitude;
    basin=aodtv72_oceanbasin(lat,lon);
    rawflag=itype/1000;
    finalflag=(itype-(rawflag*1000))/100;
    (void)aodtv72_yddmy(odtcurrent_v72->IR.date,&iday,&imon,&iyear);
    itime=odtcurrent_v72->IR.time/100;
    printf("%d %d %d %d %s\n",iyear,imon,iday,itime,stormID);
    sprintf(filename,"%10s_%4d%2.2d%2.2d%4.4d_%3s_%3s","CIMSS_DVTO",iyear,imon,iday,itime,stormID,"FIX");
...
int aodtv72_initcurrent(int redo )
/* initialize odtcurrent_v72 array or reset values for land interaction situations */
{
  ...
  if(!redo) {
    ...
    odtcurrent_v72->IR.latitude=999.99;
    odtcurrent_v72->IR.longitude=999.99;
    odtcurrent_v72->IR.land=0;
    odtcurrent_v72->IR.autopos=0;
    strcpy(odtcurrent_v72->IR.comment,comm);
  ...
  odtcurrent_v72->IR.Traw=0.0;
  odtcurrent_v72->IR.TrawO=0.0;
  odtcurrent_v72->IR.Tfinal=0.0;
  odtcurrent_v72->IR.Tfinal3=0.0;
  odtcurrent_v72->IR.CI=0.0;
  odtcurrent_v72->IR.eyet=99.99;
  odtcurrent_v72->IR.warmt=99.99;
  odtcurrent_v72->IR.cloudt=99.99;
  odtcurrent_v72->IR.cloudt2=99.99;
  odtcurrent_v72->IR.cwcloudt=99.99;
  odtcurrent_v72->IR.warmlatitude=999.99;
  odtcurrent_v72->IR.warmlongitude=999.99;
  odtcurrent_v72->IR.eyecdosize=0.0;
  odtcurrent_v72->IR.eyestdv=0.0;
  odtcurrent_v72->IR.cloudsymave=0.0;
  odtcurrent_v72->IR.eyescene=0;
  odtcurrent_v72->IR.cloudscene=0;
  odtcurrent_v72->IR.eyesceneold=-1;
  odtcurrent_v72->IR.cloudsceneold=-1;
  odtcurrent_v72->IR.rule9=0;
  odtcurrent_v72->IR.rule8=0;
  odtcurrent_v72->IR.LBflag=0;
  odtcurrent_v72->IR.rapiddiss=0;
  odtcurrent_v72->IR.eyefft=0;
  odtcurrent_v72->IR.cloudfft=0;
  odtcurrent_v72->IR.cwring=0;
  odtcurrent_v72->IR.ringcb=0;
  odtcurrent_v72->IR.ringcbval=0;
  odtcurrent_v72->IR.ringcbvalmax=0;
  odtcurrent_v72->IR.CIadjp=0.0;
  odtcurrent_v72->IR.rmw=-99.9;
