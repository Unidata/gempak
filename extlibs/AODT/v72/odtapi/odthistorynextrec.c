...
int aodtv72_historygetnextrec(int reset,struct odtdata **histrec)
...
*/
{
  ...
  if(reset==-1) {
    *histrec=odtcurrent_v72;
    if(*histrec == (struct odtdata *)NULL) {
      return -2;    /* empty/invalid history file */
    }
  } else {
    if(recordnum==0) {
      *histrec=odthistoryfirst_v72;  /* pointer to start of structure */
      /*if(*histrec<=0) {*/
      if(*histrec == (struct odtdata *)NULL) {
    ...
    while((*histrec != (struct odtdata *)NULL)&&(!found)) {
      curtime=aodtv72_calctime((*histrec)->IR.date,(*histrec)->IR.time);
