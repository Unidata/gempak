...
extern int aodtv72_msg(int, int, float, float, char *, char *);

int aodtv72_qmessage(int errorcode,int inint1,char *instrng, char *message)
...
*/
{
  ...
  if(((errorcode>=20)&&(errorcode<=29))||
    ...
    infloat1=odtcurrent_v72->IR.latitude;
    infloat2=odtcurrent_v72->IR.longitude;
  }
  if((errorcode>=90)&&(errorcode<=99)) {
    infloat1=odtcurrent_v72->IR.warmlatitude;
    infloat2=odtcurrent_v72->IR.warmlongitude;
  }

  /* assign extra floating point value to integer value for output */
  if(errorcode==51) inint1=odtcurrent_v72->IR.autopos;
  /* if(errorcode==81) inint1=(int)(10.0*odtcurrent_v72->IR.sst); */
  if(errorcode==91) inint1=(int)(10.0*odtcurrent_v72->IR.warmt);

  iok=aodtv72_msg(errorcode,inint1,infloat1,infloat2,instrng,retstrng);
  ...
  if(errorcode<0) {
    ...
    strcat(diagnostics_v72,retstrng);
    message[0]='\0';
  }
