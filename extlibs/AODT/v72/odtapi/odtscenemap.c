...
int aodtv72_scenemap( int eyescene,int cloudscene, char *retstring )
...
*/
{
  ...
  if(eyescene==-1) {
    strcpy(retstring,cloudtype_v72[cloudscene]);
    retstring[strlen(cloudtype_v72[cloudscene])]='\0';
  } else if(cloudscene==-1) {
    strcpy(retstring,eyetype_v72[eyescene]);
    retstring[strlen(eyetype_v72[eyescene])]='\0';
  } else {
