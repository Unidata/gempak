...
int aodtv72_getlocation( float *olat, float *olon, int *opos )
...
*/
{
  ...
  odtcurrent_v72->IR.latitude=ilat;
  /* assign current storm center longitude value to AODT library variable */
  odtcurrent_v72->IR.longitude=ilon;
  /* assign current storm center positioning flag to AODT library variable */
  odtcurrent_v72->IR.autopos=ipos;
  if((odtcurrent_v72->IR.longitude<-180.)||(odtcurrent_v72->IR.longitude>180.)) iret=-21;
  if((odtcurrent_v72->IR.latitude<-90.)||(odtcurrent_v72->IR.latitude>90.)) iret=-21;
