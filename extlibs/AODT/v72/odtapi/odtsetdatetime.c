...
int aodtv72_setdatetime( int it1, int it2, char *id1, char *id2, int idelete )
/* set history file examination boundary dates/times within AODT library memory
   Inputs : AODT library start and end times/dates for history file examination
            also history file delete flag (for setting starttime and endtime variables
   Outputs: none
...
*/
{
  ...
  iret=0;
  iok=aodtv72_datetime(it1,it2,id1,id2,idelete);
  if(iok!=0) iret=-61;
  return iret;
}
