...
int aodtv72_gettopovalue( int *topo )
...
int aodtv72_settopovalue( int curtopo )
...
*/
{
  ...
  if((odtcurrent_v72->IR.latitude<-999.0)||(odtcurrent_v72->IR.latitude<-999.0)) {
    iret=-21;
  } else {
    /* set topography flag structure element for AODT library  */
    odtcurrent_v72->IR.land=curtopo;
    iret=0;
    if((odtcurrent_v72->IR.land<0)||(odtcurrent_v72->IR.land>2)) iret=-33;
  }
