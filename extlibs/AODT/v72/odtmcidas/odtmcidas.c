...
int mcidas_initialize( void )
{
  ...
  topopath=(char *)calloc((size_t)200,sizeof(char));
  histpath=(char *)calloc((size_t)200,sizeof(char));
  sstpath=(char *)calloc((size_t)200,sizeof(char));
  listpath=(char *)calloc((size_t)200,sizeof(char));
  autopath=(char *)calloc((size_t)200,sizeof(char));
  datapath=(char *)calloc((size_t)200,sizeof(char));
...
*/
{
  ...
  iras = Mcluc (63);    /* obtain raster value at cursor */
  ipix = Mcluc (64);    /* obtain pixel value at cursor */
...
*/
{
  ...
  mcsatfile=(char *)calloc((size_t)200,sizeof(char));
...
*/
  ...
  ARABOX(fdir,fstln,lstln,fstel,lstel,band,"TEMP",4,nele,&scale,(long *)istrm);
  ...
  for(line=fstln;line<=lstln;line++) {
    ...
    for(ixx=0;ixx<nele;ixx++) {
      ...
      temps[iyy][ixx]=(float)istrm[kk]/(float)scale;
      /* printf("temp=%d scale=%d\n",istrm[kk],scale);  */
...
*/
{
   ...
   if(cscenei != csceneo) {
     ...
     *origcloud=cscenei;
     ...
     if((csceneo<=4)&&(esceneo==3)) {
       aodt_classifyredo( );
     } else {
       if(esceneo==1) odtcurrent->IR.eyecdosize=5.0;
       if(esceneo==2) odtcurrent->IR.eyecdosize=45.0;
     }
     iret=32;
...
{
  FILE *fp;
  int  maxline=200,lwidth=2,hfilelen,hfilelen2,bytes,iok;
  ...
  int  escene,cscene,efft,cfft,rcb,rcbv,rapdiss;
  ...
  float ecsize,estdv,csym,ciadjp,rmw,cip;
  ...
  char *p1,*p2,*stormname,*stormtitle;
  /*logical TIEavl=FALSE;*/
...
*/
  ...
  it1=10;
  wrtext_(&it1,&eb,&const11,"UW-CIMSS ADT Tropical Cyclone Intensity Estimate  ",&const50,&iword,(size_t)50);
  it1=35;
  it2=39;
...
*/
  ...
  p2--;
  hfilelen=p2-p1+1;
  stormname=(char *)calloc((size_t)100,(size_t)sizeof(char));
  stormtitle=(char *)calloc((size_t)100,(size_t)sizeof(char));
  (void)strncpy((char *)&stormname[0],p1,hfilelen);
  sprintf(stormtitle,"STORM NAME : ");
  strncat(stormtitle,stormname,strlen(stormname));
  hfilelen2=strlen(stormtitle);
  wrtext_(&it1,&it2,&const9,stormtitle,&hfilelen2,&iword,(size_t)hfilelen2);
...
*/
{
  ...
  *time=bufarr[4];
  ...
  if(isat==85) *rsat= 7; /* strcpy(csat,"MTSAT2"); */
...
int mcidas_getremotename( int larea,char *lserver,char *satname )
{
  ...
  cband=(char *)calloc((size_t)25,sizeof(char));
  name=(char *)calloc((size_t)25,sizeof(char));
  llpos=(char *)calloc((size_t)40,sizeof(char));
  cname=(char *)calloc((size_t)50,sizeof(char));
  fname=(char *)calloc((size_t)64,sizeof(char));
  carea=(char *)calloc((size_t)100,sizeof(char));
  command=(char *)calloc((size_t)200,sizeof(char));
  iout2=(char *)calloc((size_t)5000,sizeof(char));
