...
void mcidas_stradd(char *, char *, int *last, int len1, int len2);
...
int mcidas_getpaths(char *hpath,char *tpath,char *spath,char *lpath,char *apath,char *dpath)
{
  ...
  dpathx=(char *)calloc((size_t)200,sizeof(char));
...
int mcidas_getinputs(int nnarg, char **vvarg,
                    ...
                    logical *oremote,logical *osearch,logical *ocommadd,logical *oatcf,logical *ovrad,
                    ...
                    int *igraph,int *iword,int *windvmax,int *windgrid,int *rmwsize,
                    float *osstr,
                    char *hfile,char *listfile,char *fixfile,char *lserver,char *topofile,
                    char *sstfile,char *atcfpath,char *stormID,char *atcfsrcID,
                    char *gridpath,char *comment,char *od1,char *od2)
...
*/
{
  char *ccgraph,*cclist,*ccdel,*ccwind,*ccdomain,*catcfsrc,*catcfout,*cstormID;
  char *ccauto,*ccoverride,*ccdmp,*ccrem,*ccland,*ccsearch,*cccommadd,*ccvrad;
  ...
  *ovrad=FALSE;
  ...
  if(strlen(tmpfile)>0) {
    ...
    hfilex=(char *)calloc((size_t)50,sizeof(char));
    if(found==2) {
  ...
  *larea=0000;
  ...
  if(strncmp(ccgraph,"NO",2)!=0) {
    ...
    *drawTIEa=0; /* TIE Model value color - averaged value */
    *drawTIEr=0; /* TIE Model value color - raw value */
  ...
  cstormID=mcidas_ckwp("ATCF",1,"XX");             /* storm ID... such as 04L,10W,etc. */
  strncat(stormID,cstormID,strlen(cstormID));
  *listtype=-1;
  if(!strncmp(stormID,"XX",2)==0) {
    *oatcf=TRUE;
    ...
    *listtype=(atcff1*10)+atcff2;                  /* + value indicates storm number with various ATCF output flags */
  ...
  *osstr=(float)mcidas_dkwp("IC",1,1.0);
  ...
  /* check for VRAD output keyword */
  ccvrad=mcidas_ckwp("VRAD",1,"NO");
  if(strncmp(ccvrad,"YES",3)==0) {
    *ovrad=TRUE;
    *windvmax=mcidas_ikwp("VRAD",2,-99);
    *windgrid=mcidas_ikwp("VRAD",3,8888);
    strcpy(gridpath,listpath);
    gridpath[strlen(gridpath)]='\0';
  }
  free(ccvrad);

  /* check for RMW output keyword */
  *rmwsize=mcidas_ikwp("RMW",1,-99);
...
void mcidas_setfil ()
{
char *cfile, *cprint, *cprint2, cline[151], wline[1000];
...
cprint2 = mcidas_ckwp("PRT",2,"   ");
scmp = strncmp(cprint2,"   ",3);
if (scmp != 0)
 {/*printf("write to file for stderr: %s\n",cprint);*/
  fp = freopen(cprint2,"w",stderr);
 }
  if(cprint2 != (char *)NULL) {
    free(cprint2);
    cprint2=NULL;
  }
}
