/* nv3msgt.f -- translated by f2c (version 20031025).
...
#define msgmsgtnv3_1 msgmsgtnv3_

struct {
    integer loff, coff, lfac, cfac;
} nvparammsgtnv3_;

#define nvparammsgtnv3_1 nvparammsgtnv3_

integer nv3inimsgt_(integer *ifunc, integer *iparms)
{
	...
	nvparammsgtnv3_1.loff = iparms[2];
	nvparammsgtnv3_1.coff = iparms[3];
	nvparammsgtnv3_1.lfac = iparms[4];
	nvparammsgtnv3_1.cfac = iparms[5];
    } else if (*ifunc == 2) {
	clit_(ch__1, (ftnlen)4, &iparms[1]);
	if (i_indx(ch__1, "XY", (ftnlen)4, (ftnlen)2) != 0) {
	    msgmsgtnv3_1.itype = 1;
	}
	clit_(ch__1, (ftnlen)4, &iparms[1]);
	if (i_indx(ch__1, "LL", (ftnlen)4, (ftnlen)2) != 0) {
	    msgmsgtnv3_1.itype = 2;
	}
    }
    return ret_val;
} /* nv3inimsgt_ */

integer nv3saemsgt_(real *xlin, real *xele, real *xdum, real *xlat, real *
	xlon, real *z__)
{
    /* System generated locals */
    integer ret_val;
    ...
    /* Local variables */
    extern /* Subroutine */ int msg_to_llmsgtnv3__(real *, real *, real *, 
	    real *);
    static real a, b;
    ...
    msg_to_llmsgtnv3__(&r__1, &r__2, xlat, xlon);
    if (*xlat < -900.f) {
	ret_val = -1;
	return ret_val;
    }
    *xlon = -(*xlon);
    if (msgmsgtnv3_1.itype == 1) {
	a = *xlat;
	b = *xlon;
...
integer nv3easmsgt_(real *xlat, real *xlon, real *z__, real *xlin, real *xele,
	 real *xdum)
{
    /* System generated locals */
    integer ret_val;
    ...
    /* Local variables */
    extern /* Subroutine */ int ll_to_msgmsgtnv3__(real *, real *, real *, 
	    real *);
    static real a, b;
    ...
    ll_to_msgmsgtnv3__(&a, &b, xlin, xele);
    if (*xlin < 0.f) {
	ret_val = -1;
	return ret_val;
    }
...
integer nv3optmsgt_(integer *ifunc, real *xin, real *xout)
{
    /* System generated locals */
    integer ret_val;
    char ch__1[4];
...
/* Subroutine */ int ll_to_msgmsgtnv3__(real *xlat, real *xlon, real *xlin, 
	real *xele)
{
    /* Builtin functions */
...
/*       Subroutine assumes that line 1856.5 and element 1856.5 is 0/0 deg */
    ...
    deltax = 1.f / (nvparammsgtnv3_1.cfac / 1e6f);
    deltay = 1.f / (nvparammsgtnv3_1.lfac / 1e6f);
    xfi = *xlat * cdr;
    xla = *xlon * cdr;
	...
	*xele = -999.f;
    ...
    xr = px / deltax;
    yr = py / deltay;
    *xele = nvparammsgtnv3_1.coff / 10.f - xr;
    *xlin = nvparammsgtnv3_1.loff / 10.f - yr;
    *xele = 3713.f - *xele;
    *xlin = 3713.f - *xlin;
    return 0;
} /* ll_to_msgmsgtnv3__ */

/* Subroutine */ int msg_to_llmsgtnv3__(real *xlin, real *xele, real *xlat, 
	real *xlon)
{
    /* Builtin functions */
...
/*       Subroutine assumes that line 1856.5 and element 1856.5 is 0/0 deg */
    ...
    deltax = 1.f / (nvparammsgtnv3_1.cfac / 1e6f);
    deltay = 1.f / (nvparammsgtnv3_1.lfac / 1e6f);
    xr = *xele - nvparammsgtnv3_1.coff / 10.f;
    yr = *xlin - nvparammsgtnv3_1.loff / 10.f;
    xr = xr * deltax * cdr;
    yr = yr * deltay * cdr;
    tanx = tan(xr);
    tany = tan(yr);
