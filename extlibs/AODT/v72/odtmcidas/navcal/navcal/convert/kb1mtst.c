/* kb1mtst.f -- translated by f2c (version 20031025).
...
#define fy2mtstkb1_1 fy2mtstkb1_

/* Table of constant values */

static integer c__26112 = 26112;

integer kb1inimtst_(char *input_cal__, char *output_cal__, integer *io_size__,
...
/* number of bytes for source pixel */
    ...
    /* Function Body */
    s_copy(fy2mtstkb1_1.cal_input__, input_cal__, (ftnlen)4, (ftnlen)4);
    s_copy(fy2mtstkb1_1.cal_output__, output_cal__, (ftnlen)4, (ftnlen)4);
    if (s_cmp(fy2mtstkb1_1.cal_input__, "RAW", (ftnlen)3, (ftnlen)3) != 0) {
	ret_val = -1;
    } else if (s_cmp(fy2mtstkb1_1.cal_output__, "TEMP", (ftnlen)4, (ftnlen)4) 
	    != 0 && s_cmp(fy2mtstkb1_1.cal_output__, "RAD", (ftnlen)3, (
	    ftnlen)3) != 0 && s_cmp(fy2mtstkb1_1.cal_output__, "RAW", (ftnlen)
	    3, (ftnlen)3) != 0 && s_cmp(fy2mtstkb1_1.cal_output__, "ALB", (
	    ftnlen)3, (ftnlen)3) != 0 && s_cmp(fy2mtstkb1_1.cal_output__, 
	    "BRIT", (ftnlen)4, (ftnlen)4) != 0) {
	ret_val = -1;
    } else {
	fy2mtstkb1_1.src_byte_size__ = io_size__[1];
	fy2mtstkb1_1.dest_byte_size__ = io_size__[2];
	ret_val = 0;
    }
    return ret_val;
} /* kb1inimtst_ */

integer kb1calmtst_(integer *prefix, integer *area_dir__, integer *
	num_pixels__, integer *band, integer *ibuf)
{
    ...
    static integer detector, i__, j, alb_table__[256], cal_block__[6528], 
	    rad_table__[1024], temp_brit__[1024], store_albedo_brit__[1280]	
	    /* was [5][256] */, cal_offset__, temp_table__[1024];
    extern doublereal temp_to_radmtstkb1__(real *, integer *);
    static integer vis_offset__, albedo_brit__[256], area_number__;
...
/* radiance returned from temp_to_radmtstkb1 */
...
/* L40: */
	...
	for (i__ = 1; i__ <= 1024; ++i__) {
	    temperature = (real) cal_block__[ir_band_offset__ + i__ - 1] / 
		    1e3f;
	    radiance = temp_to_radmtstkb1__(&temperature, band);
...
/* L110: */
    ...
    if (vis_data__) {
	if (s_cmp(fy2mtstkb1_1.cal_output__, "RAW", (ftnlen)3, (ftnlen)3) == 
		0) {
	    mpixel_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1]);
	} else if (s_cmp(fy2mtstkb1_1.cal_output__, "ALB", (ftnlen)3, (ftnlen)
		3) == 0) {
	    mpixtb_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1], alb_table__);
	} else if (s_cmp(fy2mtstkb1_1.cal_output__, "BRIT", (ftnlen)4, (
		ftnlen)4) == 0) {
	    mpixtb_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1], albedo_brit__);
	}
    }
    if (ir_data__) {
	if (s_cmp(fy2mtstkb1_1.cal_output__, "RAW", (ftnlen)3, (ftnlen)3) == 
	    ...
	    mpixel_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1]);
	} else if (s_cmp(fy2mtstkb1_1.cal_output__, "RAD", (ftnlen)3, (ftnlen)
		3) == 0) {
	    mpixtb_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1], rad_table__);
	} else if (s_cmp(fy2mtstkb1_1.cal_output__, "TEMP", (ftnlen)4, (
		ftnlen)4) == 0) {
	    mpixtb_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1], temp_table__);
	} else if (s_cmp(fy2mtstkb1_1.cal_output__, "BRIT", (ftnlen)4, (
		ftnlen)4) == 0) {
	    mpixtb_(num_pixels__, &fy2mtstkb1_1.src_byte_size__, &
		    fy2mtstkb1_1.dest_byte_size__, &ibuf[1], temp_brit__);
	}
    }
    last_band__ = *band;
    return ret_val;
...
integer kb1optmtst_(char *option, integer *param_in__, integer *param_out__, 
	ftnlen option_len)
{
    /* System generated locals */
    integer ret_val;
...
/* flag indicating visible data requested */
	...
	if (ischar_(&param_in__[38]) == 1) {
	    ...
	    if (brkset_(su_file__, fy2mtstkb1_1.cal_input__, (ftnlen)8, (
		    ftnlen)4) != 0) {
		ret_val = -3;
	    }
	}
	...
	if (brkset_(su_file__, fy2mtstkb1_1.cal_input__, (ftnlen)8, (ftnlen)4)
		 != 0) {
	    ret_val = -3;
	}
...
doublereal temp_to_radmtstkb1__(real *temperature, integer *band)
{
    /* Initialized data */

    static real fk1[4] = { 9485.28809f,6888.16162f,38792.1172f,222569.438f };
    static real fk2[4] = { 1333.62585f,1198.72144f,2132.70923f,3818.0271f };
    static real tc[8]	/* was [2][4] */ = { .341152728f,.998827279f,
	    .18731679f,.999288201f,.380203456f,.999109149f,2.36744547f,
	    .996900678f };
