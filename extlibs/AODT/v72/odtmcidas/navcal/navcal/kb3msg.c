/* kb3msg.f -- translated by f2c (version 20031025).
...
#define msgcommsgkb3_1 msgcommsgkb3_
...
integer kb3inimsg_(char *cin, char *cout, integer *iopt, ftnlen cin_len, 
	ftnlen cout_len)
{
...
integer kb3calmsg_(integer *pfx, integer *idir, integer *nval, integer *band, 
	shortint *ibuf)
{
    /* Initialized data */
...
/* local variables */
	...
	if (msgcommsgkb3_1.calflg != 0) {
	    movw_(&c__51, msgcommsgkb3_1.calarr, buf);
	} else {
	    araget_(&idir[33], &idir[63], &c__104, buf);
	}
	if (s_cmp(cbuf, "MSGT", (ftnlen)4, (ftnlen)4) == 0) {
	    if (msgcommsgkb3_1.calflg != 0) {
		movw_(&c__313, msgcommsgkb3_1.calarr, buf);
	    } else {
...
/* Writing concatenation */
	...
	isou = msgcommsgkb3_1.jopt[0];
	ides = msgcommsgkb3_1.jopt[1];
    }
    i__2 = *nval;
    for (i__ = 1; i__ <= i__2; ++i__) {
	itemp = ibuf[i__];
	if (*band < 4 || *band == 12) {
	    if (msgcommsgkb3_1.itype == 4) {
		ibuf[i__] = 0;
	    } else {
		...
		if (msgcommsgkb3_1.itype == 2) {
		    r__1 = xtemp * 100.f;
		    ibuf[i__] = (shortint) i_nint(&r__1);
		} else if (msgcommsgkb3_1.itype == 3) {
		...
		ibuf[i__] = 0;
	    } else if (msgcommsgkb3_1.itype == 4) {
		if (xtemp > 0.f) {
...
integer kb3optmsg_(char *cfunc, integer *iin, integer *iout, ftnlen cfunc_len)
{
    /* System generated locals */
    integer ret_val;
...
/* ----- MAXAREARQSTLEN - max length of area request string */
	...
	msgcommsgkb3_1.calflg = 1;
	movw_(&c__313, &iin[1], msgcommsgkb3_1.calarr);
    } else {
	ret_val = -1;
    }
    return ret_val;
