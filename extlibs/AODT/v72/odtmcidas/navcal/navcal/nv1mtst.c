/* nv1mtst.f -- translated by f2c (version 19990503).
...
#define mtstmtstnv1_1 mtstmtstnv1_

struct {
    integer loff, coff, lfac, cfac;
} nvparammtstnv1_;

#define nvparammtstnv1_1 nvparammtstnv1_

integer nv1inimtst_(integer *ifunc, integer *iparms)
{
	...
	nvparammtstnv1_1.loff = iparms[2];
	nvparammtstnv1_1.coff = iparms[3];
	nvparammtstnv1_1.lfac = iparms[4];
	nvparammtstnv1_1.cfac = iparms[5];
    } else if (*ifunc == 2) {
	clit_(ch__1, (ftnlen)4, &iparms[1]);
	if (i_indx(ch__1, "XY", (ftnlen)4, (ftnlen)2) != 0) {
	    mtstmtstnv1_1.itype = 1;
	}
	clit_(ch__1, (ftnlen)4, &iparms[1]);
	if (i_indx(ch__1, "LL", (ftnlen)4, (ftnlen)2) != 0) {
	    mtstmtstnv1_1.itype = 2;
	}
    }
    return ret_val;
} /* nv1inimtst_ */

integer nv1saemtst_(real *xlin, real *xele, real *xdum, real *xlat, real *
	xlon, real *z__)
{
    /* System generated locals */
    integer ret_val;
    ...
    /* Local variables */
    static real a, b;
    extern /* Subroutine */ int mtst_to_llmtstnv1__(real *, real *, real *, 
	    real *), nllxyz_(real *, real *, real *, real *, real *);

    ret_val = 0;
    r__1 = 2750.f - (*xlin - 2) / 4.f;
    r__2 = 2750.f - (*xele - 2) / 4.f;
    mtst_to_llmtstnv1__(&r__1, &r__2, xlat, xlon);
    if (*xlat < -900.f) {
	ret_val = -1;
	return ret_val;
    }
    *xlon = -(*xlon);
    if (mtstmtstnv1_1.itype == 1) {
	a = *xlat;
	b = *xlon;
...
integer nv1easmtst_(real *xlat, real *xlon, real *z__, real *xlin, real *xele,
	 real *xdum)
{
    /* System generated locals */
    integer ret_val;
    ...
    /* Local variables */
    static real a, b;
    extern /* Subroutine */ int ll_to_mtstmtstnv1__(real *, real *, real *, 
    ...
    ll_to_mtstmtstnv1__(&a, &b, xlin, xele);
    if (*xlin < 0.f) {
	ret_val = -1;
	return ret_val;
    }
...
integer nv1optmtst_(integer *ifunc, real *xin, real *xout)
{
    /* System generated locals */
    integer ret_val;
    char ch__1[4];
...
/* Subroutine */ int ll_to_mtstmtstnv1__(real *xlat, real *xlon, real *xlin, 
	real *xele)
{
    /* Builtin functions */
...
/*       Subroutine assumes that line 1375 and element 1375 is 0/0 deg */
    ...
    deltax = 1.f / (nvparammtstnv1_1.cfac / 1e6f);
    deltay = 1.f / (nvparammtstnv1_1.lfac / 1e6f);
    xfi = *xlat * cdr;
	...
	*xele = -999.f;
    ...
    xr = px / deltax;
    yr = py / deltay;
    *xele = nvparammtstnv1_1.coff / 10.f - xr;
    *xlin = nvparammtstnv1_1.loff / 10.f - yr;
    *xele = 2751.f - *xele;
    *xlin = 2751.f - *xlin;
    return 0;
} /* ll_to_mtstmtstnv1__ */

/* Subroutine */ int mtst_to_llmtstnv1__(real *xlin, real *xele, real *xlat, 
	real *xlon)
{
    /* Builtin functions */
...
/*       Subroutine assumes that line 1375 and element 1375 is 0/0 deg */
    ...
    deltax = 1.f / (nvparammtstnv1_1.cfac / 1e6f);
    deltay = 1.f / (nvparammtstnv1_1.lfac / 1e6f);
    xr = *xele - nvparammtstnv1_1.coff / 10.f;
    yr = *xlin - nvparammtstnv1_1.loff / 10.f;
    xr = xr * deltax * cdr;
    yr = yr * deltay * cdr;
    tanx = tan(xr);
    tany = tan(yr);
