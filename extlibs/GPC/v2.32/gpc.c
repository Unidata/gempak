...
/* GPC_EPSILON */
double	_GPC_epsilon = GPC_EPSILON;
                      ...
                      gpc_polygon *result)
{
  ...
  while (scanbeam < sbt_entries)
  {
    ...
    for (next_edge= aet->next; next_edge; next_edge= next_edge->next)
    {
      ...
      {
        if (EQ(e0->xb, next_edge->xb, _GPC_epsilon) && 
	    EQ(e0->dx, next_edge->dx, _GPC_epsilon)
	 && (e0->top.y != yb))
        {
