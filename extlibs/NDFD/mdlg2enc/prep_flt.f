...
C
            DO 90 K=1,NVAL
               IF((A(K).EQ.XMISSP).OR.(A(K).EQ.XMISSS))GOTO 90
               A(K)=A(K)*D
 90         CONTINUE
C
         ENDIF
...
C
         DO 95 K=1,NVAL
            IF((A(K).NE.XMISSP).AND.(A(K).NE.XMISSS))GOTO 96
            IPOS=IPOS+1
 95      CONTINUE
 96      CONTINUE
...
C
         DO 100 K=IPOS+1,NVAL
            IF((A(K).EQ.XMISSP).OR.(A(K).EQ.XMISSS))GOTO 100
            IF(A(K).LT.XMINA)XMINA=A(K)
 100     CONTINUE
C
         IF(XMINA.NE.0.)THEN
C
            DO 110 K=1,NVAL
               IF((A(K).EQ.XMISSP).OR.(A(K).EQ.XMISSS))GOTO 110
               A(K)=A(K)-XMINA
               IF(A(K).GT.MALLOW)GO TO 901
C                 A(K) SHOULD NOT EXCEED 2**30.  THIS IS CAUSED BY TOO LARGE
C                 A RANGE OF VALUES.
