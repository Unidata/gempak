...
C
            DO 90 K=1,NVAL
               IF((IA(K).EQ.MISSP).OR.(IA(K).EQ.MISSS))GOTO 90
               IA(K)=IA(K)*JD
 90         CONTINUE
C
         ENDIF
...
C
         DO 95 K=1,NVAL
            IF((IA(K).NE.MISSP).AND.(IA(K).NE.MISSS))GOTO 96
            IPOS=IPOS+1
 95      CONTINUE
 96      CONTINUE
C
...
C
         DO 100 K=IPOS+1,NVAL
            IF((IA(K).EQ.MISSP).OR.(IA(K).EQ.MISSS))GOTO 100
            IF(IA(K).LT.MINA)MINA=IA(K)
 100     CONTINUE
C
         IF(MINA.NE.0)THEN
C
            DO 110 K=1,NVAL
               IF((IA(K).EQ.MISSP).OR.(IA(K).EQ.MISSS))GOTO 110
               IA(K)=IA(K)-MINA
               IF(IA(K).LT.0)GO TO 901
C                 IA(K) SHOULD NOT BE NEGATIVE.  THIS IS PROBABLY AN
C                 OVERFLOW BECAUSE OF TOO LARGE A RANGE OF VALUES.
...
C
            DO 120 K=1,NVAL
               IF((IA(K).EQ.MISSP).OR.(IA(K).EQ.MISSS))GOTO 120
               IA(K)=IA(K)*JE
 120        CONTINUE
C
         ENDIF
