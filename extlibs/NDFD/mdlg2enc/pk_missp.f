...
C
C      DIMENSION IA(NXY),IB(NXY),IWORK(NXY)
      DIMENSION IA(NXY),IB(NXY),IWORK(800000)
C
      DATA IFEED/12/
C
      IER=0
...
C         IF(IWORK(K).EQ.MISSP)CYCLE
         IF(IWORK(K).EQ.MISSP)GOTO 132
         IF(IWORK(K).LT.IMINA)IMINA=IWORK(K)
 132  CONTINUE
C
      DO 134 K=ISECPOS+1,NXY
C         IF(IWORK(K).EQ.MISSP)CYCLE
         IF(IWORK(K).EQ.MISSP)GOTO 134
...
C            IF(IA(J).EQ.MISSP)CYCLE
            IF(IA(J).EQ.MISSP)GOTO 135
