      SUBROUTINE UNPK_POLSTER(KFILDO,IPACK,ND5,IS3,NS3,L3264B,
     1                        LOCN,IPOS,BOUST,IER,*)
C
C        MARCH    2000   LAWRENCE  GSC/TDL    ORIGINAL CODING
C        JANUARY  2001   GLAHN     COMMENTS; ADDED * ARGUMENT
C                                  TO CALL TO UNEARTH; ADDED
C                                  ARGUEMNT BOUST IN CALL; CHANGED
...
C                                  ADDED SETTING OF BOUST
C
C        PURPOSE
C            UNPACKS TEMPLATE 3.20, A POLAR STEREOGRAPHIC MAP
...
C              IS3(J) = THE POLAR STEREOGRAPHIC PROJECTION
C                       INFORMATION IS WRITTEN TO ELEMENTS 15 THROUGH
C                       65 AS IT IS UNPACKED FROM IPACK( ) (J=1,NS3).
C                       (INPUT/OUTPUT)
...
C
C             LOCAL VARIABLES
C               ISIGN = A FLAG INDICATING WHETHER A UNPACKED VALUE
...
C
      ...
      IER=0
C
C        CHECK TO MAKE SURE THAT A POLAR STEREOGRAPHIC PROJECTION
...
C
C        UNPACK THE LATITUDE WHERE Dx AND Dy ARE SPECIFIED.
      CALL UNPKBG(KFILDO,IPACK,ND5,LOCN,IPOS,ISIGN,1,N,IER,*900)
      CALL UNPKBG(KFILDO,IPACK,ND5,LOCN,IPOS,IS3(48),
     1          31,N,IER,*900)
      IF(ISIGN.EQ.1)IS3(48)=-IS3(48)
C
C        UNPACK THE ORIENTATION OF THE GRID.
      CALL UNPKBG(KFILDO,IPACK,ND5,LOCN,IPOS,ISIGN,1,N,IER,*900)
