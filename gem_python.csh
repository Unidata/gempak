...
if ( -d /awips2/python ) then
    ...
    set pv = `$PYHOME/bin/python -V |& cut -c8- | cut -d. -f1`
    set pr = `$PYHOME/bin/python -V |& cut -c8- | cut -d. -f2`
    setenv PYTHONPATH "${PYHOME}/lib64/python${pv}.${pr}/site-packages:${PYHOME}/lib/python${pv}.${pr}/site-packages:${NAWIPS}/scripts/python"
    if ( ! $?MACHTYPE ) then
      # set the MACHTYPE
      set MACHTYPE = `uname -m`
    endif 
    set MACHTEST = `echo $MACHTYPE | grep -c "_64"`
    set IS64 = `echo $MACHTEST | grep -c "_64"`
    if ( ${IS64} > 0 ) then
      set ARCH = "64"
    endif
    setenv LD_LIBRARY_PATH ${LD_LIBRARY_PATH}:/usr/lib${ARCH}
endif
