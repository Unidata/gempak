...
if [ "$os_type" = "centos" ]; then
    ...
    if [ "$os_version" = "container" ]; then
        ...
        sudo docker run --rm=true -v `pwd`:/gempak:rw unidata/$IMAGE:$TAG /bin/bash -c "bash -xe /gempak/build/build_extlibs.sh"
        ...
        sudo docker exec -ti $DOCKER_CONTAINER_ID /bin/bash -xec "bash -xe /gempak/build/build_extlibs.sh";
        sudo docker ps -a
    ...
    sudo docker run --rm=true -v `pwd`:/gempak:rw unidata/$IMAGE:fedora /bin/bash -c "bash -xe /gempak/build/build_extlibs.sh"
